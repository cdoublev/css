...
describe('<random()>', () => {
    ...
    test('valid', () => {
        ...
        assert.valid('<length>', 'random(fixed 1, 0em, 1em)', 'random(fixed 1, 0em, 1em)')
        assert.valid('<length-percentage>', 'random(fixed 1, 0%, 1%)', 'random(fixed 1, 0%, 1%)')
        assert.valid('<percentage>', 'random(fixed 1, 0%, 1%)', 'calc(1%)')
        // Different units
        assert.valid('<length>', 'random(fixed 1, 1px, 1in)', 'calc(96px)')
        assert.valid('<length>', 'random(fixed 1, 1px, 1em)', 'random(fixed 1, 1px, 1em)')
        assert.valid('<length-percentage>', 'random(fixed 1, 1px, 1%)', 'random(fixed 1, 1px, 1%)')
        // Infinite minimum or step results to the minimum
        assert.valid('<number>', 'random(fixed 1, 0, 1, infinity)', 'calc(0)')
        assert.valid('<number>', 'random(fixed 1, infinity, 1)', 'calc(infinity)')
        assert.valid('<number>', 'random(fixed 1, -infinity, 1)', 'calc(-infinity)')
        // Discarded step when its multiplier is infinite
        assert.valid('<number>', 'random(fixed 0.1, 0, 1, 0)', 'calc(0.1)')
    })
})
...
describe('<a-n-plus-b>', () => {
    test('invalid', () => {
        assert.invalid('<a-n-plus-b>', '+ n-1')
        assert.invalid('<a-n-plus-b>', '+ n- 1')
        assert.invalid('<a-n-plus-b>', '+ n -1')
        assert.invalid('<a-n-plus-b>', '+ n - 1')
    })
    test('representation', () => {
        assert.representation('<a-n-plus-b>', 'even', { types: ['<a-n-plus-b>'], value: { a: 2, b: 0 } })
        assert.representation('<a-n-plus-b>', '1n+1', { types: ['<a-n-plus-b>'], value: { a: 1, b: 1 } })
    })
    test('valid', () => {
        assert.valid('<a-n-plus-b>', 'even', '2n')
        assert.valid('<a-n-plus-b>', 'odd', '2n+1')
        assert.valid('<a-n-plus-b>', '1')
        assert.valid('<a-n-plus-b>', '1n', 'n')
        assert.valid('<a-n-plus-b>', 'n')
        assert.valid('<a-n-plus-b>', '+n', 'n')
        assert.valid('<a-n-plus-b>', '-n')
        assert.valid('<a-n-plus-b>', '1n-1', 'n-1')
        assert.valid('<a-n-plus-b>', 'n-1')
        assert.valid('<a-n-plus-b>', '+n-1', 'n-1')
        assert.valid('<a-n-plus-b>', '-n-1')
        assert.valid('<a-n-plus-b>', '1n -1', 'n-1')
        assert.valid('<a-n-plus-b>', 'n -1', 'n-1')
        assert.valid('<a-n-plus-b>', '+n -1', 'n-1')
...
describe('<supports-named-feature-fn>', () => {
    test('invalid', () => {
        assert.invalid('<supports-named-feature-fn>', 'named-feature(unknown)')
    })
    test('representation', () => {
        assert.representation('<supports-named-feature-fn>', 'named-feature(ANCHOR-POSITION-FOLLOWS-TRANSFORMS)', {
            name: 'named-feature',
            types: ['<supports-named-feature-fn>'],
            value: identifier('anchor-position-follows-transforms'),
        })
    })
})
