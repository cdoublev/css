...
function getFirstAvailableFontFamily(element) {
    const { fontFaces } = states.get(element.ownerDocument.defaultView)
...
function getComputedLength(value, element, property) {
    switch (value.unit) {
        ...
        case 'dvh': {
            ...
            const { shared: { agent: { viewport: { interfaces: { bottom, top } } } } } = states.get(defaultView)
        ...
        case 'dvw': {
            const { ownerDocument: { defaultView } } = element
            const { shared: { agent: { viewport: { interfaces: { left, right } } } } } = states.get(defaultView)
