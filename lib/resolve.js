...
import { deprecated as deprecatedColors, named as namedColors, system as systemColors } from './values/color.js'
import { findAncestorElement, getParentElement } from './utils/dom/node.js'
...
import { canonicalize } from './values/dimension.js'
 ...
 * @returns {boolean}
 */
function isContainer(element, type) {
    if (type === 'style') {
        return true
    }
    const specified = getSpecifiedValue('container-type', element)
    if (specified.value === 'normal') {
        return false
    }
    if (type === 'inline-size') {
        return specified[0].value.endsWith('size')
    }
    return specified.some(value => value.value === type)
}
...
function getAbsoluteSelectors(selectors, context) {
    ...
    const normalized = []
    selectors.forEach(([combinator, complex]) => {
 ...
 * @param {object} start
 * @param {object} context
 * @param {CSSStyleSheetImpl} parentStyleSheet
 * @returns {Element}
 */
...
function registerLayer(name, { layer = '', layers }) {
    name.split('.').forEach(segment => {
        layer += `[${segment === '' ? layers.length : segment}]`
 ...
 * @param {CSSPseudoElementImpl|Element} element
...
function getRuleDeclarations(property, element, rule, context) {
    ...
    if (CSSContainerRule.isImpl(rule)) {
        ...
        if (findAncestorElement(element, element => matchContainer(rule._condition, element))) {
...
function getDeclaredValues(property, element) {
    ...
    const { ownerDocument, style } = element
    const { agent, user } = states.get(ownerDocument.defaultView)
    const root = element.getRootNode({ composed: true })
    const { adoptedStyleSheets = [], styleSheets } = root

    const declarations = [
        getStyleSheetDeclarations(property, element, agent.styleSheet, createContext(root, USER_AGENT_ORIGIN)),
        getStyleSheetDeclarations(property, element, user.styleSheet, createContext(root, USER_ORIGIN)),
...
function getFirstAvailableFontFamily(element) {
    const { document: { fontFaces } } = states.get(element.ownerDocument.defaultView)
...
function getComputedAbsoluteSize({ value }, element) {
    const userPreferredFontSize = getUserPreferredFontSize(element.ownerDocument.defaultView)
    switch (value) {
        case 'large':
    ...
    switch (name) {
        case 'random': {
            if (base === undefined) {
                ...
                base = getRandomBaseValue(element.ownerDocument.defaultView, { element, identifier, scope })
                value = { ...value, base }
            }
            break
        }
        case 'sibling-count':
            return number(getParentElement(element)?.children.length ?? 1)
        case 'sibling-index': {
            const parent = getParentElement(element)
...
function getComputedColor(color, element) {
    ...
    if (color.types.includes('<deprecated-color>')) {
        return parseGrammar(systemColors[deprecatedColors[color.value]][getElementColorScheme(element)], '<color>', element)
    }
    ...
    if (color.types.includes('<named-color>')) {
        return parseGrammar(namedColors[color.value], '<color>', element)
    }
    if (color.types.includes('<system-color>')) {
        return parseGrammar(systemColors[color.value][getElementColorScheme(element)], '<color>', element)
    }
    return color
}
...
function getComputedLength(value, element, property) {
    switch (value.unit) {
        ...
        case 'cqb': {
            const container = findAncestorElement(element, element => isContainer(element, 'size'))
            if (container) {
                const writingMode = getComputedValue('writing-mode', element)
                ...
                if (isNumeric(blockSize, { literal: true })) {
                    return length(value.value * blockSize.value / 100, 'px')
                }
                return length(0, 'px')
            }
        ...
        case 'cqh': {
            const writingMode = getComputedValue('writing-mode', element)
            const containerType = writingMode.value === 'horizontal-tb' ? 'size' : 'inline-size'
            const container = findAncestorElement(element, element => isContainer(element, containerType))
            if (container) {
                const height = getComputedValue('height', container)
                if (isNumeric(height, { literal: true })) {
                    return length(value.value * height.value / 100, 'px')
                }
                return length(0, 'px')
            }
        ...
        case 'cqi': {
            const container = findAncestorElement(element, element => isContainer(element, 'inline-size'))
            if (container) {
                const writingMode = getComputedValue('writing-mode', element)
                ...
                if (isNumeric(inlineSize, { literal: true })) {
                    return length(value.value * inlineSize.value / 100, 'px')
                }
                return length(0, 'px')
            }
        ...
        case 'cqw': {
            const writingMode = getComputedValue('writing-mode', element)
            const containerType = writingMode.value === 'horizontal-tb' ? 'inline-size' : 'size'
            const container = findAncestorElement(element, element => isContainer(element, containerType))
            if (container) {
                const width = getComputedValue('width', container)
                console.log(width)
                if (isNumeric(width, { literal: true })) {
                    return length(value.value * width.value / 100, 'px')
                }
                return length(0, 'px')
            }
            const width = element.ownerDocument.defaultView.innerWidth
            return length(value.value * width / 100, 'px')
        }
        case 'dvb': {
            const writingMode = getComputedValue('writing-mode', element)
            const unit = writingMode.value === 'horizontal-tb' ? 'dvh' : 'dvw'
            return getComputedLength({ ...value, unit }, element, property)
        }
        case 'dvh': {
            const { ownerDocument: { defaultView } } = element
            const { agent: { viewport: { interfaces: { bottom, top } } } } = states.get(defaultView)
        ...
        case 'dvw': {
            const { ownerDocument: { defaultView } } = element
            const { agent: { viewport: { interfaces: { left, right } } } } = states.get(defaultView)
        ...
        case 'lh': {
            if (property === 'font-size' || property === 'line-height') {
                ...
                if (lineHeight.value === 'normal') {
                    ...
                    const lineHeight = getNormalLineHeight(firstAvailableFontFamily, fontSize.value)
        ...
        case 'px':
            return value
        case 'rcap': {
            const { ownerDocument: { defaultView, documentElement } } = element
            const firstAvailableFontFamily = getFirstAvailableFontFamily(documentElement)
            const { ascent } = getFontFamilyMetrics(firstAvailableFontFamily)
            if (element === documentElement && property === 'font-size') {
                const fontSize = getUserPreferredFontSize(defaultView)
                return length(value.value * ascent * fontSize, 'px')
            }
            const fontSize = getComputedValue('font-size', documentElement)
            return length(value.value * ascent * fontSize.value, 'px')
        }
        case 'rch': {
            const { ownerDocument: { defaultView, documentElement } } = element
            const writingMode = getComputedValue('writing-mode', documentElement)
            const textOrientation = getComputedValue('text-orientation', documentElement)
            const measure = (writingMode.value.startsWith('vertical') && textOrientation.value === 'upright') ? 1 : 0.5
            if (element === documentElement && property === 'font-size') {
                const fontSize = getUserPreferredFontSize(defaultView)
...
function getComputedComponentValue(value, property, element) {
    ...
    for (let index = types.length - 1; 0 <= index; index--) {
        switch (types[index]) {
            ...
            case '<calc-invert>':
            case '<calc-negate>':
                return { ...value, value: getComputedComponentValue(value.value, property, element) }
            case '<color>':
...
function getUsedCalculationFunction({ range, round, type, value }, element, property) {
    if (!isNumeric(value, { literal: true })) {
        const resolutionType = type.percentHint ? `<${type.percentHint}>` : null
        value = simplifyCalculation(
            map(value, value => getUsedComponentValue(value, property, element)),
            resolutionType,
            element.ownerDocument.defaultView.devicePixelRatio)
    }
    let resolved = clamp(range.min, value.value, range.max)
...
function getUsedComponentValue(value, property, element) {
    ...
    for (let index = types.length - 1; 0 <= index; index--) {
        switch (types[index]) {
            ...
            case '<calc-invert>':
            case '<calc-negate>':
                return { ...value, value: getUsedComponentValue(value.value, property, element) }
        }
    }
    return value
}
