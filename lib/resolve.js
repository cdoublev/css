...
import { delimiter, identifierToken, list, map, omitted } from './values/value.js'
import { findAncestor, getParent } from './utils/dom/element.js'
import { isDelimiter, isList, isOmitted } from './utils/value.js'
...
function getComputedColor(value, element) {
    ...
    if (value.types.includes('<deprecated-color>')) {
        return parseGrammar(colors.system[colors.deprecated[value.value]][getElementColorScheme(element)], '<color>', element)
    }
    if (value.types.includes('<light-dark-color>')) {
        return getComputedColor(value.value[getElementColorScheme(element) === 'light' ? 0 : 2], element)
    }
    if (value.types.includes('<named-color>')) {
        return parseGrammar(colors.named[value.value], '<color>', element)
    }
    if (value.types.includes('<system-color>')) {
        return parseGrammar(colors.system[value.value][getElementColorScheme(element)], '<color>', element)
    }
    return value
}
