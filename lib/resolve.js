...
/**
 * @param {CSSScopeRuleImpl} rule
 * @param {object} context
 * @returns {Element[]}
 */
function findScopingLimits({ _end }, context) {
    if (isOmitted(_end)) {
        return []
    }
    return matchTreesAgainstSelectors([context.tree], getAbsoluteSelectors(_end[1].value, context), context)
}
 ...
 * @returns {Element}
 */
function findScopingRoots({ _start, parentStyleSheet }, context) {
    if (isOmitted(_start)) {
        return [parentStyleSheet.ownerNode]
    }
    return matchTreesAgainstSelectors([context.tree], getAbsoluteSelectors(_start.value, context), context)
}
...
function getRuleDeclarations(property, element, rule, context) {
    ...
    if (CSSContainerRule.isImpl(rule)) {
        ...
        registerLayer(rule.nameList, context)
        ...
        const roots = findScopingRoots(rule, context)
        const scopes = { inclusive: true, roots }
        context = { ...context, anchors: undefined, scopes, selectors: [] }
        const limits = findScopingLimits(rule, context)
...
function getDeclaredValues(property, element) {
    ...
    const { ownerDocument: { userAgentStyleSheet, userStyleSheet }, style } = element
    const tree = element.getRootNode({ compose: true })
    const { adoptedStyleSheets = [], styleSheets } = tree

    const declarations = [
        getStyleSheetDeclarations(property, element, userAgentStyleSheet, createContext(tree, USER_AGENT_ORIGIN)),
        getStyleSheetDeclarations(property, element, userStyleSheet, createContext(tree, USER_ORIGIN)),
...
function getElementSupportedColorSchemes(element) {
    const declarations = getDeclaredValues('color-scheme', element)
    return getCascadedValue(declarations, element)
}
...
function getUsedColorScheme(element) {
    ...
    if (supported) {
        ...
        return supported[0].find(({ value }) => value === 'dark' || value === 'light')
    }
    return 'light'
}
