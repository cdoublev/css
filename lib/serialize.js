 ...
 * @param {string} origin
 * @returns {*[]}
 * @see {@link https://drafts.csswg.org/css-animations-2/#propdef-timeline-trigger-range-start}
 ...
 */
function representRuleInset(declarations, shorthand) {
    let values = declarations.map(serializeValue)
    if (values[0] === 'auto') {
        return ''
    }
    if (shorthand.startsWith('rule')) {
        ...
        values = values.filter((_, index) => index % 2)
    }
    ...
    return values.filter(value => value !== 'auto').join(' ')
}
        ...
        for (const [index, entry] of value.entries()) {
            switch (name) {
                ...
                case 'timeline-trigger-source':
                    ...
                    break
                case 'timeline-trigger-exit-range-end':
                case 'timeline-trigger-range-end': {
...
function representDeclarationValue({ name, pending, source, value }) {
    ...
    if (shorthands.has(name)) {
        ...
        switch (name) {
            ...
            case 'rule-interior-inset':
                return representRuleInset(declarations, name)
            case 'columns':
    ...
    switch (name) {
        ...
        case 'timeline-trigger-exit-range-end':
        case 'timeline-trigger-exit-range-start':
        case 'timeline-trigger-range-end':
        case 'timeline-trigger-range-start':
