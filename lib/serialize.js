 ...
 */
function serializeColor({ name, types, value }, preserved) {
    ...
    if (types.includes('<color-interpolate()>')) {
        return `color-interpolate(${serializeComponentValues(value, name)})`
    }

    if (types.includes('<color-mix()>')) {
        let [interpolation,, items] = value
        let string = 'color-mix('
        ...
        if (declared.every(percentage => percentage === balanced)) {
            return `${string}${items.map(color => serializeColor(color[0], name)).join(', ')})`
        }
    ...
    if (types.includes('<light-dark-color>')) {
        return `light-dark(${serializeComponentValues(value, name)})`
    }
    ...
    // Assert: color is a <color-function>
