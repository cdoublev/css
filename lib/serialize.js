...
import { isBlock, isCalculationOperator, isComma, isList, isNumeric, isOmitted } from './utils/value.js'
...
function serializeBlock({ associatedToken, value }) {
    ...
    return `${associatedToken}${value}${associatedTokens[associatedToken]}`
}
...
function representFontSynthesis(declarations) {
    const values = declarations.map(serializeCSSValue)
    const keywords = shorthands.get('font-synthesis')[0].map(name => name.replace('font-synthesis-', ''))
