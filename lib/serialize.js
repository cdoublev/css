...
import { isCalculationOperator, isComma, isFunction, isList, isNumeric, isOmitted, isOpenCurlyBrace } from './utils/value.js'
import { isDigit, isIdentifierCharacter, isNonASCIIIdentifierCharacter } from './utils/string.js'
import { list, map, number, percentage, string } from './values/value.js'
 ...
 */
function serializeCalculationFunction({ range, round, value }) {
 ...
 */
function serializeColorRelative({ name, value }, context) {

    const isSRGB = sRGBColorFunctions.includes(name)
    const destination = relativeColorFunctions.includes(context)
    let resolved = true
    let source
    let alpha
    ...
    value = value.flat(2).reduce((channels, component, index, components) => {
        ...
        if (isHue && types.includes('<angle>')) {
            ...
            if (!isSRGB || value === undefined || !isNumeric(value, { literal: true })) {
                resolved = false
    ...
    if (!resolved || source || destination) {
...
function serializeComponentValue(value, context) {
    ...
    for (let index = types.length - 1; 0 <= index; --index) {
        switch (types[index]) {
            ...
            case '<rgb()>':
            case '<rgba()>':
                return serializeColorRelative(value, context)
            case '<alpha-value>':
