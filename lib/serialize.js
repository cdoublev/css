...
import { MAX_INTEGER, MIN_INTEGER } from './values/integer.js'
import { canonicalize, definitions as dimensions, getCanonicalUnitFromType } from './values/dimension.js'
...
import { colorFunctions } from './values/color.js'
 ...
 * @returns {string}
 * @see {@link https://drafts.csswg.org/css-syntax-3/#serialize-an-a-n-plus-b-value}
 */
...
function serializeBasicShapeRect({ name, value }) {
    ...
    value = name === 'inset'
        ...
        : serializeComponentValues(value.slice(0, -1))
 ...
 */
function serializeCalcMix({ value }) {
    const [[, weight]] = value
    if (value.every(([, w]) => w.value === weight.value)) {
        value = value.map(([v]) => serializeComponentValue(v)).join(', ')
    } else {
        value = serializeComponentValue(value)
    }
    return `calc-mix(${value})`
}
    ...
    switch (types[0]) {
        case '<calc-product>': {
            ...
            const product = tail.reduce(
                (product, child) => {
                    if (child.types.includes('<calc-invert>')) {
        ...
        case '<function>':
            ...
            if (name === 'calc-mix') {
                return serializeCalcMix(calculation)
            }
            if (name === 'round') {
        ...
        default:
            throw RangeError('Unexpected calculation type')
    }
}
 ...
 */
function serializeCalculationFunction({ value }) {
    if (isNumeric(value) || isCalculationOperator(value)) {
 ...
 * @param {object} color
 * @returns {string}
 * @see {@link https://drafts.csswg.org/css-color-hdr-1/#funcdef-hdr-color}
 */
function serializeColorHDR({ name, value }) {
    value = value.map(([color, headroom]) => {
        color = serializeComponentValue(color, name)
        headroom = serializeComponentValue(headroom)
        if (!headroom || headroom === '0') {
            return color
        }
        return `${color} ${headroom}`
    })
 ...
 */
function serializeColorHex({ value }) {
    ...
    if (alpha && alpha < 1) {
        return `rgba(${value.join(', ')}, ${serializeAlpha(number(alpha), true)})`
    }
 ...
 * @returns {string}
 * @see {@link https://drafts.csswg.org/css-color-5/#color-interpolation-method}
 */
function serializeColorInterpolationMethod([, method]) {
    let string = 'in'
    if (isList(method)) {
        const [colorSpace, hueInterpolationMethod] = method.map(serializeComponentValue)
        string += ` ${colorSpace}`
        if (hueInterpolationMethod && hueInterpolationMethod !== 'shorter hue') {
            string += ` ${hueInterpolationMethod}`
        }
    } else {
        string += ` ${serializeComponentValue(method)}`
    }
    return string
}
 ...
 * @param {color} object
 * @returns {string}
 * @see {@link https://drafts.csswg.org/css-color-5/#serializing-device-cmyk-values}
 * @see {@link https://drafts.csswg.org/css-color-5/#typedef-legacy-device-cmyk-syntax}
 */
function serializeColorLegacyDeviceCMYK({ value }) {
    return `device-cmyk(${serializeComponentValues(list(value))})`
}

/**
 * @param {object} color
 * @returns {string}
 * @see {@link https://drafts.csswg.org/css-color-5/#funcdef-color-mix}
 * @see {@link https://drafts.csswg.org/css-color-5/#serial-color-mix}
 */
function serializeColorMix({ name, value: [interpolation,, items] }) {

    let string = `${name}(`

    interpolation = serializeComponentValue(interpolation)
    ...
    for (const [, percentage] of items) {
        if (isFunction(percentage)) {
            return `${string}${serializeComponentValues(items, name)})`
        }
        if (isOmitted(percentage)) {
            omitted++
        } else {
            declared.push(percentage.value)
        }
    }

    const balanced = 100 / items.length
    if (declared.every(percentage => percentage === balanced || (omitted === 0 && percentage === declared[0] && balanced < percentage))) {
        return `${string}${items.map(color => serializeComponentValue(color[0], name)).join(', ')})`
    }

    const sum = declared.reduce((sum, percentage) => sum + percentage, 0)
    const normalized = percentage(Math.max(0, (100 - sum)) / omitted)
    items = items.map(([color, percentage]) => {
        color = serializeComponentValue(color, name)
        percentage = serializeComponentValue(isOmitted(percentage) ? normalized : percentage)
 ...
 * @returns {string}
 * @see {@link https://drafts.csswg.org/css-color-4/#serializing-color-values}
 * @see {@link https://drafts.csswg.org/css-color-5/#serial-origin-color}
 * @see {@link https://drafts.csswg.org/css-color-5/#serial-relative-color}
 * @see {@link https://drafts.csswg.org/css-color-hdr-1/#typedef-color-function}
 */
function serializeColorModel({ name, value }, context) {

    const {
        [context]: parentColorFunction,
    ...
    value = value.flat(2).reduce((channels, component, index, components) => {
        ...
        if (types.includes('<color>')) {
            source = `from ${serializeComponentValue(component, name)}`
            return channels
        }
        ...
        if (types.includes('<ident>')) {
            ...
            if (value === 'xyz') {
                value = 'xyz-d65'
            } else if (colorSpace === 'srgb' && value === 'none') {
                value = '0'
            }
            if (isAlpha) {
                alpha = value
        ...
        if (isHue && types.includes('<angle>')) {
            ...
            // Extract a resolved calculation function value only in sRGB color functions
            if (colorSpace !== 'srgb' || value === undefined || !isNumeric(value, { literal: true })) {
                resolved = false
    ...
    if (!resolved || source || parentColorFunction) {
...
function serializeConicGradientSyntax([configuration,, stops]) {
    ...
    if (!isOmitted(line)) {
        ...
        if (position && position !== 'at center center') {
            options.push(position)
        }
    }
    if (!isOmitted(interpolation) && interpolation[1].value !== 'oklab') {
        options.push(serializeComponentValues(interpolation))
    }
    if (0 === options.length) {
        return stops
    }
...
function serializeCounter({ name, value }) {
    ...
    return `${name}(${serializeComponentValues(value)})`
}
...
function serializeDegenerate({ types, unit, value }) {
    ...
    throw RangeError('Unexpected dimension type')
}
 ...
 * @see {@link https://drafts.csswg.org/motion-1/#typedef-offset-path}
 */
function serializeEllipse({ name, types, value }) {
    ...
    if (position && (position !== 'at center center' || types.includes('<offset-path>'))) {
        options.push(position)
    }
 ...
 * @param {object} filter
 * @returns {string}
 * @see {@link https://drafts.csswg.org/filter-effects-1/#funcdef-filter-blur}
 * @see {@link https://drafts.csswg.org/filter-effects-1/#funcdef-filter-brightness}
 * @see {@link https://drafts.csswg.org/filter-effects-1/#funcdef-filter-contrast}
 * @see {@link https://drafts.csswg.org/filter-effects-1/#funcdef-filter-drop-shadow}
 * @see {@link https://drafts.csswg.org/filter-effects-1/#funcdef-filter-grayscale}
 * @see {@link https://drafts.csswg.org/filter-effects-1/#funcdef-filter-hue-rotate}
 * @see {@link https://drafts.csswg.org/filter-effects-1/#funcdef-filter-invert}
 * @see {@link https://drafts.csswg.org/filter-effects-1/#funcdef-filter-opacity}
 * @see {@link https://drafts.csswg.org/filter-effects-1/#funcdef-filter-saturate}
 * @see {@link https://drafts.csswg.org/filter-effects-1/#funcdef-filter-sepia}
 */
 ...
 * @param {string} context
 * @returns {string}
 * @see {@link https://drafts.csswg.org/css-values-5/#free-form-productions}
 */
function serializeFreeFormValue(value, context) {
    const string = serializeComponentValues(value, context)
    switch (context) {
        case undefined:
            return string
        case 'var':
        ...
        default:
            return (value.some(value => isComma(value) || isOpenCurlyBrace(value)))
                ? `{${string}}`
                : string
    }
}
 ...
 * @param {object} param0
 * @returns {string}
 * @see {@link https://drafts.csswg.org/mediaqueries-5/#typedef-general-enclosed}
 */
function serializeGeneralEnclosed({ name, types, value }) {
    value = `(${serializeComponentValues(value)})`
    if (types[0] === '<function>') {
        value = `${serializeIdentifier({ value: name })}${value}`
    }
    return value
}
...
function serializeIdentifier({ value }) {
    ...
    return [...value].reduce(
        (identifier, character, index) => {
            ...
            } else if (index === 1 && isDigit(character) && identifier[0] === '-') {
                return `${identifier}\\${codePoint.toString(16)} `
            }
            if (
                isNonASCIIIdentifierCharacter(character)
...
function serializeInterpolate({ name, value: [[progress, ...options],, position,, output,, stops] }) {
    ...
    if (0 < tail.length) {
        const options = tail.join(' ')
        if (options) {
            parameters.push(serializeComponentValues(tail))
        }
    }

    const [from, to] = position.map(serializeComponentValue)
    output = serializeComponentValue(output, name)
...
function serializeKeyframeSelector(selector) {
    switch (selector.types[0]) {
        ...
        default:
            return serializeComponentValues(selector)
    }
}
...
function serializeMediaQuery(query) {
    ...
    return serializeComponentValues(query)
}
...
function serializePosition(position) {
    if (isList(position)) {
        return serializeComponentValues(position)
    }
    let side
    if (position.types.includes('<ident>')) {
        side = position.value
 ...
 * @param {boolean} [query]
 * @returns {string}
 * @see {@link https://drafts.csswg.org/css-anchor-position-1/#typedef-position-area}
 * @see {@link https://drafts.csswg.org/css-anchor-position-2/#typedef-position-area-query}
 */
function serializePositionArea(area, query) {
    let [a, b] = area.map(serializeComponentValue)
    if (!a) {
        if (/bottom|inline|top|y/.test(b)) {
            a = query ? 'any' : 'span-all'
        } else {
            a = b
        }
    } else if (!b) {
        if (/block|left|right|x/.test(a)) {
            b = query ? 'any' : 'span-all'
        } else {
            b = a
        }
    }
    return `${a} ${b}`.replace(/(block|inline)-/g, '')
}
...
function serializeRadialGradientSyntax([configuration,, stops]) {
    ...
    let [line, interpolation] = configuration
    const options = []
    if (!isOmitted(line)) {
        let [aspect, position] = line
        if (!isOmitted(aspect)) {
        ...
        position = serializeComponentValue(position)
        if (position && position !== 'at center center') {
            options.push(position)
        }
    }
 ...
 * @param {object} ray
 * @returns {string}
 * @see {@link https://drafts.csswg.org/motion-1/#funcdef-ray}
 */
...
function serializeShape({ value: [fillRule, ...tail] }) {
    tail = serializeComponentValues(tail)
...
function serializeSides(sides) {
    return simplifySides(sides.map(serializeComponentValue))
}
    ...
    return [...value].reduce(
        (string, character) => {
            ...
            if (codePoint === 0) {
                ...
                character = `\\${codePoint.toString(16)} `
...
function serializeComponentValue(value, context) {
    ...
    for (let index = types.length - 1; 0 <= index; index--) {
        switch (types[index]) {
            ...
            case '<lab()>':
            case '<lch()>':
            case '<modern-device-cmyk-syntax>':
            case '<oklab()>':
            case '<oklch()>':
            case '<rgb()>':
            case '<rgba()>':
                return serializeColorModel(value, context)
            case '<alpha-value>':
                return serializeAlpha(value)
            case '<any-value>':
            case '<declaration-value>':
            case '<whole-value>':
                return serializeFreeFormValue(value, context)
            case '<a-n-plus-b>':
            ...
            case '<color-interpolation-method>':
                return serializeColorInterpolationMethod(value)
            case '<color-mix()>':
                return serializeColorMix(value)
            case '<color-stripe>':
            ...
            case '<hex-color>':
                return serializeColorHex(value)
            case '<hdr-color()>':
                return serializeColorHDR(value)
            case '<inset()>':
            case '<rect()>':
            case '<xywh()>':
            ...
            case '<legacy-device-cmyk-syntax>':
                return serializeColorLegacyDeviceCMYK(value)
            case '<line-names>':
            ...
            case '<position-area>':
                return serializePositionArea(value)
            case '<position-area-query>':
                return serializePositionArea(value, true)
    ...
    if (isList(value)) {
        return serializeComponentValues(value, context)
    }
 ...
 */
function serializeComponentValues(list, context) {
...
function representAnimation(declarations) {
    ...
    for (const { value } of declarations) {
        ...
        value.forEach((value, index) => animations[index]?.push(value) ?? animations.push(list([value])))
    }
 ...
 * @see {@link https://drafts.csswg.org/css-animations-1/#propdef-animation}
 */
function representAnimationDelay([{ value: starts }, { value: ends }]) {
    if (starts.length !== ends.length) {
        return ''
    }
    return starts
        .map((start, index) => {
            start = serializeComponentValue(start)
            const end = serializeComponentValue(ends[index])
            if (end === properties['animation-delay-end'].initial.serialized) {
                return start
            }
            return `${start} ${end}`
        })
 ...
 * @returns {string}
 * @see {@link https://drafts.csswg.org/animation-triggers-1/#propdef-timeline-trigger-activation-range}
 * @see {@link https://drafts.csswg.org/animation-triggers-1/#propdef-timeline-trigger-active-range}
...
function representAnimationRange([{ value: starts }, { value: ends }], initial) {
    ...
    return starts
        .map((start, index) => representAnimationRangeValue(start, ends[index], initial))
        .join(', ')
}
 ...
 * @param {string} origin
 * @returns {*[]}
 * @see {@link https://drafts.csswg.org/animation-triggers-1/#propdef-timeline-trigger-activation-range-start}
 * @see {@link https://drafts.csswg.org/animation-triggers-1/#propdef-timeline-trigger-active-range-start}
...
function representBackground(declarations) {
    const { value: { length } } = declarations.find(declaration => declaration.name === 'background-image')
    const clip = declarations.find(declaration => declaration.name === 'background-clip')
    const layers = []
    for (const { name, value } of declarations) {
        ...
        value.forEach((value, index) => {
            value = serializeValue({ name, value: list([value]) })
    ...
    return layers
        .map((layer, layerIndex) => {
            let valueIndex = layer.length
            while (0 < valueIndex-- && 1 < layer.length) {
                const { [valueIndex]: { name } } = declarations
                const value = layer[valueIndex]
                const prev = layer[valueIndex - 1]
                if (name === 'background-repeat-y') {
                    if (value === prev) {
                        layer.splice(valueIndex, 1)
                    } else if (value === 'no-repeat' && prev === 'repeat') {
                        layer.splice(--valueIndex, 2, 'repeat-x')
                    } else if (value === 'repeat' && prev === 'no-repeat') {
                        layer.splice(--valueIndex, 2, 'repeat-y')
                    }
                } else if (value === properties[name].initial.serialized) {
                    layer.splice(valueIndex, 1)
                } else if (name === 'background-size') {
                    layer.splice(valueIndex--, 1, `/ ${value}`)
                } else if (
                    name === 'background-clip'
                    && value !== prev
                    && clip.value[layerIndex].types.includes('<visual-box>')
                    && prev === properties['background-origin'].initial.serialized
                ) {
                    valueIndex--
                }
            }
...
function representBackgroundRepeat(declarations) {
    const { value: { length } } = declarations.find(({ name }) => name === 'background-repeat-x')
    const repetitions = []
    for (const { value } of declarations) {
        if (length !== value.length) {
            return ''
        ...
        value.forEach((value, index) => {
            value = serializeComponentValue(value)
            repetitions[index]?.push(value) ?? repetitions.push([value])
        })
    }
    // Simplification
    return repetitions
 ...
 * @param {*[]} size
 * @returns {*[]}
 * @see {@link https://drafts.csswg.org/css-backgrounds-4/#propdef-background-size}
 * @see {@link https://drafts.csswg.org/css-masking-1/#propdef-mask-size}
 */
function representBackgroundSize(size) {
    return map(size, value => {
        if (isList(value)) {
            const [width, height] = value
            if (width.value !== 'auto' && !height) {
                return list([width, auto])
            }
 ...
 * @param {number} sides
 * @returns {string}
 * @see {@link https://drafts.csswg.org/css-borders-4/#border-shorthands}
 ...
 * @see {@link https://drafts.csswg.org/css-gaps-1/#propdef-column-rule-inset-start}
 * @see {@link https://drafts.csswg.org/css-gaps-1/#propdef-row-rule-inset-start}
 * @see {@link https://drafts.csswg.org/css-gaps-1/#propdef-rule-break}
 * @see {@link https://drafts.csswg.org/css-gaps-1/#propdef-rule-color}
 * @see {@link https://drafts.csswg.org/css-gaps-1/#propdef-rule-inset-start}
...
function representBorderImage(declarations) {
    ...
    while (0 < index-- && 1 < values.length) {
        ...
        } else if (name.endsWith('outset') || name.endsWith('width')) {
 ...
 * @param {object[]} slice
 * @returns {string}
 * @see {@link https://drafts.csswg.org/css-borders-4/#propdef-border-image-slice}
 * @see {@link https://drafts.csswg.org/css-masking-1/#propdef-mask-border-slice}
 */
function representBorderSlice([offsets, fill]) {
    offsets = serializeSides(offsets)
    return isOmitted(fill) ? offsets : `${offsets} fill`
}
...
function representBoxShadow(declarations) {
    ...
    for (const { value } of declarations) {
        ...
        value.forEach((value, i) => shadows[i]?.push(value) ?? shadows.push(list([value])))
    }
 ...
 * @param {*[]} offsets
 * @returns {string}
 * @see {@link https://drafts.csswg.org/css-borders-4/#propdef-box-shadow-offset}
 */
function representBoxShadowOffset(offsets) {
    return offsets
        .map(offset => {
            const string = serializeComponentValue(offset)
            if (isList(offset) && offset.length === 1) {
...
/**
 * @param {object|object[]} color
 * @returns {string}
 * @see {@link https://drafts.csswg.org/css-ui-4/#propdef-caret-color}
 */
function representCaretColor(color) {
    if (isList(color)) {
        const [background, text] = color.map(serializeComponentValue)
        if (text && text !== 'auto') {
 ...
 * @returns {object|string}
 * @see {@link https://drafts.csswg.org/css-masking-1/#propdef-clip-path}
 */
 ...
 */
function representCorner(declarations, longhands) {
    const half = longhands.length / 2
    const radii = representBorderRadius(declarations.slice(0, half))
    const shape = simplifySides(declarations.slice(half).map(serializeValue))
    if (shape === properties['corner-top-left-shape'].initial.serialized) {
        return radii
    }
    if (radii === properties['border-top-left-radius'].initial.serialized) {
        return shape
    }
    return `${radii} ${shape}`
}
...
function representCounter(counters, property) {
    if (isList(counters)) {
        return counters
            .map(counter => {
                const [name, value] = counter.map(serializeComponentValue)
...
function representDisplay(type) {
    if (isList(type)) {
        const values = type
            .map(serializeComponentValue)
            .filter((value, index, values) => value && value !== 'flow' && (value !== 'block' || values[index + 1] === 'ruby'))
        if (values[0] === 'inline') {
            switch (values[1]) {
                case 'flex':
                case 'grid':
                case 'table':
                    return values.join('-')
                case 'flow-root':
                    return values[2] ? values.join(' ') : 'inline-block'
                case 'ruby':
                    return 'ruby'
            }
        }
 ...
 * @param {object[]} declarations
 * @returns {string}
 * @see {@link https://drafts.csswg.org/animation-triggers-1/#propdef-event-trigger}
 */
function representEventTrigger(declarations) {
    const { value: { length } } = declarations.find(({ name }) => name === 'event-trigger-name')
    let triggers = []
    for (const { value } of declarations) {
        if (length !== value.length) {
            return ''
        ...
        value.forEach((value, index) => {
            value = serializeComponentValue(value)
            triggers[index]?.push(value) ?? triggers.push([value])
        })
    }
    triggers = triggers.map(trigger => trigger.join(' ')).join(', ')
    if (triggers === 'none none') {
        return 'none'
    }
    return triggers
}
 ...
 * @param {object[]} declarations
 * @returns {string}
 * @see {@link https://drafts.csswg.org/css-flexbox-2/#propdef-flex}
 */
...
function representFontRange(range) {
    return isList(range) ? serializeSides(range) : range
}
...
function representFontSynthesis(declarations) {
    const values = declarations.map(serializeValue)
    const keywords = []
    for (const [index, name] of shorthands.get('font-synthesis')[0].entries()) {
        const value = values[index]
        switch (value) {
            case 'oblique-only':
                return ''
    ...
    return 0 < keywords.length ? keywords.join(' ') : 'none'
}
...
function representFontVariant(values) {
    ...
    return values.filter(value => value !== '' && value !== 'normal').join(' ')
}
...
function representGrid(declarations) {
    ...
    if (autoFlow === 'row' && autoRows === 'auto' && autoColumns === 'auto') {
        ...
        return serializeComponentValue(representGridTemplate([rows, columns, areas]))
    }
    if (
        areas !== 'none'
    ...
    if (autoFlow.startsWith('row')) {
        ...
        return `${flow} ${autoRows} / ${columns}`
    }
    if (autoColumns === 'auto') {
        return `${rows} / ${flow}`
    }
    return `${rows} / ${flow} ${autoColumns}`
}
...
function representGridAutoFlow(flow) {
    flow = serializeComponentValues(flow)
 ...
 * @param {object[]} declarations
 * @returns {string}
 * @see {@link https://drafts.csswg.org/css-masking-1/#propdef-mask}
 */
function representMask(declarations) {
    const { value: { length } } = declarations.find(declaration => declaration.name === 'mask-image')
    const clip = declarations.find(declaration => declaration.name === 'mask-clip')
    const layers = []
    for (const { name, value } of declarations) {
        if (length !== value.length) {
            return ''
        ...
        .map((layer, layerIndex) => {
            ...
            while (0 < valueIndex-- && 1 < layer.length) {
                ...
                if (value === properties[name].initial.serialized) {
                    ...
                    if (value === prev) {
                        layer.splice(valueIndex, 1)
                    } else if (clip.value[layerIndex].types.includes('<geometry-box>')) {
                        valueIndex--
                    }
                }
            }
 ...
 * @param {object[]} declarations
 * @returns {string}
 * @see {@link https://drafts.csswg.org/motion-1/#propdef-offset}
 */
 ...
 * @param {object|object[]} path
 * @returns {object|string}
 * @see {@link https://drafts.csswg.org/motion-1/#propdef-offset-path}
 */
 ...
 * @returns {string}
 * @see {@link https://drafts.csswg.org/motion-1/#propdef-offset-rotate}
 */
function representOffsetRotate(rotation) {
    const value = serializeComponentValues(rotation)
    switch (value) {
        case 'auto 0deg':
        case 'reverse 180deg':
        case 'reverse -180deg':
...
function representPointerTimeline(declarations, longhands) {
    ...
    return timelines
        .map(timeline => representWithoutInitialValues(timeline, longhands, 'pointer-timeline-name'))
        .join(', ')
}
 ...
 * @returns {string}
 * @see {@link https://drafts.csswg.org/css-gaps-1/#propdef-column-rule}
 * @see {@link https://drafts.csswg.org/css-gaps-1/#propdef-row-rule}
 * @see {@link https://drafts.csswg.org/css-gaps-1/#propdef-rule}
 */
function representRule(declarations, longhands, shorthand) {

    if (shorthand === 'rule') {
        const [rowWidth, columnWidth, rowStyle, columnStyle, rowColor, columnColor] = declarations
        const rowRule = representRule([rowWidth, rowStyle, rowColor], shorthands.get('row-rule')[0], 'row-rule')
        if (rowRule === '') {
            return ''
        }
        const columnRule = representRule([columnWidth, columnStyle, columnColor], shorthands.get('column-rule')[0], 'column-rule')
        if (columnRule === '' || columnRule !== rowRule) {
            return ''
        }
        return rowRule
    }

    const [{ value: widths }, { value: styles }, { value: colors }] = declarations
    const rules = [...widths]
    const { length } = rules

    if (length !== styles.length || length !== colors.length || styles.length !== colors.length) {
        return ''
    }
    ...
    for (let i = 0; i < length; i++) {
        ...
        if (isFunction(width)) {
            ...
            if (!isFunction(style) || !isFunction(color)) {
                return ''
            }

            const n = serializeComponentValue(width.value[0])
            const o = serializeComponentValue(style.value[0])
            const p = serializeComponentValue(color.value[0])
            ...
            if (width.length !== style.length || width.length !== color.length || style.length !== color.length) {
                return ''
            }
            ...
            for (let j = 0; j < n; j++) {
                const value = [
                    serializeComponentValue(width[j]),
                    serializeComponentValue(style[j]),
                    serializeComponentValue(color[j]),
                ]
                values.push(representWithoutInitialValues(value, longhands))
            }
            rules[i] = `repeat(${n}, ${values.join(', ')})`
            continue
        }
        if (isFunction(style) || isFunction(color)) {
            return ''
        ...
        const value = [
            serializeComponentValue(width),
            serializeComponentValue(style),
            serializeComponentValue(color),
        ]
        rules[i] = representWithoutInitialValues(value, longhands)
    }
    return rules.join(', ')
}
 ...
 * @param {string} shorthand
 * @returns {string}
 * @see {@link https://drafts.csswg.org/css-gaps-1/#propdef-column-rule-inset-cap}
 * @see {@link https://drafts.csswg.org/css-gaps-1/#propdef-column-rule-inset-junction}
 * @see {@link https://drafts.csswg.org/css-gaps-1/#propdef-column-rule-inset}
 * @see {@link https://drafts.csswg.org/css-gaps-1/#propdef-row-rule-inset-cap}
 * @see {@link https://drafts.csswg.org/css-gaps-1/#propdef-row-rule-inset-junction}
 * @see {@link https://drafts.csswg.org/css-gaps-1/#propdef-row-rule-inset}
 * @see {@link https://drafts.csswg.org/css-gaps-1/#propdef-rule-inset-cap}
 * @see {@link https://drafts.csswg.org/css-gaps-1/#propdef-rule-inset-junction}
 * @see {@link https://drafts.csswg.org/css-gaps-1/#propdef-rule-inset}
 */
function representRuleInset(declarations, longhands, shorthand) {
    const values = declarations.map(serializeValue)
    if (shorthand.startsWith('rule')) {
        // [[row declarations, row longhands], [column declarations, column longhands]]
        const args = [[[], []], [[], []]]
        declarations.forEach((declaration, index) => {
            args[index % 2][0].push(declaration)
            args[index % 2][1].push(longhands[index])
        })
        const rows = representRuleInset(...args[0], `rows-${shorthand}`)
        const columns = representRuleInset(...args[1], `columns-${shorthand}`)
        if (rows === columns) {
            return rows
        }
        return ''
    }
    if (shorthand.endsWith('inset')) {
        const [capStart, capEnd, junctionStart, junctionEnd] = values
        if (capStart === junctionStart && capEnd === junctionEnd) {
            if (capStart === capEnd) {
                return capStart
            }
        ...
        if (capStart === capEnd) {
            if (junctionStart === junctionEnd) {
                return `${capStart} / ${junctionStart}`
            }
            return `${capStart} / ${junctionStart} ${junctionEnd}`
        }
        if (junctionStart === junctionEnd) {
            return `${capStart} ${capEnd} / ${junctionStart}`
        }
...
function representScrollSnapType(type) {
    if (isList(type)) {
        const [axis, strictness] = type.map(serializeComponentValue)
        if (strictness === '' || strictness === 'proximity') {
            return axis
        }
...
function representScrollTimeline(declarations, longhands) {
    ...
    return timelines
        .map(timeline => representWithoutInitialValues(timeline, longhands, 'scroll-timeline-name'))
        .join(', ')
}
...
function representSize(size) {
    if (isList(size) && size[0].types.includes('<length>')) {
        return serializeSides(size)
    }
    return size
}
...
/**
 * @param {object|object[]} lines
 * @returns {string}
 * @see {@link https://drafts.csswg.org/css-text-decor-4/#propdef-text-decoration-skip-self}
 */
function representTextDecorationSkipSelf(lines) {
    lines = serializeComponentValue(lines)
    return lines === 'skip-underline skip-overline skip-line-through' ? 'skip-all' : lines
}
 ...
 * @param {object[]} declarations
 * @returns {string}
 * @see {@link https://drafts.csswg.org/animation-triggers-1/#propdef-timeline-trigger}
 */
function representTimelineTrigger(declarations) {
    const { value: { length } } = declarations.find(({ name }) => name === 'timeline-trigger-name')
    let triggers = []
    for (const { name, value } of declarations) {
        if (length !== value.length) {
            return ''
    ...
    triggers = triggers.map(trigger => {
        ...
        let string = `${name} ${source} ${range.replace(' normal', '')}`
        if (exitRange !== 'auto') {
            string += ` / ${exitRange}`
        }
        return string
    })
    if (triggers.length === 1 && triggers[0] === 'none auto normal') {
        return 'none'
    }
...
function representTransition(declarations, longhands) {
    ...
    return transitions.map(transition => representWithoutInitialValues(transition, longhands)).join(', ')
}
 ...
 */
function representTranslate(value) {
    if (isList(value)) {
        const [x, y, z] = value.flat().map(serializeComponentValue)
        if (z === '' || z === '0px') {
            if (y === '' || y === '0px') {
                return x
            }
...
function representVerticalAlign(declarations, longhands) {
    ...
    if (simplified === properties['baseline-source'].initial.serialized) {
...
function representViewTimeline(declarations, longhands) {
    ...
    return timelines
        .map(timeline => representWithoutInitialValues(timeline, longhands, 'view-timeline-name'))
        .join(', ')
}
...
function representViewTimelineInset(inset) {
    return inset.map(serializeSides).join(', ')
}
 ...
 */
function representVoice(baseline) {
...
function representDeclarationValue({ name, pending, source, value }) {
    ...
    if (shorthands.has(name)) {
        ...
        for (const declaration of value) {
            ...
            if (pending) {
                // All longhands can only be represented if they are all declared with the same pending substitution
            ...
            if (cssWideKeywords.includes(longhandValue.value)) {
                ...
                // All longhands can only be represented if they are all declared with the same CSS-wide keyword
            ...
            if (resetOnly?.includes(longhand)) {
                ...
                // All longhands can only be represented if all reset-only longhands are declared to their initial value
                return ''
            }
        ...
        // Shorthand declared with a value matching its value definition
        switch (name) {
            ...
            case 'animation-delay':
                return representAnimationDelay(declarations)
            case 'animation-range':
            case 'timeline-trigger-activation-range':
            case 'timeline-trigger-active-range':
                return representAnimationRange(declarations, name.includes('active') ? 'auto' : 'normal')
            case 'background':
            ...
            case 'scroll-padding-inline':
                return simplifySides(declarations.map(serializeValue))
            ...
            case 'row-rule':
            case 'rule':
                return representRule(declarations, longhands, name)
            ...
            case 'rule-inset-junction':
                return representRuleInset(declarations, longhands, name)
            case 'columns':
            ...
            case 'corner-right':
            case 'corner-top':
                return representCorner(declarations, longhands)
            case 'event-trigger':
                return representEventTrigger(declarations)
            case 'flex':
            ...
            case 'timeline-trigger':
                return representTimelineTrigger(declarations)
            case 'transition':
    ...
    // Longhand declared with a pending substitution
    if (pending) {
        // ... of its own or for its legacy shorthand
        // https://github.com/w3c/csswg-drafts/issues/3109
    ...
    // Longhand declared with a CSS-wide keyword
    if (cssWideKeywords.includes(value.value)) {
        return value
    }
    // Longhand declared with an internal system font value
    if (value.types.includes('<system-font()>')) {
        return ''
    }
    // Longhand declared with a value matching its value definition
    switch (name) {
        ...
        case 'box-shadow-offset':
            return representBoxShadowOffset(value)
        case 'break-after':
        case 'break-before':
        case 'break-inside':
            return representBreak(value, name)
        case 'caret-color':
            return representCaretColor(value)
        case 'clip-path':
        ...
        case 'text-autospace':
            return representTextAutospace(value)
        case 'text-decoration-skip-self':
            return representTextDecorationSkipSelf(value)
...
function serializeValue(declaration) {
    return serializeComponentValue(representDeclarationValue(declaration))
}
