    ...
    value = value.flat(2).reduce((channels, component, index, components) => {
        ...
        if (isHue && types.includes('<angle>')) {
            ...
            if (!isSRGB || !value || !isNumeric(value, true)) {
                resolved = false
                value = types.includes('<math-function>')
                    ? serializeMathFunction(component)
                    : serializeCalculation(component)
                if (isAlpha) {
                    alpha = value
                } else {
                    channels.push(value)
 ...
 */
function serializeEllipse({ name, value }) {
...
function serializeKeyframeSelector(selector) {
    switch (selector.types[0]) {
        ...
        case '<function>':
            return selector.types.includes('<math-function>')
                ? serializeMathFunction(component)
                : serializeCalculation(component)
 ...
 */
function serializeMathFunction({ range = {}, round, value }, forComputedOrLater = false) {
    if (isNumeric(value)) {
...
function serializePosition(position) {
    ...
    if (position.types[0] === '<ident-token>') {
        ...
        side = position.types.includes('<math-function>')
            ? serializeMathFunction(position)
            : serializeCalculation(position)
    } else {
