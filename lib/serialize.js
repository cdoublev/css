...
function serializeColor({ name, types, value }, mixed = false) {
    ...
    if (types.includes('<color-mix()>')) {
        if (value[0][0].types?.includes('<progress>')) {
            let [head,, color1,, color2] = value
            head = serializeCSSComponentValue(head)
            color1 = serializeColor(color1, true)
            color2 = serializeColor(color2, true)
            return `color-mix(${head}, ${color1}, ${color2})`
        }
 ...
 */
function serializeCalculationTree(root, nested = true) {
    ...
    if (isNumeric(root)) {
        ...
        if (types.includes('<number>')) {
            return serializeNumber(root)
        }
        return serializeDimension(root)
    }
    if (types.includes('<function>')) {
        if (mathFunctions.includes(root.name)) {
            return serializeMathFunction({ value: root })
        }
        return serializeFunction(root)
    }
    if (types.includes('<calc-negate>')) {
        return `(-1 * ${serializeCalculationTree(value)})`
    }
    if (types.includes('<calc-invert>')) {
        return `(1 / ${serializeCalculationTree(value)})`
    }
    const [first, ...children] = sortCalculation(value)
    let string = `${serializeCalculationTree(first)}`
    if (types.includes('<calc-sum>')) {
        children.forEach(child => {
            const { types, value } = child
            if (types.includes('<calc-negate>')) {
                string += ` - ${serializeCalculationTree(value)}`
            } else if (typeof value === 'number' && value < 0) {
                string += ` - ${serializeCSSComponentValue({ ...child, value: 0 - value })}`
            } else {
                string += ` + ${serializeCalculationTree(child)}`
            }
        })
        ...
        root = root.reduce(
            (root, child, index) => {
                ...
                if (isCalculation(child)) {
                    child = serializeCalculationTree(child, false)
            ...
            [])
        string += root.join(separator)
    } else {
        root = serializeCalculationTree(root, false)
        string += root
    }
    string += ')'
    return string
...
function serializeMediaFeatureDeclaration([name,, value]) {
    return `${name.value}: ${serializeCSSComponentValue(value)}`
}
    ...
    if (!isOmitted(line)) {
        ...
        if (!isOmitted(aspect)) {
            ...
            if (!isOmitted(size) && size.value !== 'farthest-corner') {
                options.push(serializeRadialSize(size))
            }
...
function serializeStepEasingFunction({ value }) {
    switch (value) {
        ...
        default: {
            ...
            const steps = serializeInteger(count)
...
function serializeCSSComponentValue(component) {
    ...
    for (let index = types.length - 1; 0 <= index; --index) {
        switch (types[index]) {
            ...
            case '<calc-sum>':
            case '<calc-value>':
                return serializeCalculationTree(component, false)
            case '<circle()>':
            case '<ellipse()>':
