...
const { hasPendingSubstitution, isCalculationOperator, isComma, isInfinityOrNaN, isNumeric, isOmitted } = require('./utils/value.js')
...
function serializeCommaContainingValue(value, context) {
    ...
    if (context && value.some(component => isComma(component) || associatedToken === '{')) {
        return `{${string}}`
    }
    return string
}
