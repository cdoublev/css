 ...
 *
 * The color function arguments must be clamped "at computed value time" but
...
function serializeColor(color) {
    ...
    if (type.has('hex-color')) {
        ...
        // Color must be serialized to <rgb()> or <rgba()>
        rgb = hslToRgb(...hsl)
        alpha = a
    } else if (name === 'rgb' || name === 'rgba') {
        const [r, g, b, a] = value.flat().filter(({ value }) => value !== ',' && value !== '/')
        // RGB must be serialized to unsigned 8 bit integers clamped to 0-255
