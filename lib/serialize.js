...
function serializeCounter({ name, value }) {
    ...
    return `${name}(${serializeComponentValues(list)})`
}
...
function serializeFunction({ closed = true, name, value }) {
    return `${name}(${serializeComponentValue(value)}${closed ? ')' : ''}`
}
...
function serializeMediaQuery(query) {
     ...
     * <general-enclosed> = [ <function> <any-value> ) ] | ( <ident> <any-value> )
     *
    ...
    type = serializeIdentifier(type.toLowerCase())
    ...
    return `${s}${features
        .map(({ name, value }) => {
            s += `(${name.toLowerCase()}`
            if (value) {
                s += `: ${serializeComponentValue(value)})`
            }
            return s
        })
        .join(' and ')}`
...
function serializeMediaQueryList(list) {
    return serializeComponentValues(list)
}
...
function serializeCalculationTree(root, forComputedOrLater) {
    ...
    if (isNumeric(root) || (type.has('function') && !type.has('math-function'))) {
        ...
        return serializeComponentValue(root)
    }
    if (type.has('math-function')) {
    ...
    if (type.has('calc-sum')) {
        children.forEach(child => {
            if (child.type.has('calc-negate')) {
                ...
                s += ` - ${serializeComponentValue({ ...child, value: child.value * -1 })}`
            } else {
                s += ` + ${serializeCalculationTree(child, forComputedOrLater)}`
            }
        })
...
function serializeMathFunction({ value: root }, forComputedOrLater = false) {
    ...
    if (isResolved) {
        if (forComputedOrLater) {
            ...
            return serializeComponentValue({ type, value: clamp(min, value, max) })
        }
        if (isInfinityOrNaN(root)) {
...
function serializePosition(position) {
    if (Array.isArray(position)) {
        return position.map(side => {
            ...
            return serializeComponentValue(side)
        }).join(' ')
    }
    const { type } = position
    let side
 ...
 */
function serializeSelector([compound, combined = []]) {
    /**
     * Test cases to create (parse):
     ...
     *   - between the ':'s, or between the ':' and <ident> or <function>, of a <pseudo-element-selector> or a <pseudo-class-selector>
     ...
     * - ::before, ::after, ::first-line, and ::first-letter may be represented using the <pseudo-class-selector> grammar, with only a single ":" character at their start
     * - in <id-selector>, <hash> must be an <ident> (not an <hex-color>)
     * Test cases to create (serialize):
     * - serialize <combinator> without whitespaces between `|` and `|`
     * - serialize <type-selector> without whitespaces around `|`
     * - serialize <compound-selector> without whitespace around `.`
     * - serialize <attribute-selector> without whitespaces after `[` and before `]`
     * - serialize <attr-matcher> without whitespaces between `~`,`|`, `^`, `$`, or `*`, and `=`
     * - serialize <pseudo-element-selector> without whitespaces between `:` and <pseudo-class-selector>
     * - serialize <pseudo-class-selector> without whitespaces between `:` and <ident> or <function>
     * - serialize <pseudo-class-selector> without whitespaces between `<any-value>` and `)`
     */
    let s = ''
    return s
}
 ...
 * @param {object[]} group
 * @returns {string}
 * @see {@link https://drafts.csswg.org/cssom/#serialize-a-group-of-selectors}
 */
function serializeSelectorGroup(group) {
    return serializeComponentValues(group)
}
...
function serializeSides(sides) {
    return serializeComponentValues(simplifySides(sides))
}
 ...
 *
 * It serializes without whitespaces:
 * - type and namespace as `<type-selector>`
 * - type and class as `<compound-selector>`
 * - type and attribute as `<compound-selector>`
 * - type and pseudo-element or pseudo-class as `<compound-selector>
 * - pseudo-element and pseudo-class as `<compound-selector>`
 * - column combinator as `<combinator>`
 * - attribute name, value, matcher, as `<attribute-selector>`
 */
function serializeWithoutWhitespace(component) {
    if (Array.isArray(component)) {
        return component.map(serializeComponentValue).join('')
    }
    return serializeComponentValue(component.value)
}
 ...
 */
function serializeComponentValue(component) {
    // Delimiter or custom property value
    ...
    if (Array.isArray(component)) {
        return serializeComponentValues(component)
    }
 ...
 * @return {string}
 * @see {@link https://drafts.csswg.org/cssom/#serializing-css-values}
 */
function serializeComponentValues(list, separator = list.separator ?? ' ') {
    if (separator !== ' ') {
        separator += ' '
    }
    return list.reduce(
        (value, component) => {
            component = serializeComponentValue(component)
            if (component === '') {
                return value
            }
...
function simplifyBackground(longhandValues) {
    ...
    return longhandValues.reduce((layers, longhandValue, longhandIndex) => {
        ...
        if (longhand === 'background-color') {
            ...
            if (value !== initial) {
                layers.at(-1).push(serializeComponentValue(longhandValue))
            }
            return layers
        }
...
function serializeValue(list) {
    if (Array.isArray(list)) {
        ...
        if (shorthand) {
            ...
            return serializeValue({ input: inputList, name, value: valueList })
        }
        return ''
    }
    const components = representDeclarationValue(list)
    return serializeComponentValues(components)
}
 ...
 */
function serializeDeclaration(property, value, priority) {
...
function serializeDeclarationBlock(declarations) {
    ...
    declarationLoop: for (const declaration of declarations.values()) {
        ...
        shorthandLoop: for (const [shorthand, longhands] of shorthandsForProperty) {
            ...
            const serializedValue = serializeValue(currentLonghandDeclarations)

            if (serializedValue === '') {
                continue shorthandLoop
            }

            const serializedDeclaration = serializeDeclaration(shorthand, serializedValue, important)
