 ...
 * @param {object[][]} radii
 * @returns {string}
 * @see {@link https://drafts.csswg.org/css-backgrounds/#propdef-border-radius}
 */
 ...
 * @param {object} color
 * @returns {string}
 * @see {@link https://www.w3.org/TR/css-color-4/#serializing-color-values}
 *
 ...
 */
function serializeMediaQuery({ features, negated, type }) {
    let s = ''
    if (negated) {
        s += 'not '
    }
    ...
    return `${s}${features
        .map(({ name, value }) => {
            s += `(${name.toLowerCase()}`
            if (value) {
                // TODO: figure out the point of using the procedure to serialize a media feature value
                s+= `: ${value.toLowerCase()})`
            }
            return s
        })
...
function serializeMediaQueryList(list) {
    return list.map(serializeMediaQuery).join(', ')
}
