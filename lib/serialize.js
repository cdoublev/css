 ...
 */
function serializeCalculationTree(root, forComputedOrLater, nested = true) {
...
function serializeMathFunction({ value: root }, forComputedOrLater = false) {
    ...
    if (Array.isArray(root)) {
        ...
        root = root.reduce(
            (root, child, index) => {
                ...
                if (isCalculation(child)) {
                    child = serializeCalculationTree(child, forComputedOrLater, false)
        ...
        root = serializeCalculationTree(root, forComputedOrLater, false)
        string += root
    }
    string += ')'
    return string
...
function serializeCSSComponentValue(component) {
    ...
    for (let index = types.length - 1; 0 <= index; --index) {
        switch (types[index]) {
            ...
            case '<calc-product>':
            case '<calc-sum>':
                return serializeCalculationTree(component, false, false)
            case '<circle()>':
            case '<ellipse()>':
    ...
    layers.forEach(layer => {
        ...
        while (0 < index-- && 1 < layer.length) {
            ...
            if (name === 'background-position') {
                ...
                if (value === layer[index - 1] || value === 'no-repeat') {
                    layer.splice(index, 1)
                }
