...
const { hasPendingSubstitution, isCalculationOperator, isComma, isInfinityOrNaN, isNumeric, isOmitted, isSimpleBlock } = require('./utils/value.js')
 ...
 * @param {object[]} edges
 * @returns {string}
 * @see {@link https://drafts.csswg.org/css-inline-3/#typedef-text-edge}
 */
function serializeTextEdge(edges) {
    const [over, under] = edges.map(serializeCSSComponentValue)
    if (under === '' || under === over || under === 'text') {
        return over
    }
    return `${over} ${under}`
}
...
function serializeCSSComponentValue(component, context) {
    ...
    for (let index = types.length - 1; 0 <= index; --index) {
        switch (types[index]) {
            ...
            case '<text-edge>':
                return serializeTextEdge(component)
            case '<translate()>':
...
function representDeclarationValue({ name, source, value }) {
    ...
    if (shorthands.has(name)) {
        ...
        for (const declaration of value) {
            ...
            if (hasPendingSubstitution(longhandValue)) {
                ...
                return (value.length === 1 || value.every(declaration => declaration.value.pending === name))
    ...
    if (value.pending) {
        // https://github.com/w3c/csswg-drafts/issues/3109
        return [shorthands.get(value.pending)?.length === 1 ? value : '']
    }
