...
function representAnimationRange([{ value: starts }, { value: ends }], initial) {
    ...
    const boundaries = starts.map((start, index) => representAnimationRangeValue(start, ends[index], initial))
    ...
    layers.forEach((layer, i) => {
        ...
        while (0 < index-- && 1 < layer.length) {
            ...
            if (name === 'background-repeat-y') {
                ...
                && value !== prev
                && clip.value[i].types.includes('<visual-box>')
...
function representCorner(declarations, longhands) {
    ...
    const radii = representBorderRadius(declarations.slice(0, half))[0]
    const shape = representSides(declarations.slice(half).map(serializeValue)).join(' ')
...
function representEventTrigger(declarations) {
    ...
    for (const { value } of declarations) {
        ...
        value.forEach((value, index) => {
            value = serializeComponentValue(value)
            triggers[index]?.push(value) ?? triggers.push(list([value]))
        })
    }
    if (triggers.length === 1 && triggers[0][0] === 'none' && triggers[0][1] === 'none') {
        return ['none']
    }
    ...
    layers.forEach((layer, i) => {
        ...
        while (0 < index-- && 1 < layer.length) {
            ...
            if (value === properties[name].initial.serialized) {
                ...
                if (value === prev) {
                    layer.splice(index, 1)
                } else if (clip.value[i].types.includes('<geometry-box>')) {
                    --index
                }
            }
        }
...
function representRule(declarations, longhands, shorthand) {
    ...
    if (shorthand === 'rule') {
        ...
        const rowRule = representRule([rowWidth, rowStyle, rowColor], shorthands.get('row-rule')[0], 'row-rule')[0]
        if (rowRule === '') {
            return ['']
        }
        const columnRule = representRule([columnWidth, columnStyle, columnColor], shorthands.get('column-rule')[0], 'column-rule')[0]
    ...
    for (let i = 0; i < length; ++i) {
        ...
        if (isFunction(width)) {
            ...
            for (let j = 0; j < n; j++) {
                ...
                values.push(representWithoutInitialValues(list(value), longhands))
            }
            rules[i].value[2] = list(values, ',')
            continue
        }
        if (isFunction(style) || isFunction(color)) {
            return ['']
        ...
        rules[i] = representWithoutInitialValues(list(value), longhands)
    }
    return [serializeComponentValueList(list(rules, ','))]
}
...
function representTimelineTrigger(declarations) {
    ...
    triggers = triggers.map(trigger => {
        const [,, range, exitRange] = trigger
        if (exitRange === 'auto') {
            trigger.pop()
        } else {
            trigger.splice(-1, 0, '/')
        }
        if (range.endsWith(' normal')) {
            trigger[2] = range.replace(' normal', '')
        }
        return trigger
    })
    if (triggers.length === 1 && triggers[0].join(' ') === 'none auto normal') {
        return ['none']
    }
    return list(triggers, ',')
}
