...
const { isCalculationOperator, isComma, isInfinityOrNaN, isNumeric, isOmitted, isSimpleBlock } = require('./utils/value.js')
...
function serializeCommaContainingValue(value, context) {
    ...
    if (context && value.some(component => isComma(component) || isSimpleBlock('{', component))) {
        return `{${string}}`
    }
    return string
}
...
function serializeCalculation(calculation, parent) {
    ...
    switch (types[0]) {
        case '<calc-product>': {
            const [head, ...tail] = sortCalculation(value)
        ...
        case '<percentage-token>':
            return isInfinityOrNaN(calculation) ? serializeInfinityOrNaN(calculation) : serializeDimension(calculation)
        case '<number-token>':
            return isInfinityOrNaN(calculation) ? serializeInfinityOrNaN(calculation) : serializeNumber(calculation)
        case '<ident-token>':
            return value
        case '<function>':
            if (name === 'random') {
...
function serializeColor({ name, types, value }, specified) {
    ...
    if (types.includes('<color-mix()>')) {
        if (value[0][0].types?.includes('<progress>')) {
            let [progress,, color1,, color2] = value
            progress = serializeCSSComponentValue(progress)
            color1 = serializeColor(color1, true)
            color2 = serializeColor(color2, true)
            return `color-mix(${progress}, ${color1}, ${color2})`
        }
    ...
    if (types.includes('<contrast-color()>')) {
        return `contrast-color(${serializeColor(value, true)})`
    }
    if (types.includes('<light-dark()>')) {
        const [light,, dark] = value
        return `light-dark(${serializeColor(light, true)}, ${serializeColor(dark, true)})`
    }
    let alpha
 ...
 * @returns {string}
 */
function serializeDashedFunctionHead(head) {
    const strings = head.map(serializeCSSComponentValue)
 ...
 *
 * It serializes the declaration with no whitespace between the name and column
 * (browser conformance).
 */
function serializeMediaFeatureDeclaration([name,, value]) {
    return `${serializeCSSComponentValue(name)}: ${serializeCSSComponentValue(value)}`
}
...
function serializeRadialSize(size) {
    return Array.isArray(size) ? serializeSides(size) : size.value
}
 ...
 */
function serializeCSSComponentValue(component, context) {
    ...
    for (let index = types.length - 1; 0 <= index; --index) {
        switch (types[index]) {
            ...
            case '<whole-value>':
                return serializeCommaContainingValue(component, context)
            case '<an+b>':
            ...
            case '<calc-function>':
                return serializeCalculationFunction(component)
            case '<calc-product>':
            case '<calc-sum>':
            case '<calc-value>':
                return serializeCalculation(component)
            case '<circle()>':
            case '<ellipse()>':
                return serializeEllipse(component)
            case '<color>':
            ...
            case '<control-value()>':
                return serializeControlValue(component)
            case '<counter>':
                return serializeCounter(component)
            case '<ident>':
            case '<ident-token>':
                return serializeIdentifier(component)
            case '<dashed-function-head>':
                return serializeDashedFunctionHead(component)
            case '<declaration>':
            ...
            case '<mf-name>':
                return serializeMediaFeatureName(component)
            case '<mf-plain>':
                return serializeMediaFeatureDeclaration(component)
            case '<number>':
            case '<number-token>':
            ...
            case '<radial-size>':
                return serializeRadialSize(component)
            case '<random()>':
                return serializeRandom(component)
            case '<ratio>':
                return serializeRatio(component)
            case '<ray()>':
                return serializeRay(component)
            case '<relative-control-point>':
                return serializeRelativeControlPoint(component)
            case '<repeat-style>':
                return serializeRepeatStyle(component)
            case '<round()>':
                return serializeRound(component)
            case '<scale()>':
            ...
            case '<step-easing-function>':
                return serializeStepEasingFunction(component)
            case '<string>':
            case '<string()>':
            case '<string-token>':
    ...
    if (Array.isArray(component)) {
        return serializeCSSComponentValueList(component, context)
    }
 ...
 * removed from all values except an arbitrary value, which is represented with
 * a list whose separator is an empty string.
 */
function serializeCSSComponentValueList(list, context) {
    const separator = list.separator ?? ' '
    return list.reduce(
        (value, component, index) => {
            component = serializeCSSComponentValue(component, context)
            if (0 < index) {
                switch (separator) {
                    case ' ':
                        if (value && component && component[0] !== ',' && component[0] !== ';') {
                            value += separator
                        }
                        break
                    case '':
                    ...
                    case ',':
                    case ';':
                        value += component ? `${separator} ` : separator
                        break
                    default:
                        value += ` ${separator} `
                        break
...
function representAnimationRangeLonghand(range, origin) {
    const percentage = origin.endsWith('start') ? '0%' : '100%'
...
function representAnimationRangeValue(start, end, initial) {
    ...
    if (Array.isArray(end)) {
        ...
        if (offset && offset !== '100%') {
            endRange += ` ${offset}`
        } else if (Array.isArray(start) && start[0].value === endRange) {
            return startRange
        }
    } else {
        endRange = serializeCSSComponentValue(end)
        if (!Array.isArray(start) && endRange === initial) {
            return startRange
        }
    }
...
function representAnimationTrigger(declarations) {
    const { value: { length } } = declarations.find(({ name }) => name === 'animation-trigger-type')
    let triggers = []
