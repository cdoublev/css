...
const { numeric: { math: mathFunctions }, property: { keywords: cssWideKeywords } } = require('./values/substitutions.js')
 ...
 */
function serializeColor({ name, types, value }) {
    ...
    if (types.includes('<color-mix()>')) {
        ...
        c1 = serializeCSSComponentValue(c1)
        c2 = serializeCSSComponentValue(c2)
        if (isOmitted(p2)) {
            if (isOmitted(p1)) {
    ...
    value = value.flat(2).reduce((channels, component, index, components) => {
        ...
        if (types.includes('<color>')) {
            source = `from ${serializeColor(component)} `
            return channels
        }
        ...
        if (types.includes('<percentage>')) {
            switch (name) {
                ...
                case 'device-cmyk':
                    ...
                    break
                case 'lab':
                    value = 1 < index ? value * 1.25 : value
                    break
                case 'lch':
                    value = 1 < index ? value * 1.5 : value
                    break
                case 'oklab':
                case 'oklch':
                    value = 1 < index ? value * 0.004 : value * 0.01
                    break
                case 'rgb':
                case 'rgba':
        ...
        switch (name) {
            ...
            case 'hsla':
                ...
                break
            case 'hwb':
                // Clamp negative whiteness and blackness
                value = Math.max(0, value)
                break
            case 'lab':
                // Clamp lightness
                value = index === 1 ? clamp(0, value, 100) : value
                break
            case 'lch':
                // Clamp lightness and negative chromaticity
                value = index === 1 ? clamp(0, value, 100) : Math.max(0, value)
                break
            case 'oklab':
                // Clamp lightness
                value = index === 1 ? clamp(0, value, 1) : value
                break
            case 'oklch':
                // Clamp lightness and negative chromaticity
                value = index === 1 ? clamp(0, value, 1) : Math.max(0, value)
                break
            case 'rgb':
            case 'rgba':
                // Clamp all color channels
