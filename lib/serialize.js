...
const { areEqualNumeric, hasSubstitution, isCalcOperatorNode, isInfinityOrNaN, isNumeric } = require('./values/validation.js')
const { clamp, safeFloat, toEightBit } = require('./utils/math.js')
const { hslToRgb, hwbToRgb, sRGB } = require('./utils/colorSpace.js')
...
function serializeAlpha(alpha, is8Bit = false) {
    ...
    if (is8Bit) {
        ...
        if (Math.round(safeFloat(integer * 2.55)) === a) {
            alpha.value = integer / 100
        } else {
            alpha.value = Math.round(a / 0.255) / 1000
        }
    }
 ...
 *
 * It matches the current browser output for serializing a color as a component
 * of a specified value, instead of the current specification (see issue).
 *
 * It serializes:
 ...
 *   - with <alpha-value> as 8-bit unsigned integer
 ...
 * - <color()> with out of range color channel values gamut-mapped
 * - <lab()>
...
function serializeColor(color) {
    ...
    if (type.has('hex-color')) {
        ...
        value = value.flat(2).reduce((channels, component, index, input) => {
            let { omitted, type, value } = component
            if (omitted || value === ',' || value === '/') {
                return channels
            }
            // `none` or color space
            if (type.has('ident')) {
 ...
 *
 * It serializes a number to the shortest form possible, by rounding it to not
 * produce more than 6 decimals, and by removing trailing 0.
 *
 * User agents have some different (unspecified) behaviors:
...
function serializeCSSComponentValue(component) {
    ...
    while (types[0]) {
        ...
        switch (types.pop()) {
            ...
            case 'math-function': // TODO: the `type` of a math function must have `function`
...
function representDeclarationValue({ name, value }) {
    if (shorthands.has(name)) {
        const substitution = value.find(hasSubstitution)
        if (substitution) {
            // All longhands are declared with the same shorthand substitution value
            if (value.every(value => value === substitution)) {
                return substitution
            }
        ...
        if (cssWideKeyword) {
            const { value: keyword } = cssWideKeyword
            // All longhands are declared with the same CSS wide keyword
            if (value.every(({ value }) => value === keyword)) {
                return [keyword]
            }
            return ['']
        }
        switch (name) {
            ...
            case 'border-radius':
                // `border-radius` is serialized as a <border-radius>
                return [value.reduce(
                    (radii, [h, v]) => {
                        radii[0].push(h)
                        radii[1].push(v)
...
function serializeCSSValue(list) {
    if (Array.isArray(list)) {
        ...
        if (shorthand) {
            ...
            const value = createList(list.map(({ value }) => value))
            return serializeCSSValue({ name, value })
        }
        return ''
    }
...
function serializeCSSDeclarationBlock(declarations) {
    ...
    declarationLoop: for (const declaration of declarations.values()) {
        ...
        shorthandLoop: for (const [shorthand, longhands] of shorthandsForProperty) {
            ...
            // All longhands that map to shorthand must be present in longhand declarations
            ...
            // None or all longhand declarations for this shorthand must have the important flag
