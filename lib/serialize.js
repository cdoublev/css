...
function serializeDefinition(definition) {
    ...
    if (isMultiplied(definition)) {
        ...
        if (isCombination(value) || isCompound(value) || multipliedType === 'optional' || string.endsWith('}')) {
            string = `[${string}]`
        }
        if (type === 'optional') {
            string += '?'
