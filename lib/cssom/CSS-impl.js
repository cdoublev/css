 ...
 */
function supportsCondition(conditionText, globalObject) {
...
export default class CSSImpl {
     ...
     * @returns {number}
     */
    static getRandomBaseValue({ element, identifier, scope }) {
        const name = this.randomCacheNames.find(name =>
        ...
        this.randomCacheNames.push({ base, element, identifier, scope })
        return base
    }
    ...
    static registerProperty(globalObject, { name, inherits, initialValue, syntax }) {
        ...
        const register = globalObject.document._registeredProperties
    ...
    static supports(globalObject, propertyOrConditionText, value) {
        return value === undefined
            ? supportsCondition(propertyOrConditionText, globalObject)
