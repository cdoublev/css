...
const { isComputationallyIndependent, parseSyntax } = require('../parse/syntax.js')
...
const { parseCSSDeclaration, parseCSSGrammar } = parser
 ...
 */
function supportsValue(property, value, important = false) {
    return parseCSSDeclaration({ important, name: property.toLowerCase(), value }, createContext()) !== null
}
...
class CSSImpl {
    ...
    registerProperty({ name, inherits, initialValue, syntax }) {
        ...
        syntax = parseSyntax(syntax, parser)
        if (syntax === null) {
