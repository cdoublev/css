...
function supportsValue(property, value) {
    ...
    return definition && parseCSSGrammar(value, definition.value)
}
...
function supportsCondition(conditionText) {
    return parseCSSGrammar(conditionText, '<supports-condition>')
        || parseCSSGrammar(`(${conditionText})`, '<supports-condition>')
