...
function supportsValue(property, value) {
    const declaration = { name: property.toLowerCase(), value }
    const context = createContext('@style')
    return parseCSSDeclaration(declaration, context) !== null
}
...
function supportsCondition(conditionText) {
    const parsed = parseCSSGrammar(`(${conditionText})`, '<supports-condition>', createContext('@style'))
...
class CSSImpl {
    ...
    registerProperty({ name, inherits, initialValue, syntax }) {
        ...
        if (!parseCSSGrammar(name, '<custom-property-name>', context)) {
        ...
        syntax = parseCSSGrammar(syntax, '<syntax>', context)
        if (syntax === null) {
        ...
        if (syntax === '*') {
            ...
            initialValue = parseCSSGrammar(initialValue, '<declaration-value>?', context)
        ...
        if (initialValue) {
            initialValue = parseCSSGrammar(initialValue, syntax, context)
