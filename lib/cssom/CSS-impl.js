...
import { isComputationallyIndependent, isFailure, isList } from '../values/value.js'
...
function supportsValue(globalObject, property, value) {
    const context = createVirtualContext(globalObject, '@style')
    return !isFailure(parseDeclarationValue(value, property, context))
}
...
function supportsCondition(globalObject, conditionText) {
    const condition = parseGrammar(`(${conditionText})`, '<supports-condition>', globalObject)
...
export default class CSSImpl {
    ...
    static registerProperty(globalObject, { name, inherits, initialValue, syntax }) {
        ...
        const { document: { customProperties } } = states.get(globalObject)
        ...
        if (initialValue === undefined) {
            customProperties.set(name, { inherits, syntax })
            return
        }
        if (syntax === '*') {
            ...
            if (!isFailure(initialValue) && isValidUniversalInitialValue(initialValue)) {
                customProperties.set(name, { inherits, initialValue, syntax })
                return
            }
