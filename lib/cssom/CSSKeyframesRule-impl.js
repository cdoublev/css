...
export default class CSSKeyframesRuleImpl extends CSSRuleImpl {
    ...
    deleteRule(selector) {
        ...
        if (!isFailure(selector)) {
            selector = serializeComponentValues(selector)
            const index = this.cssRules._rules.findLastIndex(rule => rule.keyText === selector)
            if (-1 < index) {
    ...
    findRule(selector) {
        ...
        if (!isFailure(selector)) {
            selector = serializeComponentValues(selector)
            return this.cssRules._rules.findLast(rule => rule.keyText === selector) ?? null
        }
        return null
    }
}
