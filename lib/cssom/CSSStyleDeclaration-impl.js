...
const { parseCSSDeclaration, parseDeclarationBlock } = require('../parse/parser.js')
...
class CSSStyleDeclarationImpl {
    ...
    constructor(globalObject, args, privateData) {
        ...
        if (ownerNode && !computed) {
            ...
            if (value) {
                this._declarations = parseDeclarationBlock(value, parentRule ?? ownerNode)
            }
        }
    }
    ...
    set cssText(value) {
        ...
        this._declarations = parseDeclarationBlock(value, this.parentRule ?? this.ownerNode)
        this._update()
    }
    ...
    setProperty(name, value, priority = '') {
        ...
        const context = this.parentRule ?? this.ownerNode
        ...
        if (shorthands.has(declaration.name)) {
            ...
            declarations.forEach((value, name) =>
                updated = setCSSDeclaration({ important, name, value }, this._declarations) && updated)
        } else {
    ...
    _change(name, value, namespace) {
        ...
        this._declarations = value ? parseDeclarationBlock(value, this.parentRule ?? this.ownerNode) : []
    }
