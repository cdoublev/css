...
import { getDeclarationName, parseDeclarations, parseDestructuredDeclaration } from '../parse/parser.js'
...
export default class CSSStyleDeclarationImpl {
    ...
    getPropertyValue(name) {
        ...
        if (this._computed) {
            let value
            if (shorthands.has(name)) {
                value = shorthands.get(name).flat().map(name => ({ name, value: getResolvedValue(name, this._ownerNode) }))
            } else {
                value = getResolvedValue(name, this._ownerNode)
            }
