...
const { NoModificationAllowedError } = require('../errors.js')
...
class CSSStyleDeclarationImpl {
     ...
     *
     * `associatedProperties` should be:
     ...
     * - for `Element.style`:
     *   - `computed`: false
     *   - `declarations`: (computed from `Element` style attribute, see below)
     *   - `parentRule`: null
     *   - `ownerNode`: context object
     * - for `getComputedStyle(element)`:
     *   - `computed`: true
     *   - `declarations`: (see related procedure)
     *   - `parentRule`: null
     *   - `ownerNode`: element
     */
    constructor(globalObject, args, associatedProperties) {
        ...
        if (ownerNode && !computed) {
            ...
            if (value) {
                this.#declarations = parseDeclarationBlock(value)
            }
        } else {
