...
class CSSStyleDeclarationImpl {
    ...
    constructor(globalObject, args, privateData) {
        ...
        if (ownerNode && !computed) {
            ...
            if (value) {
                const context = createContext(this.parentRule ?? '@style')
                this._declarations = parseCSSDeclarationBlock(value, context)
            }
        }
    }
    ...
    _change(name, value, namespace) {
        ...
        const context = createContext(this.parentRule ?? '@style')
        this._declarations = value ? parseCSSDeclarationBlock(value, context) : []
    }
