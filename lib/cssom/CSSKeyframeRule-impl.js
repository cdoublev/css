...
const { createParser, getDeclarationsInSpecifiedOrder, parseCSSGrammar } = require('../parse/syntax.js')
...
class CSSKeyframeImpl extends CSSRuleImpl {
    ...
    constructor(globalObject, args, privateData) {
        ...
        this._keyText = serializeCSSComponentValueList(keyText)
     ...
     */
    set keyText(select) {
        select = parseCSSGrammar(select, '<keyframe-selector>#', createParser(select))
        if (select) {
            this._keyText = serializeCSSComponentValueList(select)
        } else {
