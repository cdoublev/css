...
import { isFunction, isOmitted } from '../values/value.js'
...
async function processResponse({ body, headers, url }, rule) {
    ...
    return CSSStyleSheet.createImpl(_globalObject, undefined, privateData)
}
 ...
 * @returns {boolean}
 */
export function isMatchingImportConditions(rule) {
    return rule._supports && matchMediaQueryList(rule.media._list, rule._globalObject)
}
...
function fetchImport(rule, signal) {
    ...
    if (!isMatchingImportConditions(rule)) {
...
export default class CSSImportRuleImpl extends CSSRuleImpl {
    ...
    constructor(globalObject, args, privateData) {
        ...
        const { prelude: [url, options, media] } = privateData
        this._url = url
        if (isFunction(url)) {
        ...
        if (isOmitted(options)) {
            ...
            const [layer, scope, supports] = options
            if (isFunction(layer)) {
            ...
            if (isOmitted(supports)) {
                ...
                this._supports = matchSupport(supports.value, globalObject)
            }
        }
     ...
     */
    get cssText() {
        const { _scope, href, layerName, media: { mediaText }, supportsText } = this
        ...
        if (_scope) {
            string += ` ${serializeComponentValue(_scope)}`
        }
        if (supportsText) {
    ...
    // TODO: fetch in a listener of a change event emitted by MediaQueryList representing this.media
    ...
    _fetch() {
        ...
        this._promise = fetchImport(this, this._controller.signal)
            ...
            .catch(error => {
                if (environment.test && error.name !== 'AbortError') {
