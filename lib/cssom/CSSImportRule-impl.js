...
import { serializeComponentValue, serializeComponentValueList, serializeURL } from '../serialize.js'
...
export default class CSSImportRuleImpl extends CSSRuleImpl {
    ...
    get cssText() {
        ...
        if (media) {
            ...
            return `${string} ${serializeComponentValueList(supports)} ${mediaText};`
        }
        // Fallback to the declared `<media-query-list>` if any
        if (isOmitted(_conditions)) {
            return `${string};`
        }
        return `${string} ${serializeComponentValueList(_conditions)};`
    }
     ...
     */
    get media() {
        return this.styleSheet?.media ?? null
    }
}
