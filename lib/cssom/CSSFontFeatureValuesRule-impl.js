...
import { serializeComponentValues, serializeValue } from '../serialize.js'
...
export default class CSSFontFeatureValuesRuleImpl extends CSSRuleImpl {
    ...
    constructor(globalObject, args, privateData) {
        ...
        this.fontFamily = serializeComponentValues(prelude)

        block.value.forEach(content => Array.isArray(content) ? declarations.push(...content) : rules.push(content))
        declarations.forEach(declaration => this[toIDLAttribute(declaration.name)] = serializeValue(declaration))
