...
export default class CSSMediaRuleImpl extends CSSConditionRuleImpl {
     ...
     */
    get matches() {
        const { _globalObject, media, parentStyleSheet } = this
        const { document: { adoptedStyleSheets, styleSheets } } = _globalObject
        const parent = wrapperForImpl(parentStyleSheet)
        for (const styleSheet of styleSheets) {
            if (styleSheet === parent) {
                return matchMediaQueryList(media._list, _globalObject)
            }
        }
