...
class CSSViewTransitionRuleImpl extends CSSRuleImpl {
    ...
    constructor(globalObject, args, privateData) {
        ...
        const navigation = this._declarations.find(declaration => declaration.name === 'navigation')
        if (navigation) {
            this.navigation = serializeCSSValue(navigation)
        }
        const types = this._declarations.find(declaration => declaration.name === 'types')
