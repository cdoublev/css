...
const descriptorEntries = Object.entries(descriptors['@property'])
...
export default class CSSPropertyRuleImpl extends CSSRuleImpl {
    ...
    constructor(globalObject, args, privateData) {
        ...
        descriptorEntries.forEach(([name, definition]) => {
            const declaration = declarations.find(declaration => declaration.name === name)
            const value = declaration ? serializeValue(declaration) : definition.initial.serialized
            name = toIDLAttribute(name)
            switch (name) {
                case 'inherits':
