...
/**
 * @param {MediaListImpl|string|null} list
...
function createMediaList(list, globalObject) {
    if (list === null) {
        list = ''
    } else if (MediaList.isImpl(list)) {
        list = list.mediaText
    }
    list = parseGrammar(list, '<media-query-list>', globalObject)
...
export default class CSSStyleSheetImpl extends StyleSheetImpl {
    ...
    /**
     * @see {@link https://drafts.csswg.org/cssom/#last-css-style-sheet-set-name}
     */
    _lastName = null

    /**
     * @see {@link https://drafts.csswg.org/cssom/#preferred-css-style-sheet-set-name}
     */
    _preferredName = null
     ...
     * @see {@link https://html.spec.whatwg.org/multipage/infrastructure.html#create-a-css-style-sheet}
     */
    constructor(globalObject, [options], privateData) {
        super()
    ...
    async replace(text) {
        ...
        this._rules.forEach((_, index) => removeRule(this.cssRules, index))
