...
const { insertCSSRule, parseCSSRuleList, parseRule, parseRuleList, removeCSSRule } = require('../parse/syntax.js')
...
const INSERT_INVALID_IMPORT_ERROR = {
    message: 'Can not insert @import in a constructed stylesheet',
    name: 'SyntaxError',
    type: 'DOMException',
}
const UPDATE_LOCKED_STYLESHEET_ERROR = {
    message: "Can not insert or replace rules (pending replacement or non-constructed stylesheet)",
...
class CSSStyleSheetImpl extends StyleSheetImpl {
    ...
    replace(text) {
        ...
        return Promise.resolve().then(() => {
            const rules = parseRuleList(text).filter(({ name }) => name !== 'import')
            this._rules.splice(0, this._rules.length, ...parseCSSRuleList(rules, this, true))
