...
class CSSCounterStyleRuleImpl extends CSSRuleImpl {
    ...
    constructor(globalObject, args, privateData) {
        ...
        value.declarations.forEach(({ name, value }) => this[`_${cssPropertyToIDLAttribute(name)}`] = value)
    }
    ...
    set additiveSymbols(value) {
        ...
        value = parseCSSDeclaration({ name: 'additive-symbols', value }, createContext(this))?.value
        if (value) {
    ...
    set fallback(value) {
        value = parseCSSDeclaration({ name: 'fallback', value }, createContext(this))?.value
        if (value) {
    ...
    set name(value) {
        const name = parseCSSGrammar(CSS.escape(value), '<counter-style-name>', createContext(this))
        if (name) {
            const lowercase = name.value.toLowerCase()
            this._name = counterStyles.predefined.includes(lowercase)
                ? keyword(lowercase, ['counter-style-name'])
                : name
        }
    }
    ...
    set negative(value) {
        value = parseCSSDeclaration({ name: 'negative', value }, createContext(this))?.value
        if (value) {
    ...
    set pad(value) {
        value = parseCSSDeclaration({ name: 'pad', value }, createContext(this))?.value
        if (value) {
            this._pad = value
        }
    }
    ...
    set prefix(value) {
        value = parseCSSDeclaration({ name: 'prefix', value }, createContext(this))?.value
        if (value) {
            this._prefix = value
        }
    }
    ...
    set range(value) {
        value = parseCSSDeclaration({ name: 'range', value }, createContext(this))?.value
        if (value) {
            this._range = value
        }
    }
    ...
    set speakAs(value) {
        value = parseCSSDeclaration({ name: 'speak-as', value }, createContext(this))?.value
        if (value) {
            this._speakAs = value
        }
    }
    ...
    set suffix(value) {
        value = parseCSSDeclaration({ name: 'suffix', value }, createContext(this))?.value
        if (value) {
            this._suffix = value
        }
    }
    ...
    set symbols(value) {
        ...
        value = parseCSSDeclaration({ name: 'symbols', value }, createContext(this))?.value
    ...
    set system(value) {
        value = parseCSSDeclaration({ name: 'system', value }, createContext(this))?.value
        if (value) {
