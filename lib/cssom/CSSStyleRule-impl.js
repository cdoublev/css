...
import { isFailure, isOmitted, list, omitted } from '../values/value.js'
...
export default class CSSStyleRuleImpl extends CSSGroupingRuleImpl {
    ...
    constructor(globalObject, args, privateData) {
        ...
        const { value } = parseGrammar(input, { associatedToken: '{', type: 'block', value: definition.value }, { ...node, value: this }, 'lazy')
        const declarations = Array.isArray(value[0]) ? value.shift() : []
     ...
     */
    get cssText() {
        const { cssRules, selectorText, style: { cssText: declarations } } = this
        let value = cssRules._rules.map(rule => rule.cssText)
        if (declarations) {
            value.unshift(declarations)
        }
        value = value.join(' ')
        return value
            ? `${selectorText} { ${value} }`
    ...
    get selectorText() {
        ...
        return serializeComponentValues(this._selectors)
    }
