 ...
 */
function parseMediaQuery(input, globalObject) {
    input = parseGrammar(input, '<media-query-list>', globalObject)
...
export default class MediaListImpl {
     ...
     */
    set mediaText(text) {
        this._list.splice(0, this._list.length, ...parseGrammar(text, '<media-query-list>', this._globalObject))
    }
     ...
     */
    appendMedium(medium) {
        medium = parseMediaQuery(medium, this._globalObject)
     ...
     */
    deleteMedium(medium) {
        const { _globalObject, _list } = this
        medium = parseMediaQuery(medium, _globalObject)
        if (medium) {
            ...
            if (removed === 0) {
                throw error(DELETE_UNEXISTENT_MEDIUM_ERROR, _globalObject)
            }
        }
    }
