...
const { parseCSSGrammar, parseCommaSeparatedComponentValuesList } = require('../parse/engine.js')
...
function parseMediaQueryList(input) {
    return parseCommaSeparatedComponentValuesList(input)
        .map(query => parseCSSGrammar(query, '<media-query>'))
}
