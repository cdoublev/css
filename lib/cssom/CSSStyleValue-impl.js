...
const { parseCSSValue } = require('./CSSStyleDeclaration-impl.js')
...
function parseStyleValue(property, cssText, parseMultiple = false) {
    ...
    const values = parseCSSValue(property, cssText)
    if (values === null) {
        // values is already subdivided into iterations by `parseCSSValue()`
        if (parseMultiple) {
            return values[0]
        }
        return values
