...
const { parseCSSPropertyValue } = require('./CSSStyleDeclaration-impl.js')
...
function parseStyleValue(property, cssText, parseMultiple = false) {
    ...
    const values = parseCSSPropertyValue(cssText, property)
    if (values === null) {
        // values is already subdivided into iterations by `parseCSSPropertyValue()`
        if (parseMultiple) {
            return values[0]
        }
        return values
