...
const propertyDefinitions = require('../properties/definitions.js')
 ...
 * @returns {object|null}
 */
function getDeclarationValueDefinition({ name }, { rule: { definition: { name: ruleName, value: { descriptors, properties } } } }) {
    ...
    if (properties?.includes(name)) {
        ...
        if (mappings.has(name)) {
            return { name, type: 'property', value: propertyDefinitions[mappings.get(name)].value }
        }
        if (aliases.has(name)) {
            name = aliases.get(name)
        }
        return { name, type: 'property', value: propertyDefinitions[name].value }
    }
    if (descriptors) {
...
function getRuleDefinition({ name, value }, { root, rule }) {
    ...
    const { qualified = null, rules = [] } = contextDefinition
    if (name) {
        name = `@${name.toLowerCase()}`
    ...
    return rules.find(rule => rule.name === qualified)
}
