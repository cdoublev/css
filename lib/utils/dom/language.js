...
import { getParentElement, isHTMLElement, isSVGElement } from './node.js'
import { isDigit, toLowerCase } from '../string.js'
import { isError, isFailure } from '../../values/value.js'
import Stream from '../../parse/stream.js'
import { XML_NAMESPACE } from './constants.js'
...
function isAlphaNumeric(character) {
    return isAlphabetical(character) || isDigit(character)
}
...
function getPageLanguageTag({ ownerDocument }) {
    const selector = parseGrammar('meta[http-equiv=content-language i]', '<selector-list>', ownerDocument.defaultView)
...
function getElementLanguageTag(element) {
    ...
    if ((isHTMLElement(element) || isSVGElement(element)) && element.hasAttribute('lang')) {
