...
import { getParentElement, isHTMLElement, isShadowRoot, traverse } from './node.js'
...
function findContainedTextAutoDirectionality(element, canExcludeRoot) {
    if (canExcludeRoot && (element.dir || isHTMLElement(element, 'bdi', 'script', 'style', 'textarea'))) {
        return
    }
    for (const child of traverse(element, { nodeTypes: [ELEMENT_NODE_TYPE, TEXT_NODE_TYPE] })) {
        if (child.nodeType === ELEMENT_NODE_TYPE) {
            if (child.dir || isHTMLElement(child, 'bdi', 'script', 'style', 'textarea')) {
                return
            }
            if (isHTMLElement(child, 'slot')) {
...
function getAutoDirectionality(element) {
    if (isHTMLElement(element, 'textarea') || (isHTMLElement(element, 'input') && textInputTypes.includes(element.type))) {
        if (element.value.length === 0) {
            return null
        }
        return findStringDirectionality(element.value) ?? 'ltr'
    }
    if (isHTMLElement(element, 'slot') && isShadowRoot(element.getRootNode({}))) {
...
export default function getDirectionality(element) {
    switch (element.dir) {
        ...
        default: {
            if (isHTMLElement(element, 'bdi')) {
                return getAutoDirectionality(element) ?? 'ltr'
            }
            if (isHTMLElement(element, 'input') && element.type === 'tel') {
                return 'ltr'
            }
