...
const pendingSubstitutions = [...substitutions.arbitrary, ...substitutions.whole].map(definition => definition.name)
...
function isOmitted(component) {
    return Array.isArray(component) ? component.every(isOmitted) : component === omitted
}
 ...
 * @returns {boolean}
 */
function isSimpleBlock(value, associatedToken) {
