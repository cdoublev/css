...
function getPageSupportedColorSchemes(globalObject) {
    ...
    for (const meta of matchTreesAgainstSelectors([globalObject.document], selector)) {
        const colorScheme = parseGrammar(meta.getAttribute('content'), definition, globalObject)
...
function getUASupportedColorScheme(globalObject) {
    return states.get(globalObject).shared.agent.colorSchemes
}
...
export function getUserPreferredColorScheme(globalObject) {
    return states.get(globalObject).shared.user.colorScheme
}
