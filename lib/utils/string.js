 ...
 */
export function isIdentifierStartCharacter(character = '') {
    return /^[a-z_]$/i.test(character) || isNonASCIIIdentifierCharacter(character)
}
 ...
 */
export function isIdentifierCharacter(character = '') {
    return /^[-\w]$/.test(character) || isNonASCIIIdentifierCharacter(character)
}
 ...
 */
export function isNonPrintableCharacter(character = '') {
 ...
 */
export function isNonASCIIIdentifierCharacter(character = '') {
    const codePoint = character.codePointAt(0)
 ...
 */
export function startsWithEscape(first, second) {
    if (first instanceof Stream) {
        second = first.next()
        ;({ current: first } = first)
    }
    ...
    switch (first) {
        case '-':
            return isIdentifierStartCharacter(second)
                || second === '-'
                || startsWithEscape(second, third)
        case '\\':
            return startsWithEscape(first, second)
        default:
...
export function toCSSProperty(attribute, dashPrefix) {
    ...
    output += toLowerCase(attribute.replace(/[A-Z]/g, '-$&'))
    return output
}
 ...
 * @param {string} string
 * @returns {string}
 * @see {@link https://infra.spec.whatwg.org/#ascii-lowercase}
 */
export function toLowerCase(string) {
    let lowercase = ''
    for (const character of string) {
        const codePoint = character.codePointAt(0)
        lowercase += (0x41 <= codePoint && codePoint <= 0x5A) ? String.fromCharCode(codePoint | 32) : character
    }
    return lowercase
}
