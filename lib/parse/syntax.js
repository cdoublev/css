...
const INSERT_RULE_UNEXPECTED_NAMESPACE_ERROR = {
...
rules.qualified.value.rules = rules.nest.value.rules = nestedQualifiedRule.value.rules = {
    media: rules.media,
    nest: rules.nest,
    qualified: nestedQualifiedRule,
    supports: rules.supports,
}
...
/**
 * @see {@link https://drafts.csswg.org/css2/#charset%E2%91%A0}
 */
const charsets = [
    'US-ASCII',
    'ISO-8859-1',
 ...
 */
function parseCSSValue(value, property = '') {
 ...
 * @returns {object}
 */
function parseCSSDeclaration(declaration, parentRule) {
    ...
    if (parentRule) {
        const { name: ruleName } = parentRule
        const { value: { properties } } = rules[ruleName]
        if (!properties.includes(name)) {
            return null
        }
    }
    const parsed = parseCSSValue(value, name)
    if (parsed) {
        return { ...props, value: parsed }
    }
    return null
}
...
function isInvalidCSSNamespaceRule(rule, prevRules) {
    return CSSNamespaceRule.isImpl(rule)
        && prevRules.some(rule => !CSSImportRule.isImpl(rule) && !CSSNamespaceRule.isImpl(rule))
}
...
function isInvalidIndexForRule(list, rule, index) {
    if (CSSImportRule.isImpl(rule)) {
        return list.slice(0, index).some(rule => !CSSImportRule.isImpl(rule))
    }
    if (CSSNamespaceRule.isImpl(rule)) {
        return isInvalidCSSNamespaceRule(rule, list.slice(0, index))
            || list.slice(index).some(rule => CSSImportRule.isImpl(rule))
    }
    return list.slice(index).some(rule => CSSImportRule.isImpl(rule) || CSSNamespaceRule.isImpl(rule))
}
...
function insertCSSRule(list, rule, index, parentRule) {
    ...
    if (isInvalidCSSNamespaceRule(rule, list)) {
        throw createError(INSERT_RULE_UNEXPECTED_NAMESPACE_ERROR)
    }
 ...
 */
function matchRule({ name, prelude, type, value }, parentRule) {

    // 1. Check if rule is allowed in parent rule
    const { name: parentRuleName = 'stylesheet' } = parentRule
    const contextRules = getContextRules(parentRuleName)
    const definition = contextRules[name ?? 'qualified']
    if (!definition) {
        createError({ message: 'invalid or unrecognized rule', type: 'ParseError' })
        return null
    }
    // TODO: check against context rules (eg. no @import after any @n)

    // 2. Check grammar of the prelude (if any)
    const { prelude: preludeDefinition, type: ruleType, value: blockDefinition } = definition
    ...
    // 4. Create rule
    const properties = {
        name: name ?? ruleType ?? 'qualified',
        parentStyleSheet: parentRuleName === 'stylesheet' ? parentRule : null,
        prelude,
        type,
        value,
    }
    return create[ruleType ?? name ?? 'qualified'](globalThis, undefined, properties)
}
...
function parseCSSRule(rule, parentRule) {
    ...
    rule = matchRule(rule, parentRule)
    if (rule === null) {
 ...
 * @returns {object}
 */
function parseCSSDeclarationList(list, parentRule) {
    const declarations = []
    const rules = []
    parseDeclarationList(list).forEach(statement => {
        const { name, type } = statement
        if (type.has('at-rule')) {
            statement = matchRule(statement, parentRule)
            if (statement) {
                rules.push(statement)
            }
        } else {
            statement = parseCSSDeclaration(statement, parentRule)
            if (statement) {
...
/**
 * @param {object[]} list
 * @param {CSSStyleRuleImpl} rule
 * @returns {object}
 */
function parseCSSStyleBlock(list, parentRule) {
    const declarations = []
    const rules = []
 ...
 * @returns {CSSRule[]}
 */
function parseCSSRuleList(list, parentRule, parsed = false) {
    const topLevel = CSSStyleSheet.isImpl(parentRule)
    if (!parsed || typeof list === 'string') {
        list = parseRuleList(list, topLevel)
    }
...
const CSSNestingRuleImpl = require('../cssom/CSSNestingRule.js')
...
const create = {
    ...
    nest: CSSNestingRuleImpl.createImpl,
    page: CSSPageRule.createImpl,
    qualified: CSSStyleRule.createImpl,
