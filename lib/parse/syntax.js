 ...
 */
function matchRule({ name = 'qualified', prelude, type, value }, parentRule) {
    const { _definition } = parentRule
    const { value: { rules, type: contextType } } = _definition
    let included
    let excluded
    if (contextType === '<stylesheet>') {
    ...
    if (!definition) {
        console.error('Parse error: unrecognized rule')
        return null
    }
    if (excluded.includes(name)) {
        console.error('Parse error: invalid rule in this context')
        return null
    }
...
function parseCSSDeclarationList(list, parentRule) {
    const { definition: { value: { properties = [], rules = [] } } } = parentRule
    return parseDeclarationList(list).reduce((statements, statement) => {
        ...
        if (type.has('at-rule')) {
            ...
            if (definition) {
                parsed = matchRule(statement, parentRule)
            }
 ...
 * @returns {CSSRule[]}
 */
function parseCSSRuleList(list, parentRule) {
