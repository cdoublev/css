 ...
 */
function consumeFunction(tokens, { value: name }) {
    ...
    return { closed: false, name, type, value }
}
...
function consumeComponentValue(tokens) {
    ...
    if (currentToken.type?.has('function-token')) {
...
function normalizeIntoTokens(input) {
    if (typeof input === 'string') {
        // Trim input: https://github.com/w3c/csswg-drafts/issues/6484#issuecomment-1001482576
        input = tokenize(createStream(preprocess(input.replace(/^ +| +$/g, ''))))
...
function parseCSSPropertyValue(value, property, parser = createParser()) {
    ...
    return parser.parse(list, properties[property].value)
}
...
function parseCSSDeclaration({ important, name, type, value }, parser = createParser()) {
    ...
    if (value) {
        return { important, name, type, value }
    }
    return null
}
