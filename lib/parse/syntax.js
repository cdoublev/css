...
Object.values(styleRules).forEach(nested => {
    nested.value = {
...
const topLevelRules = { qualified: 'style', rules }
 ...
 */
function parseCSSMediaQueryList(input) {
 ...
 */
function parseCSSPropertyValue(value, property = '') {
...
function parseCSSDeclaration({ important, name, type = new Set(['declaration']), value }, context) {
    ...
    const parsed = parseCSSPropertyValue(value, name)
    if (parsed) {
...
function isInvalidCSSNamespaceRule(list, { type }) {
    return type === 'namespace' && list.some(({ type }) => type !== 'import' && type !== 'namespace')
}
...
function isInvalidIndexForRule(list, rule, index) {
    const { type } = rule
    if (type === 'import') {
        return list.slice(0, index).some(({ type }) => type !== 'import')
    }
    if (type === 'namespace') {
        return isInvalidCSSNamespaceRule(list.slice(0, index), rule)
            || list.slice(index).some(({ type }) => type === 'import')
    }
    return list.slice(index).some(({ type }) => type === 'import' || type === 'namespace')
}
...
function insertCSSRule(list, rule, index, parentRule) {
    ...
    rule = parseCSSRule(rule, getContextRules(parentRule))
 ...
 * @returns {string}
 */
function getRuleType({ name }, { qualified, rules = [] }) {
    // At-rule
    if (name) {
        // Margin at-rule
        if (pageMarginRuleNames.includes(name)) {
            return 'margin'
        }
 ...
 * @returns {object}
 */
function getIncludedRules(excluded) {
    return Object.entries(rules).reduce((included, [name, rule]) => {
...
/**
 * @param {object|CSSStyleSheetImpl|CSSRuleImpl} [parentRule]
 * @param {object} [parentContextRules]
 * @returns {object}
 */
function getContextRules(parentRule, parentContextRules = topLevelRules) {
    if (CSSStyleSheet.isImpl(parentRule)) {
        return topLevelRules
    }
    const { parentRule: grandParentRule, type } = parentRule
    if (grandParentRule) {
        parentContextRules = getContextRules(grandParentRule)
    }
    const { value: contextRules } = parentContextRules.rules[type]
    const { rules } = contextRules
    // <stylesheet>
    if (Array.isArray(rules)) {
        return { ...contextRules, rules: getIncludedRules(rules) }
    }
    return contextRules
}
...
function matchRule(rule, contextRules) {
    ...
    if (!definition) {
        createError({ message: 'invalid or unrecognized rule in this context', type: 'ParseError' })
        return null
    }
        ...
        if (value) {
            ...
            if (valueType.has('simple-block') && associatedToken === '{') {
                ...
                if (blockType === '<style-block>') {
                    value = parseCSSStyleBlock(value, blockDefinition)
                } else if (blockType === '<declaration-list>') {
                    value = parseCSSDeclarationList(value, blockDefinition)
                } else if (blockType === '<rule-list>') {
                    value = parseCSSRuleList(value, blockDefinition)
                } else if (blockType === '<stylesheet>') {
                    value = parseCSSRuleList(value, { ...blockDefinition, rules: getIncludedRules(rules) })
                } else {
 ...
 * @returns {object}
 */
function parseCSSDeclarationList(list, contextRules) {
    const declarations = []
    const rules = []
    parseDeclarationList(list).forEach(statement => {
        ...
        if (type.has('at-rule')) {
            ...
            statement = parseCSSDeclaration(statement, contextRules)
            if (statement) {
 ...
 */
function parseCSSStyleSheet(input, ignoreImport = false) {
    // TODO: decode rules as byte stream and handle (`@charset`) encoding
    const { rules } = parseCSSRuleList(input, topLevelRules, true)
