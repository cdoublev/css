 ...
 * @returns {object}
 */
function parseCSSDeclaration({ important, name, value }, context) {
    if (aliases.has(name)) {
        name = aliases.get(name)
    }
    if (context) {
        const { cascading = true, descriptors = [], properties = [] } = context
        console.log(name, properties.includes(name), important)
    ...
    if (parsed) {
        return { name, important, value: parsed }
    }
    return null
}
...
function parseCSSDeclarationList(list, contextRules, contextDeclarations) {
    ...
    parseDeclarationList(list).forEach(statement => {
        ...
        if (type.has('at-rule')) {
            ...
            statement = parseCSSDeclaration(statement, contextDeclarations, false)
            if (statement) {
