...
function getDeclarationsInSpecifiedOrder(declarations) {
    return declarations
        ...
        .reduce((declarations, declaration) => {
            ...
            if (declarations.has(name) && !important) {
                return declarations
            }
 ...
 * @returns {object}
 */
function parseCSSDeclaration(declaration, contextProperties) {
    const { value, ...props } = declaration
    let { name } = declaration
    if (aliases.has(name)) {
        name = aliases.get(name)
    }
    if (contextProperties && !contextProperties.includes(name)) {
        return null
    }
    ...
    if (blockDefinition) {
        if (value) {
            ...
            if (valueType.has('simple-block') && associatedToken === '{') {
                ({ value } = value)
                const { properties, rules, type: blockType } = blockDefinition
                if (blockType === '<style-block>') {
                    value = parseCSSStyleBlock(value, rules)
                } else if (blockType === '<declaration-list>') {
                    value = parseCSSDeclarationList(value, rules, properties)
 ...
 * @returns {object}
 */
function parseCSSDeclarationList(list, contextRules, contextProperties) {
    const declarations = []
    const rules = []
    parseDeclarationList(list).forEach(statement => {
        ...
        if (type.has('at-rule')) {
            ...
            statement = parseCSSDeclaration(statement, contextProperties)
            if (statement) {
