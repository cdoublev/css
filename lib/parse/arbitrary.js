...
import { delimiter, isCloseCurlyBrace, isDelimiter, isError, isOpenCurlyBrace, isWhitespace, list, omitted } from '../values/value.js'
import { getDeclarationDefinition, matchBlockContents, matchStyleSheet, parseGrammar } from './parser.js'
import { getFunction, getRuleName, isInputAtEnd, isProducedBy } from '../utils/context.js'
 ...
 * @param {object} node
 * @returns {boolean}
 * @see {@link https://drafts.csswg.org/css-values-5/#non-strict-free-form-production}
 * @see {@link https://github.com/w3c/csswg-drafts/issues/11042}
 */
function isRestrictedFreeFormValue(node) {
    const fn = getFunction(node)
    if (!fn) {
        return false
    }
    switch (fn.definition.name) {
        case null:
        case 'supports':
            return false
        case undefined:
            return isProducedBy(node, '<default-value>')
        case 'var':
            return node.parent.definition.type !== 'optional'
                || node.input.peek(isOpenCurlyBrace)
        default:
            return true
    }
}
...
export default {
    '<any-value>': node => matchFreeFormValue(node, matchAnyValue),
    ...
    '<declaration-value>': node => matchFreeFormValue(node, matchDeclarationValue),
    '<declaration>': matchDeclaration,
    '<qualified-rule-list>': matchBlockContents,
    '<rule-list>': node => getRuleName(node) === '@sheet'
