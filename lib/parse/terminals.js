...
const { definitions: dimensions, getTypeFromUnit } = require('../values/dimensions.js')
...
function parseZero(zero) {
    return zero.representation === '0' ? zero : null
}
...
function parseInteger(integer, node) {
    integer = parseNumber(integer, node)
...
function parseSignedInteger(integer, node) {
    integer = parseInteger(integer, node)
    if (integer) {
 ...
 */
function parseDimension(dimension, { name, min = -Infinity, max = Infinity }) {
    ...
    const { canonicalUnit, legacyZero = false, units } = dimensions.get(name)
    ...
    const { type, unit, value } = canonicalize(dimension)
