...
function simplifyCalculation(root, resolutionType) {
    ...
    if (Array.isArray(value)) {
        function simplifyArguments(components, component) {
            if (!isComma(component)) {
                if (Array.isArray(component)) {
                    ...
                    component = simplifyCalculation(component, resolutionType)
                }
        ...
        args = [simplifyCalculation(value, resolutionType)]
    }
    ...
    if (name.endsWith('progress') ) {
        if (name === 'progress' && args.every(node => !isCalculation(node) || isCombinable(node, resolutionType))) {
    ...
    if (substitutions.numeric.find(definition => definition.name === name).element) {
        return map(root, () => list(args, ','))
    }
    if (name === 'clamp' && args.some(component => component.value === 'none')) {
        ...
        switch (name) {
            ...
            case 'atan2':
                return angle(toDegrees(Math.atan2(...args.map(component => component.value))), 'deg', ['<calc-value>'])
            case 'clamp':
                return map(args[0], () => clamp(...args.map(node => node.value)), ['<calc-value>'])
            case 'cos':
            ...
            case 'mod':
                return map(
                    ...
                    ['<calc-value>'])
            case 'pow':
                return number(Math.pow(...args.map(node => node.value)), ['<calc-value>'])
            case 'rem':
                return map(
                    args[0],
                    value => {
