...
const { getCalculationType, getFeatureTypes, matchNumericType, types: numericTypes } = require('./types.js')
...
function simplifyCalculation(node, resolutionType) {
    ...
    node = map(node, function simplify(value) {
        if (isCalculation(value)) {
            if (name === 'container-progress' || name === 'media-progress') {
                const feature = node.value[0].value
                const rule = `@${name.split('-')[0]}`
                const types = getFeatureTypes(feature, rule)
                resolutionType = types.includes('<percentage>')
                    ? types.find(type => type !== '<percentage>')
                    : null
                return simplifyCalculation(value, resolutionType)
            }
            return simplifyCalculation(value, resolutionType)
        }
        if (isList(value) && !value.types.includes('<progress>')) {
