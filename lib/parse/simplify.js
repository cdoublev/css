...
function isResolvableCalculation(value, resolutionType) {
    ...
    return !isCalculation(value) || isCombinable(value, resolutionType)
}
...
export function simplifyCalculation(node, resolutionType, devicePixelRatio) {
    ...
    if (name === 'calc-mix') {
        ...
        const items = new Map
        let { length } = value
        value.forEach(([value, weight], index) => {
            if (isOmitted(weight)) {
                weight = percentage(distributedWeight)
            }
            if (weight.value === 0) {
                length--
                if (!resolutionType || !getCalculationType(value).percentHint) {
                    return
                }
                if (isNumeric(value, true) && value.types[0] === '<percentage-token>') {
        ...
        switch (simplified.length) {
            case 0:
                return map(value.find(item => isNumeric(item[0], true))[0], () => 0)
            case 1:
    ...
    if (isResolvableCalculation(value, resolutionType)) {
        switch (name) {
            ...
            case 'clamp': {
                ...
                return map(min, () => clamp(min.value, center.value, max.value), ['<calc-value>'])
            }
            case 'cos':
            ...
            case 'round': {
                ...
                if (strategy.value === 'line-width') {
                    if (isOmitted(b)) {
                        return map(a, a => roundToDevicePixel(a, devicePixelRatio), ['<calc-value>'])
                    }
                    const step = b.value
                    return map(a, a => {
                ...
                return map(a, a => round(strategy.value, a, b.value), ['<calc-value>'])
            }
            case 'sin':
