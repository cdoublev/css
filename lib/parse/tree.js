 ...
 * @returns {boolean}
 */
function isInvalidSequence({ length }, { definition, state }) {
    if (isSequence(definition)) {
        const { min = state.children.length } = definition
        return length < min
    }
    return false
}
...
class ParseTree {
     ...
     * @param {Parser} parser
     */
    constructor(input, definition, parser) {
        const { tree } = parser
        this.list = Array.isArray(input) ? createStream(input) : input
        this.parent = tree
        this.parser = parser
        this.create(parseDefinition(definition, { parent: tree?.tail, useCache: true }))
    }
