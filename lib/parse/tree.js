...
function deleteType(parser, { definition: { name, type }, value }) {
    if (name && (type === 'non-terminal' || type === 'property')) {
...
function match(parser, node) {
    ...
    if (isSequence(definition)) {
        result = parser.parseSequence(node)
    } else if (type === '|') {
        result = parser.parseNode(tree.create(state.children[0], node))
    } else if (type === 'optional' || type === 'required') {
        result = parser.parseNode(tree.create(value, node))
    } else if (type === 'non-terminal' || type === 'property') {
        result = parser.parseNode(tree.create(state.child, node))
    } else {
        result = parser.parseLeaf(node)
    }
    return tree.abort ? null : result
}
