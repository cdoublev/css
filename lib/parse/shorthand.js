...
function parseOrder(list, longhands) {
    const [{ value }, order] = list
    switch (value) {
        case 'layout':
            return new Map([['layout-order', order], ['reading-order', zero]])
        case 'reading':
            return new Map([['layout-order', zero], ['reading-order', order]])
        case 'reading-and-layout':
            return new Map([['layout-order', order], ['reading-order', order]])
        default:
            return parseLonghandsByIndex(list, longhands)
    }
}
