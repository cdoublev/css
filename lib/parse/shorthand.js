...
function parseRule(rules, longhands, shorthand) {
    ...
    rules.forEach(rule => {
        if (isList(rule)) {
            ...
            declarations.get(`${shorthand}-width`).push(isOmitted(width) ? properties[`${shorthand}-width`].initial.parsed[0] : width)
            declarations.get(`${shorthand}-style`).push(isOmitted(style) ? properties[`${shorthand}-style`].initial.parsed[0] : style)
            declarations.get(`${shorthand}-color`).push(isOmitted(color) ? properties[`${shorthand}-color`].initial.parsed[0] : color)
            ...
            rules.forEach(([width, style, color]) => {
                widths.push(isOmitted(width) ? properties[`${shorthand}-width`].initial.parsed[0] : width)
                styles.push(isOmitted(style) ? properties[`${shorthand}-style`].initial.parsed[0] : style)
                colors.push(isOmitted(color) ? properties[`${shorthand}-color`].initial.parsed[0] : color)
            })
