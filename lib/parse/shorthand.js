...
const { hasPendingSubstitution, isOmitted } = require('../utils/value.js')
...
function parseAnimationRange(ranges, longhands) {
    ranges = ranges.map(boundaries => {
        const [start, end] = boundaries
        if (Array.isArray(start) && isOmitted(end)) {
            return list([start, list([start[0], omitted], ' ', ['animation-range-end'])])
        }
        return boundaries
    })
    return parseCoordinatedValueList(ranges, longhands)
}
...
function parseGridTemplate(template, longhands) {
    ...
    return new Map([
        ['grid-template-areas', list(areas)],
        ['grid-template-columns', isOmitted(columns) ? none : columns[1]],
        ['grid-template-rows', list([list(rows), lineNames], ' ', ['<track-list>', '<explicit-track-list>'])],
    ])
}
 ...
 */
function parseLineClamp(values, longhands, legacy = false) {
    if (legacy) {
 ...
 */
function parseShorthand(value, shorthand) {
    const longhands = shorthands.get(shorthand)
    if (hasPendingSubstitution(value)) {
        value.pending = true
    ...
    switch (shorthand) {
        ...
        case 'corners':
            return parseCorners(value, longhands)
        case 'flex':
