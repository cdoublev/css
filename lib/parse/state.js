...
class ParserState {
    ...
    createNodeState(node) {
        const { list: { index: startIndex }, types } = this.#parser
        ...
        } else if (types.production[value]) {
            const definition = types.production[value]
            const type = parseDefinition(definition, { repeat: getRepeatConfiguration(value) })
            state = { startIndex, type }
        } else if (types.property[value]) {
            const { value: definition } = types.property[value]
     ...
     * @returns {string[]}
     *
     * If the given type is currently matched against `<type-percentage>`/etc
     * mixed types, it returns the mixed type as the production type, and the
     * given type as the context type, otherwise it returns the given type as
     * the production type and the context type is undefined.
     *
