...
const { findParent } = require('../utils/context.js')
...
function consumeMultiplier(chars, value, production) {
    charLoop: for (const char of chars) {
        switch (char) {
            ...
            case '#': {
                if (findParent({ parent: production }, node => node.definition.type === 'property')?.parent) {
                    return value
                }
                if (chars.consume('{')) {
...
function consumeDelimiter(chars, production) {
    ...
    return consumeMultiplier(chars, { type: 'token', value }, production)
}
...
function consumeIdentifier(chars, production) {
    ...
    return consumeMultiplier(chars, keyword, production)
}
...
function consumeType(chars, production) {
    ...
    return consumeMultiplier(chars, Object.fromEntries(entries), production)
}
...
function consumeSymbol(chars, production) {
    ...
    switch (char) {
        ...
        case '[': {
            ...
            chars.consume(']', Error)
            return consumeMultiplier(chars, group, production)
        }
        case '<':
...
function parseDefinition(string, production) {
    return consumeDefinition(new Stream(string), production)
}
