...
function consumeIdentifier(definition, currentChar, repeat) {
    ...
    const node = { type: 'terminal', range: identifier, value: 'keyword' }
...
function consumeType(definition, repeat) {
    ...
    for (const currentChar of definition) {
        ...
        if (currentChar === '>') {
            let node
            if (type.startsWith("'")) {
                node = { type: 'property', value: type.slice(1, -1) }
            } else if (structures.includes(type)) {
                node = { type: 'structure', value: type }
            } else if (types[type]) {
                node = { type: 'non-terminal', value: type }
            } else if (terminals[type]) {
                node = { type: 'terminal', value: type }
            } else {
...
function consumeDelimiter(definition, value, quote) {
    ...
    const multiplier = consumeMultiplier(definition)
    if (multiplier) {
        return { repeat: multiplier, type: 'delimiter', value }
    }
...
function consumeSimpleBlock(definition, associatedToken, quote) {
    ...
    if (associatedToken === '[') {
        ...
        // Remove closing quote following `]`
        args = args.slice(0, -1)
    } else {
...
function parseDefinition(definition, { combinator, nodes = [], parentCombinator, repeat, useCache = false } = {}) {
    ...
    charLoop: for (const currentChar of definition) {
        switch (currentChar) {
            ...
            // Simple block
            case '(':
                nodes.push(consumeSimpleBlock(definition, currentChar))
                break
            // Separators
            case ' ':
            case '&':
