...
const { MAX_ARGUMENTS: MAX_MATH_FUNCTION_ARGUMENTS, MAX_NESTING: MAX_MATH_FUNCTION_NESTING } = require('./math-functions.js')
...
const structures = require('../values/structures.js')
...
function getMaxRepeat(parent) {
    switch (parent?.definition.name) {
        case 'calc-sum':
        case 'calc-product':
            return MAX_MATH_FUNCTION_ARGUMENTS - 1
        case 'hypoth':
        case 'min':
        case 'max':
            return MAX_MATH_FUNCTION_NESTING
        default:
            return MAX_REPEAT
    }
}
 ...
 * @returns {object}
 */
function consumeType(string, parent = { definition: {} }) {
    ...
    } else if (structures.includes(name)) {
        type = 'structure'
    } else if (terminals[name]) {
        type = 'terminal'
    } else if (types[name]) {
        type = 'non-terminal'
        value = types[name]
 ...
 * @returns {object}
 */
function consumeGroup(string, parent) {
    let group = ''
    let open = 1
    string.consume(' ')
    for (const char of string) {
        if (char === ']') {
            if (open === 1) {
                return consumeMultiplier(string, parseDefinition(group), parent)
            }
            open--
        } else if (char === '[') {
            open++
 ...
 */
function parseDefinition(string, { parent, type, useCache = false, value = [] } = {}) {
    let cacheId
    if (typeof string === 'string') {
        ...
        cacheId = string
        string = createStream(string)
    }
    charLoop: for (const char of string) {
        switch (char) {
            // Group
            case '[':
                value.push(consumeGroup(string, parent))
                break
            // Simple block
            case '(':
            ...
            case '|':
                ...
                if (combinators.indexOf(type) < combinators.indexOf(combinator)) {
                    ...
                    value.push(parseDefinition(string, init))
                    break
                }
            ...
            // Production
            case '<':
                value.push(consumeType(string, parent))
                break
            // Keyword or function
            default:
