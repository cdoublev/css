 ...
 * @returns {number}
 */
function getMaxRepetitions(parent) {
    if (['<calc-product>', '<calc-sum>'].includes(parent?.definition.name)) {
        return MAX_MATH_FUNCTION_TERMS - 1
    }
    if (['<hypoth()>', '<min()>', '<max()>'].includes(parent?.definition.name)) {
 ...
 * @returns {object}
 */
function consumeMultiplier(chars, value, context, parent) {
    charLoop: for (const char of chars) {
        switch (char) {
            ...
            case '*':
                return { max: getMaxRepetitions(parent), min: 0, type: 'repetition', value }
            case '+':
                value = { max: getMaxRepetitions(parent), min: 1, type: 'repetition', value }
                continue
            case '{':
            ...
            case '#': {
                ...
                value = { max: getMaxRepetitions(parent), min: 1, separator: ',', type: 'repetition', value }
                continue
            }
            default:
                chars.reconsume()
 ...
 * @returns {object}
 */
function consumeIdentifier(chars, context, parent) {
    ...
    return consumeMultiplier(chars, keyword, context, parent)
}
 ...
 * @returns {object}
 */
function consumeType(chars, context, parent) {
    let min
    let max
    if (parent) {
        ({ definition: { max, min } } = parent)
    }
    let name = '<'
    charLoop: for (const char of chars) {
        switch (char) {
    ...
    return consumeMultiplier(chars, Object.fromEntries(entries), context, parent)
}
 ...
 * @returns {object}
 */
function consumeSymbol(chars, context, parent) {
    chars.consume(' ')
    ...
    switch (char) {
        ...
        case '[': {
            const group = consumeDefinition(chars, context, parent)
            chars.consume(']', Error)
            return consumeMultiplier(chars, group, context, parent)
        }
        case '<':
            return consumeType(chars, context, parent)
        case "'":
        case '"':
            return consumeDelimiter(chars, context, parent)
        case ',':
        case '/':
        case ':':
        case ';':
        ...
        default:
            return consumeIdentifier(chars, context, parent)
    }
}
 ...
 * @returns {object}
 */
function consumeDefinition(chars, context, parent, left = consumeSymbol(chars, context, parent), precedence = 0) {
    let combinator = peekCombinator(chars)
    while (combinator && precedence <= combinator.precedence) {
        const { end, precedence, type } = combinator
        chars.moveTo(end)
        chars.consume(' ')
        let right = consumeSymbol(chars, context, parent)
        combinator = peekCombinator(chars)
        while (combinator && precedence < combinator.precedence) {
            right = consumeDefinition(chars, context, parent, right, precedence + 1)
            combinator = peekCombinator(chars)
        }
        if (left.type === type) {
            if (type === right.type) {
 ...
 * @returns {object}
 */
function parseDefinition(string, context, parent) {
    return consumeDefinition(new Stream(string), context, parent)
}
