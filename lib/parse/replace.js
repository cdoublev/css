...
function replaceWithMathFunction(name, node, parser, topLevel) {
    ...
    if (topLevel) {
        ...
        const type = name === '<integer>' ? '<number>' : name
        const allowPercentage = type !== '<percentage>'
            && parent?.definition.type === '|'
            && parent.definition.value.some(definition => definition.name === '<percentage>')
        const resolutionType = allowPercentage ? type : null
        const numericType = getNumericFunctionType(match, resolutionType)
        if (matchNumericType(numericType, type, resolutionType)) {
