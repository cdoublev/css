...
function replaceNumeric(node, parser) {
    ...
    const { name: type, range } = definition
    ...
    if (!substitutions.numeric.math.includes(name)) {
        ...
        if (type === '<number>' && !substitutions.numeric.math.includes(context.function?.definition.name)) {
...
module.exports = {
    ...
    '<calc-keyword>': replaceColorChannel,
    '<flex>': replaceNumeric,
