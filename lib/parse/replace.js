...
import { colorFunctions, colorSpaceKeywords, colorSpaces } from '../values/color.js'
...
import { isDelimiter, isFailure } from '../values/value.js'
...
function replaceForgivingSelector(node) {
    ...
    if (!context.strict && findContext(node, node => node.definition.type === 'forgiving')) {
        return consumeComponentValues(input, ',', node)
    }
    return null
}
...
function replaceNumeric(node) {
    ...
    for (const fn of substitutions.numeric) {
        ...
        // Validate the type and simplify calculations once from top to bottom
        ...
        if (findParent(node, ({ definition: { name } }) => name === '<input-position>' || name === '<progress-source>')) {
...
function resolveColorComponentDefinition(node) {
    ...
    if (!isColorComponent(node)) {
        node = findContext(node, node => node.state.status === 'replacing' && isColorComponent(node))
    }
    ...
    if (node && findSibling(node, node => node.definition.name === '<color>')) {
        ...
        return colorFunctions[name].keywords
    }
}
