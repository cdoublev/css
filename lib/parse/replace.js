...
const { findContext, findFunction, findParent, findSibling, getFunction, getFunctionName, getRule, isProducedBy } = require('../utils/context.js')
...
function replaceCalcKeyword(node, parser) {
    return replaceWithCalcSizeKeyword(node, parser)
        ?? replaceWithColorComponentKeyword(node, parser)
}
