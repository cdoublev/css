...
function createState(node, list) {
    ...
    } else if (context.types.production[value]) {
        const definition = context.types.production[value]
        const type = parseDefinition(definition, { repeat: getRepeatConfiguration(value) })
        state = { startIndex, type }
    } else if (context.types.property[value]) {
        const { value: definition } = context.types.property[value]
 ...
 * @returns {object|null}
 */
function parseStructureType(list, node) {
    const { value } = node
    const parseStructure = context.types.production[value]
    if (parseStructure) {
        context.locked = true
        const parsed = parseStructure(list)
...
function parseNonTerminalType(list, node, isRoot) {
    ...
    if (value === 'calc-value' && context.getOperandsNumber() > MAX_FN_ARGS) {
        return parseNonTerminalType(list, node)
    }
 ...
 * @returns {object|null}
 */
function parseTerminalType(list, node) {
    const { range, value } = node
    const parse = context.types.terminal[value]
    if (!parse) {
 ...
 * @returns {boolean}
 *
 * It parses the next component value against a single separator or the first
 * matching separator (eg. when using `+#` as a multiplier), saved in state for
 * the next executions.
 *
 * Separators are not parsed to component values and are not saved in the list
 * of matched component values. Instead, the `separator` property of the list is
 * assigned the corresponding delimiter.
 *
 * While other separators are consumed by this function in `list`, a whitespace
 * separator is only consumed (if any) before parsing a terminal, a function, or
 * a simple block. A whitespace is allowed before and after other separators.
 */
function parseRepeatSeparator(list, definition, state) {
    const { values } = state
    let { separator } = state
    ...
    if (separator) {
        ...
        return list.consume(separator) || (list.consume(' ') && list.consume(separator))
    }
...
function parseRepeat(list, node) {
    ...
    while (!skip && length < max) {
        const { index } = list
        if (0 < length && !parseRepeatSeparator(list, separator, state)) {
            break
        }
...
function parseNode(list, node, isRoot) {
    ...
    switch (type) {
        ...
        case 'structure':
            return parseStructureType(list, node)
        case 'terminal':
            return parseTerminalType(list, node)
        default:
            throw RangeError('Unexpected node type')
    }
}
