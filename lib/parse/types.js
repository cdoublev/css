...
function getCalculationNodeType(node, resolutionType) {
    return node.types[0] === '<function>'
        ? getNumericFunctionType(node, resolutionType)
 ...
 * @returns {Map|null}
 */
function getNumericFunctionType({ name, value }, resolutionType) {
    switch (name) {
        case 'abs':
        case 'calc':
            return getCalculationNodeType(value, resolutionType)
        ...
        case 'tan': {
            const type = getCalculationNodeType(value, resolutionType)
            if (matchNumericType(type, '<number>', resolutionType) || matchNumericType(type, '<angle>', resolutionType)) {
        ...
        case 'log':
        case 'pow': {
            const types = value.filter(isCalculation).map(calculation => getCalculationNodeType(calculation, resolutionType))
            if (types.every(type => matchNumericType(type, '<number>', resolutionType))) {
                return types.reduce((result, type) => makeConsistentType(result, type), createType())
            }
            return null
        }
        case 'sibling-count':
...
function matchNumericType(type, production, resolutionType = null) {
    ...
    if (length === 1) {
        ...
        if (production === 'number') {
            return entry === 'percentage' && resolutionType === 'number'
        }
