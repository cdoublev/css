...
function getCalculationFunctionType({ name, value }, resolutionType) {
    switch (name) {
        ...
        case 'calc-mix': {
            ...
            const progressType = progress.types.at(-1) === "<'animation-timeline'>"
                ? createType()
                : getCalculationType(progress, '<number>')
            return matchNumericType(progressType, ['<number>', '<percentage>'], '<number>')
                ? combineCalculationTypes(addTypes, [start, end], resolutionType)
                : null
        }
        case 'clamp':
        case 'hypot':
