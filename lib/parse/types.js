...
function getMathFunctionType({ name, value }, contextType) {
    switch (name) {
        ...
        case 'sin':
        case 'tan':
            // Calculation must be <number> or <angle>, and function is resolved to <number>
        ...
        case 'acos':
        case 'atan':
            // Calculation must be <number>, and function is resolved to <angle>
            if (matchNumericType(getCalculationType(value), 'number')) {
                return createType(undefined, [['angle', 1]])
            }
            return null
        case 'atan2':
            // Calculations must have the same numeric type, and function is resolved to <angle>
            if (getCalculationType({ type: new Set(['calc-sum']), value: value.filter(isCalculation) })) {
                return createType(undefined, [['angle', 1]])
            }
            return null
        case 'sign':
            // Calculation must be any numeric type, and function is resolved to <number>
        ...
        case 'exp':
        case 'sqrt':
            // Calculation must be <number>, and function is resolved to <number>
            if (matchNumericType(getCalculationType(value), 'number')) {
                return createType(undefined, [['number', 1]])
            }
            return null
        case 'log':
        case 'pow':
            // Calculations must be <number>s, and function is resolved to <number>
