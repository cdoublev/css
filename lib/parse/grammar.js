...
function match(node, parser) {
    ...
    switch (type) {
        ...
        case 'omitted':
            return omitted
        case 'rule':
            return matchRule(input, node, parser)
        case 'simple-block':
 ...
 */
function matchRule(rule, node, parser) {
    const { definition } = node
    const context = { ...node.context, rule: node }
    let { prelude, value } = rule
    if (definition.prelude) {
        prelude = parser.parseCSSValue(new Stream(prelude, rule.source), definition.prelude, context)
    ...
    if (definition.value) {
        if (value) {
            return { ...rule, prelude, value: parser.parseCSSBlockContents(value, context) }
        }
