...
import { isBlock, isDelimiter, isError, isFailure, isList, isNull, isOmitted, isUndefined, isWhitespace } from '../utils/value.js'
 ...
 */
function intercept(node, error) {
    return actions.intercept[node.definition.name]?.(error, node) ?? error
}
...
function match(node) {
    ...
    switch (type) {
        ...
        case 'block':
            return input.consume(matchBlock, node)
        case 'forgiving':
            return parser.parseCSSGrammarList(input, forgiving[name], node)
        case 'function':
            return input.consume(matchFunction, node)
        case 'omitted':
            return omitted
        case 'token':
 ...
 * @returns {object|null}
 */
function matchBlock(value, node) {
 ...
 * @returns {object|null}
 */
function matchFunction(value, node) {
