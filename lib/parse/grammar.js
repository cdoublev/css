...
const compatibility = require('../values/compatibility.js')
...
const states = {
    aborted: {},
    // Result of matching omitted value, preprocessing, or replacing: disallow backtracking
    accepted: {
        actions: [assign],
        next: { state: 'rejected' },
    },
 ...
 */
function backtrack({ input, location }, parser, value) {
...
function match(node, parser) {
    ...
    switch (type) {
        case 'arbitrary':
            return arbitrary[name](input, parser)
        case 'block-contents':
            return parser.parseCSSBlockContents(input, context, parser)
        case 'forgiving':
...
function matchSimpleBlock(block, node, parser) {
    if (block.types[0] === '<simple-block>') {
        const { context, definition, input } = node
        const match = parser.parseCSSValue(new Stream(block.value, input.source), definition.value, context)
...
function matchFunction(fn, node, parser) {
    if (fn.types[0] === '<function>') {
        const { context, definition: { name, value } } = node
        const input = `${fn.name}()`
        const replacements = compatibility['*'].get(`${name}()`)
        if (replacements?.aliases.includes(input)) {
            fn = { ...fn, name }
        }
        if (name === fn.name || replacements?.mappings?.includes(input)) {
            if (value) {
                const match = parser.parseCSSValue(fn.value, value, { ...context, function: node })
                if (match instanceof SyntaxError) {
                    return match
                }
                if (match) {
 ...
 */
function matchRule(node, parser) {
    const { definition, input } = node
    const context = { ...node.context, rule: node }
    let { prelude, value } = input
    if (definition.prelude) {
        prelude = parser.parseCSSValue(new Stream(prelude, input.source), definition.prelude, context)
        if (prelude instanceof SyntaxError) {
            return prelude
        }
        if (prelude === null) {
            return error({ message: 'Invalid rule prelude' })
        }
    } else if (0 < prelude.length) {
        return error({ message: 'Unexpected rule prelude' })
    }
    if (definition.value) {
        if (value) {
            return { ...input, prelude, value: parser.parseCSSBlockContents(value, context) }
        }
        return error({ message: 'Missing rule block' })
    }
    if (value) {
        return error({ message: 'Unexpected rule block' })
    }
 ...
 * @returns {boolean}
 */
function matchToken(component, { definition: { name, value } }) {
    return name ? component.types[0] === name : isDelimiter(value, component)
}
 ...
 * @returns {number}
 */
function getAlternativeMaxLength({ max, type, separator }, alternative) {
    if (type === 'repetition') {
    ...
    while (alternative) {
        ...
        if (isSequence(definition)) {
            ...
            if (type === '&&' || type === '||') {
                ...
                value = node.children.map(node => node.value).filter(component => component.value !== listSeparator)
            } else {
...
function create(definition, input, context, parent = null) {
    if (typeof definition === 'string') {
        definition = parseDefinition(definition, context, parent)
    }
    const node = {
        children: [],
        context,
