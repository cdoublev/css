...
import { isDelimiter, isError, isList, isNull, isOmitted, isUndefined, isWhitespace, list, omitted } from '../values/value.js'
import {
    matchBlock,
    matchDeclaration,
    matchFunction,
...
/**
 * @param {object} node
 * @returns {CSSFontFeatureValuesMapImpl|CSSRuleImpl|SyntaxError|object|object[]|null|undefined}
 */
function match(node) {
    if (isBranch(node)) {
        return traverse(node)
    }
    const { input, definition } = node
    switch (definition.type) {
        case 'arbitrary':
            return arbitrary[definition.name](node)
        case 'block':
            return matchBlock(node)
        case 'declaration':
            return matchDeclaration(node)
        case 'forgiving':
            return parseListGrammar(input, forgiving[definition.name], node)
        case 'function':
...
/**
 * @param {object} node
 * @param {CSSFontFeatureValuesMapImpl|CSSRuleImpl|object|object[]} value
 ...
 * @param {object} node
 * @param {CSSFontFeatureValuesMapImpl|CSSRuleImpl|SyntaxError|object|object[]|null} [value]
 * @returns {boolean}
 */
...
function resolveTransition(node, value) {
    return states[node.state.status].transitions.find(transition => evalCondition(transition.condition, node, value))
}
...
export function parse(node) {
    let transition = states[node.state.status].next
    if (Array.isArray(transition)) {
        transition = transition.find(transition => evalCondition(transition.condition, node))
    }
...
export function create(definition, input, context, parent = null) {
    ...
    const node = {
        ...
        parent,
        state: { extended: new Map, status: 'waiting' },
        type,
        value: undefined,
    }
