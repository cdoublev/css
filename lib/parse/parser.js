 ...
 */
function consumeDeclarationValue(value, stops = [';', '!']) {
    return consumeAnyValue(value, stops)
}
...
function consumeBlock(tokens) {
    ...
    while (!tokens.atEnd()) {
        if (0 < tokens.consumeRunOf(isWhitespace).length || tokens.consume(isSemicolon)) {
            continue
        }
...
function parseCSSValueSubstitution(input, type, context) {
    const { declaration, rule, trees } = context
    if (!declaration || (type !== 'descriptor' && type !== 'property')) {
        return null
    }
    const { definition: { cascading, elemental } } = rule
    ...
    if (trees.length === 0) {
        const arbitrary = elemental
            ...
            : substitutions.arbitrary.filter(definition => !definition.element && (!definition.cascade || cascading))
