 ...
 */
function consumeDeclarationValue(value, restricted, stops = [';', '!']) {
    return consumeAnyValue(value, restricted, stops)
}
...
function consumeAtRule(tokens, nested = false) {
    ...
    const prelude = consumeComponentValuesList(tokens, nested ? ['{', ';', '}'] : ['{', ';'])
    if (tokens.consume(isOpenCurlyBrace)) {
        return { name, prelude, source: tokens.source, types: ['<rule>', '<at-rule>'], value: consumeBlock(tokens) }
    }
 ...
 */
function consumeAnyValue(input, restricted, exclude = []) {
    const values = list()
    input.consumeRunOf(isWhitespace)
    for (const component of input) {
        const { types: [type], value } = component
        if (type === '<delimiter-token>' && value === ' ') {
            continue
        }
        if (
            ...
            || type === '<delimiter-token>' && (
                endingTokens.includes(value)
                || exclude.includes(value)
                || (value === ',' && restricted))
            || (associatedToken === '{' && restricted)
        ) {
            input.reconsume()
            break
        }
