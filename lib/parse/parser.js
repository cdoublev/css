 ...
 */
function insertRule(list, rule, index, context, allowImport = false) {
 ...
 */
function consumeFunction(input, { start, value: name }) {
 ...
 * It deviates from the specification by trimming whitespaces.
 */
function consumeBlock(input, { start, value: associatedToken }) {
...
function consumeComponentValue(input) {
    const current = input.consume()
    if (associatedTokens[current.value]) {
        return consumeBlock(input, current)
    }
    if (current.types[0] === '<function-token>') {
        return consumeFunction(input, current)
    }
    return current
}
 ...
 * It deviates from the specification by trimming whitespaces.
 *
 * It deviates from the specification by receiving an optional and stop token
 * and context, and does not emit a parse error when processing `}`.
 */
 ...
 */
function consumeInvalidRule(input, nested, forAtRule = false) {
    while (!input.atEnd()) {
...
function matchDeclaration(node) {
    ...
    if (isFailure(value)) {
        if (isError(value) && value.message === INVALID_DECLARATION_SYNTAX_ERROR.message) {
            return null
        }
...
function matchQualifiedRule(node) {
    ...
    if (input.peek(isOpenCurlyBrace)) {
        return createCSSRule(node, undefined, prelude, input)
    }
    return error(INVALID_RULE_SYNTAX_ERROR)
}
...
/**
 * @param {object} node
 * @returns {DOMException|SyntaxError|CSSFontFeatureValuesMapImpl|CSSRuleImpl|null}
 */
function matchAtRule(node) {
    ...
    if (definition.prelude) {
        ...
        if (definition.value) {
            if (input.peek(isOpenCurlyBrace)) {
                return createCSSRule(node, name, prelude, input)
            }
            return null
        }
    ...
    if (input.peek(isOpenCurlyBrace)) {
        return createCSSRule(node, name, undefined, input)
    }
    return null
}
...
/**
 * @param {object} node
 * @returns {DOMException|SyntaxError|CSSFontFeatureValuesMapImpl|CSSRuleImpl}
 */
...
function parseRule(input, context) {
    ...
    if (isFailure(rule)) {
        return rule ?? error(INVALID_RULE_SYNTAX_ERROR)
    }
    input.consume(isWhitespace)
    if (input.atEnd()) {
        return rule
...
function parseDeclarationBlock(input, context) {
    context = createContext(context)
    const declarations = parseGrammar(input, context.definition.value, context)
 ...
 *
 * It deviates from the specification by consuming and validating items in a
 * single pass.
 */
