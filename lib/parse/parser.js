...
const { isCloseCurlyBrace, isColon, isDelimiter, isOpenCurlyBrace, isSemicolon, isWhitespace } = require('../utils/value.js')
...
function consumeFunction(tokens, { start, value: name }) {
    const value = consumeComponentValuesList(tokens, [')'])
...
function consumeSimpleBlock(tokens, { start, value: associatedToken }) {
    ...
    const value = consumeComponentValuesList(tokens, [endingToken])
 ...
 *
 * It deviates from the specification by trimming whitespaces and receiving `}`
 * as a stop token when appropriate.
 */
function consumeComponentValuesList(tokens, stops = []) {
    const values = list()
    ...
    while (isWhitespace(values.at(-1))) {
        values.pop()
    }
    return values
}
 ...
 */
function consumeDeclarationValue(value) {
    return consumeAnyValue(value, [';', '!'])
}
 ...
 */
function consumeBadDeclaration(tokens, nested = false) {
    while (!tokens.atEnd()) {
 ...
 */
function consumeDeclaration(tokens, nested = false) {
    ...
    const value = consumeComponentValuesList(tokens, nested ? [';', '}'] : [';'])
    const [penultimate, last] = value.slice(-2)
    let important = false
    if (penultimate && isDelimiter('!', penultimate) && last?.types[0] === '<ident-token>' && last.value === 'important') {
        value.splice(-2)
        important = true
    }
    ...
    if (1 < value.length && value.find(component => component.associatedToken === '{')) {
        return
    }
 ...
 */
function consumeQualifiedRule(tokens, nested = false) {
    const prelude = consumeComponentValuesList(tokens, nested ? ['{', '}', ';'] : ['{'])
 ...
 * for its prelude with the appropriate stop tokens.
 */
function consumeAtRule(tokens, nested = false) {
    const { value: name } = tokens.consume()
    const prelude = consumeComponentValuesList(tokens, nested ? ['{', ';', '}'] : ['{', ';'])
    if (tokens.consume(isOpenCurlyBrace)) {
        return { name, prelude, types: ['<rule>', '<at-rule>'], value: consumeBlock(tokens) }
    }
    if (tokens.atEnd() || tokens.consume(isSemicolon) || (nested && isCloseCurlyBrace(tokens.next()))) {
        return { name, prelude, types: ['<rule>', '<at-rule>'] }
    }
 ...
 *
 * It deviates from the specification by asserting that `{` is consumed before
 * running, and by consuming contents in place.
 */
 ...
 */
function consumeAnyValue(input, exclude = []) {
    const values = list()
    input.consumeRunOf(isWhitespace)
    for (const component of input) {
        const { types: [token], value } = component
        if (value === ' ') {
            continue
        }
        if (
            token === '<bad-string-token>'
            || token === '<bad-url-token>'
            || (token === '<delimiter-token>' && (endingTokens.includes(value) || exclude.includes(value)))
        ) {
            input.reconsume()
            break
        }
...
function parseComponentValuesList(input) {
    return consumeComponentValuesList(normalizeIntoTokens(input))
}
...
function parseCSSArbitrarySubstitution(input, context) {
    ...
    for (let component of input) {
        ...
        if (type === '<function>' || type === '<simple-block>') {
            if (substitutions.property.arbitrary.includes(name)) {
            ...
            const substitution = parseCSSArbitrarySubstitution(new Stream(value), context)
 ...
 */
function parseCSSPropertyValueSubstitution(input, context) {
    return parseCSSArbitrarySubstitution(input, context)
        ?? parseCSSValue(input, substitutions.property.keywords.join(' | '), context)
        ?? parseCSSValue(input, substitutions.property.whole.map(name => `<${name}()>`).join(' | '), context, 'greedy')
}
...
function parseCSSValue(input, definition, context, strategy = 'backtrack') {
    ...
    if (typeof input === 'string') {
        input = new Stream(parseComponentValuesList(input), input)
    ...
    if (root.definition.type === 'property') {
        const substitution = parseCSSPropertyValueSubstitution(input, context)
        if (substitution) {
            return substitution
        }
    }
...
function parseCSSDeclaration(declaration, context = createContext()) {
    if (declaration.important && !context.rule.definition.value.cascading) {
        return null
    }
...
const parser = module.exports = {
    ...
    parseCSSGrammar,
    parseCSSGrammarList,
    parseCSSPropertyValueSubstitution,
    parseCSSRule,
    parseCSSStyleSheet,
    parseCSSValue,
