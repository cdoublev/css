...
function consumeBadDeclaration(tokens, forCustomProperty, nested) {
    while (!tokens.atEnd()) {
        ...
        if (isOpenCurlyBrace(tokens.next())) {
...
function consumeDeclaration(tokens, context, nested) {
    ...
    if (!value || value instanceof SyntaxError) {
        return value
    }
 ...
 */
function parseCSSArbitrarySubstitution(input, definitions, context) {
    const matched = list([], '')
    let match = false
    for (let component of input) {
        ...
        if (type === '<function>' || type === '<simple-block>') {
            if (type === '<function>') {
                const definition = definitions.find(definition => parseCSSGrammar(name, definition.name))
                if (definition) {
                    component = parseCSSValue([component], `<${definition.name}()>`, context)
...
function parseCSSValue(input, definition, context, strategy = 'backtrack') {
    ...
    if (trees.length === 0 && (type === 'descriptor' || type === 'property')) {
        ...
        const substitution = parseCSSArbitrarySubstitution(input, definitions, context)
        if (substitution) {
            return substitution
        }
    }
