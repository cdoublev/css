...
const INVALID_DECLARATION_VALUE_ERROR = {
 ...
 */
function createCSSRule(name, prelude, value, definition, { root, rule }) {
    return cssom[definition.cssom].createImpl(globalThis, undefined, {
        name,
        parentRule: rule?.value,
        parentStyleSheet: root.value,
        prelude,
        value,
    })
}
...
function consumeDeclaration(tokens, context, nested) {
    ...
    value = parseCSSDeclarationValue(value, definition, { ...context, declaration: { context, definition } })
    if (isFailure(value)) {
...
function consumeQualifiedRule(tokens, context, nested) {
    ...
    if (tokens.consume(isOpenCurlyBrace)) {
        ...
        if (definition && !isFailure(prelude = parseCSSGrammar(prelude, definition.prelude, { ...context, rule: { context, definition } }))) {
...
function consumeAtRule(tokens, context, nested) {
    ...
    if (tokens.consume(isOpenCurlyBrace)) {
        ...
        if (!definition || (definition.prelude && isFailure(prelude = parseCSSGrammar(prelude, definition.prelude, { ...context, rule: { context, definition } })))) {
            tokens.reconsume()
            consumeComponentValue(tokens)
            return error(INVALID_RULE_SYNTAX_ERROR)
        }
    ...
    if (definition?.prelude && !definition.value && !isFailure(prelude = parseCSSGrammar(prelude, definition.prelude, { ...context, rule: { context, definition } }))) {
...
function parseCSSArbitrarySubstitution(input, context) {
    ...
    for (const { cascade, element, name, type = `<${name}()>` } of substitutions.arbitrary) {
        ...
        if (isFailure(parseCSSGrammar(input.name, name))) {
            continue
        }
...
function parseCSSValueSubstitution(input, context) {
    ...
    if (cascading || elemental) {
        const match = parseCSSGrammar(input, substitutions.keywords.join(' | '), context, 'greedy')
        if (!isFailure(match)) {
            return match
        }
    }
    // Try parsing <whole-value> substitution function
...
function parseCSSDeclarationValue(value, definition, context) {
    ...
    return parseCSSArbitrarySubstitutionContainingValue(value, context)
...
function parseCSSDeclaration(name, value, important, context) {
    ...
    value = parseCSSDeclarationValue(value, definition, context)
    if (isFailure(value)) {
        return value
    }
    if (forCustomProperty) {
