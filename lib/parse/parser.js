...
import { associatedTokens, closingTokens, openingTokens } from '../values/block.js'
...
function getDeclarationName(context, name) {
    ...
    const { definition } = getRule({ context })
    if (definition.value.properties) {
        return compatibility.properties.aliases.get(name) ?? name
    }
    return compatibility.descriptors[definition.name]?.aliases.get(name) ?? name
}
