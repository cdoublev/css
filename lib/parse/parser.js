 ...
 * when the consumed declaration cannot be parsed as a valid rule, to resume
 * consuming next content instead of backtracking.
 *
 * It deviates from the specification by consuming the declaration value like a
 * <declaration-value> but returning an error for <bad-*-token>, `]`, `)`, or
 * nothing for `!` not followed by `important` or a positioned {}-block.
 */
 ...
 * @param {object} context
 * @param {boolean} [nested]
 * @returns {CSSRuleImpl|CSSFontFeatureValuesMapImpl|undefined}
 ...
 * order.
 */
function consumeBlock(tokens, context) {
    const declarations = []
    const rules = []
    ...
    return { declarations: getDeclarationsInSpecifiedOrder(declarations), rules, types }
}
