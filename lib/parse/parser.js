 ...
 * @returns {boolean}
 */
function mustElideComma(list, index = list.index) {
    switch (list.source[index]) {
        case ' ':
            return mustElideComma(list, index - 1)
        case ',':
            // Adjacent commas
            return true
        default:
...
function simplifyCalcTree(root, contextType) {
    ...
    // Leaf: non-math functions (`var()` or `attr()`) that resolve to a numeric value (at computed value time)
    ...
    if (type.has('calc-product')) {
        const result = children
            ...
            // Resolve the product of unitless numbers
            .reduce((product, operand) => {
        ...
        // Resolve a product with "compatible" types
        if (result.every(operand => isNumeric(operand) || (operand.type.has('calc-invert') && operand.value.map(isNumeric)))) {
            ...
            if (productType && mathFunctionTypes.some(mathFnType => matchNumericType(productType, mathFnType))) {
                ...
                let initial
                if (type === 'number') {
                    initial = { type: new Set([type]), value: 1 }
                } else if (type === 'percentage') {
                    initial = { type: new Set([type]), unit: '%', value: 1 }
                } else {
                    initial = { type: new Set(['dimension', type]), unit: getCanonicalUnitFromType(type), value: 1 }
                }
                return result.reduce((product, operand) => {
                    if (operand.type.has('calc-invert')) {
                        ({ value: [operand] } = operand)
                    }
                    product.value *= operand.value
                    return product
                }, initial)
            }
        }
...
function getMathFunctionInternalRepresentation(fn, isTopLevel, contextType, range, round) {
    ...
    if (name === 'calc') {
        ...
        // TODO: the representation of a math function must not have `type`
 ...
 * @returns {object}
 *
 * It transforms a component value without side effects that would otherwise
 * make backtracking fail.
 */
...
class Parser {
    ...
    parseRepeat(node) {
        ...
        while (!skip && length < max) {
            ...
            if (match === null) {
                // Backtrack to the separator consumed before failing to parse
        ...
        if (max === 1) {
            ...
            const [match = omitted(node, list.index)] = values
            /**
             * - values matching optional types in a group multiplied with `!`
             * must not all be omitted
             * - whitespace is required between two <compound-selector> if
     ...
     * first matching separator when the repeated node is multiplied with `+#`,
     * then this separator is assigned to the node and it is read from it for
     * the next executions.
     *
     * The separator is consumed by this function, except a whitespace, which is
     * only consumed before parsing a terminal, a function, or a simple block.
     * It is not parsed to a component value and is not added to the list of
     * matched component values. Instead, this list has a `separator` property
     * that is assigned the corresponding separator.
     *
     * A whitespace is allowed before and after other separators.
     */
    parseRepeatSeparator(definition, state) {
        ...
        if (separator) {
            if (separator === ' ') {
                // Prevent infinite match of omitted value(s)
        ...
        separator = definition.find(separator => {
            // Whitespace separator must not be consumed by this function
            if (separator === ' ') {
                return false
            }
    ...
    parseSequence(node) {
        ...
        for (let typeIndex = 0; typeIndex < types.length; typeIndex++) {
            ...
            if (value === ',' && isOmitted(values.at(-1)) && mustElideComma(list)) {
