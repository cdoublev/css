 ...
 */
function insertRule({ _globalObject, _rules: list }, input, index, context, allowImport) {
    ...
    if (isError(rule)) {
        if (CSSGroupingRule.isImpl(context)) {
            ...
            if (0 < declarations.length) {
                rule = CSSNestedDeclarations.createImpl(_globalObject, undefined, {
                    declarations,
                    parentRule: context,
 ...
 */
function removeRule({ _globalObject, _rules: list }, index) {
    if (list.length <= index) {
        throw error(INVALID_RULE_INDEX_ERROR, _globalObject)
    }
...
function createRule(node, name, prelude) {
    ...
    return cssom[node.definition.cssom].createImpl(parentStyleSheet._globalObject, undefined, properties)
}
...
function consumeFunction(input) {
    ...
    const value = consumeComponentValues(input, [')'])
...
function consumeBlock(input) {
    ...
    const value = consumeComponentValues(input, [closingToken])
 ...
 */
function consumeComponentValues(input, stop, context) {
    const values = []
...
function matchDeclaration(node) {
    ...
    const { context, definition, input, state } = node
...
/**
 * @param {object} node
 * @returns {CSSFontFeatureValuesMapImpl|CSSRuleImpl|SyntaxError|null}
 */
function matchAtRule(node) {

    const { definition, input } = node
    // Assert: the next token matches the definition name
    const name = toLowerCase(input.consume().value)
...
function matchStyleSheet(node) {
    ...
    while (!isInputAtEnd(node)) {
        ...
        if (CSSNamespaceRule.isImpl(rule)) {
            context.namespaces.set(rule.prefix, rule.namespaceURI)
        }
        rules.push(rule)
    }
    return list(rules)
...
function getDeclarationName(context, name) {
    if (name.startsWith('--')) {
        return serializeIdentifier({ value: name })
    }
...
function createContext(value) {
    ...
    const globalObject = value?._globalObject ?? value?.ownerDocument?.defaultView
    ...
    // Assert: value is the (relevant) global object
    return { ...context, globalObject: value }
}
...
function normalizeIntoTokens(input) {
    ...
    if (typeof input === 'string') {
        return new Stream(
            {
                empty: [],
                iterator: tokenize(new Stream(
                    {
                        empty: '',
                        iterator: preprocess(input),
                    },
                    input)),
            },
            input)
...
function parseArbitrarySubstitution(input, context) {
    ...
    for (const { cascade, definition, element, name } of substitutions.arbitrary) {
        ...
        const node = grammar.create(definition, input, context)
...
function parseArbitrarySubstitutionContainingValue(input, context) {
    ...
    const node = grammar.create({ type: 'arbitrary' }, input, context)
    const { index } = input
    ...
    if (hasArbitrarySubstitution && !hasBadToken) {
        context.state.extended.set('pending', true)
        return value
    }
...
function parseDeclarationValue(input, definition, context) {
    ...
    if (typeof definition === 'string') {
        ...
        context = { ...context, context, definition, state: { extended: new Map } }
...
function parseDestructuredDeclaration(name, value, important, context) {
    ...
    if (node.state.extended.has('pending')) {
 ...
 * @returns {object[]}
 */
function parseDeclarations(input, context) {
    context = createContext(context)
 ...
 */
function parseListGrammar(input, definition, context) {
    input = normalizeIntoTokens(input)
    context = createContext(context)
    ...
    const value = [match]
    if (isFailure(match)) {
        consumeComponentValues(input, ',', context)
    }
