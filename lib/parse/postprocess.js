 ...
 *
 * It aborts parsing the feature when it is an unknown property or a declaration
 * whose value is either `revert` or `revert-layer`.
 */
function postParseStyleFeature(feature) {
    const { types, value } = feature
    if (types[0] === '<ident-token>' ? properties[value.toLowerCase()] : !value.value?.startsWith?.('revert')) {
        return feature
    }
    return null
}
