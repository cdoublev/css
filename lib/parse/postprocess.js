 ...
 *
 * It aborts parsing the keyword when it is invalid in the context.
 */
function postParseSizeKeyword(keyword, node) {
    const definition = { ...getDeclaration(node).definition, type: 'non-terminal' }
    keyword = getInputTokensFromMatch(keyword, node.input.data)
    keyword = parseGrammar(keyword, definition, node)
    if (isFailure(keyword)) {
        return keyword
    }
    keyword.types.push('<size-keyword>')
    return keyword
}
...
export default {
    ...
    '<size-keyword>': postParseSizeKeyword,
