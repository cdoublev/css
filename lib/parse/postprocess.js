...
const reserved = {
    ...
    flowInto: ['auto', 'none'],
    fontFamilyName: [
        ...types['<generic-complete>'].split(' | '),
        ...types['<generic-incomplete>'].split(' | '),
        ...types['<system-family-name>'].split(' | '),
    ],
    keyframeName: ['none'],
    lineName: ['auto', 'span'],
 ...
 *
 * It represents random() with its arguments in a comma separated list and with
...
function postParseRandom({ value: [options,, min,, max,, step], ...props }) {
    if (isOmitted(options)) {
        options = keyword('auto', ['<random-cache-key>'])
    }
