...
const { findLastNode, findParentTree, getTopLevelTree } = require('../utils/context.js')
...
const reservedMediaQueryTypes = ['and', 'layer', 'not', 'only', 'or']
 ...
 *
 * It invalidates the list when all font sources are invalid.
 */
function postParseFontSourceList(list) {
    list = postParseForgivingList(list)
    return 0 < list.length ? list : null
}
 ...
 * @returns {object[]}
 *
 * It represents the comma-separated list without invalid results.
 */
 ...
 *
 * It invalidates the type when it is a reserved keyword.
 */
function postParseMediaType(type) {
    return reservedMediaQueryTypes.includes(type.value) ? null : type
}
...
function isValidPseudoClass(type, tree) {
    ...
    // :has() nested in :has()
    if (type === 'has()' && findParentTree(tree, tree => tree.list.current.name === 'has')) {
        return null
    }
    // Pseudo-class qualifying pseudo-element
    const topLevel = getTopLevelTree(tree)
    const pseudoElement = findLastNode(
        topLevel.nodes,
...
function postParsePseudoElementSelector(selector, parser, node) {
    ...
    const lowercase = value.toLowerCase()
    const isDefined = pseudos.elements.identifiers[lowercase] ?? lowercase.startsWith('-webkit-')
