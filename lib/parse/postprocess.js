...
import { isCalculation, isColon, isFailure, isList, isOmitted, isWhitespace } from '../utils/value.js'
...
function postParseCustomFunctionDefinition(definition, node) {
    if (isFailure(parser.parseCSSGrammar(definition[0].name, '<dashed-ident>', node))) {
        return error(node)
    }
    return definition
}
...
function postParseFirstValid(substitution, node) {
    ...
    for (let value of substitution.value) {
        if (!isFailure(value = parser.parseCSSDeclarationValue(value, definition, context))) {
            return value
        }
    }
    return error(node)
...
function postParseFunctionParameter(parameter, node) {
    ...
    if (!isOmitted(type)) {
        if (!isOmitted(defaultValue)) {
            ...
            if (isFailure(parser.parseCSSGrammar(defaultValue[1], definition, node))) {
                return error(node)
            }
        }
...
function postParseMediaFeaturePlain(declaration, node) {
    ...
    const list = getInputComponentValuesFromMatch(value, node.input.data)
    const match = parser.parseCSSGrammar(list, definition, node)
    if (isFailure(match)) {
        return error(node)
    }
...
function postParseMediaFeatureRange(range, node) {
    ...
    for (let value of range.splice(0)) {
        if (value.types.at(-1) === '<mf-value>') {
            value = getInputComponentValuesFromMatch(value, node.input.data)
            value = parser.parseCSSGrammar(value, definition, node)
            if (isFailure(value)) {
                return error(node)
            }
...
function postParsePseudoClassSelector(selector, node) {
    // Pseudo-element
    if (isColon(node.input.prev(1, 1))) {
        return selector
    }
    ...
    if (name) {
        ...
        if (definition && isValidPseudoClass(name, node)) {
            ...
            value = parser.parseCSSGrammar(node.input.current.value, definition, node)
            if (!isFailure(value)) {
...
function postParsePseudoElementSelector(selector, node) {
    ...
    if (name) {
        ...
        if (definition && isValidPseudoElement(name, node)) {
            node = { ...node, definition: { name, type: 'function', value: definition } }
            value = parser.parseCSSGrammar(input.current.value, definition.value, node)
            if (!isFailure(value)) {
                const fn = { ...pseudo, name, value }
...
function postParseSizeKeyword(keyword, node) {
    const definition = { ...getDeclaration(node).definition, type: 'non-terminal' }
    return parser.parseCSSGrammar([keyword], definition, node)
}
...
function postParseSyntaxString(syntax, node) {
    return parser.parseCSSGrammar(syntax.value, '<syntax>', node) ?? error(node)
}
...
function postParseWholeValue(value, node) {
    ...
    return parser.parseCSSDeclarationValue(value, definition, node.context) ?? error(node)
}
