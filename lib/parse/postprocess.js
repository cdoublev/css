 ...
 * @see {@link https://drafts.csswg.org/css-values-5/#funcdef-palette-interpolate}
...
function postParseInterpolate(interpolation, node) {
    ...
    const progressType = isCalculation(progress) ? getCalculationType(progress, '<number>') : null
    let left
    if (matchNumericType(progressType, dimensionTypes, '<number>')) {
        left = progressType
    }
    for (const calculation of [...head, ...tail.flatMap(item => item[2])]) {
        const positionType = getCalculationType(calculation, '<number>')
        if (!matchNumericType(positionType, dimensionTypes, '<number>')) {
            continue
        }
        if (left) {
 ...
 * @returns {SyntaxError|object}
 *
 * It aborts parsing when the position is a calculation that does not match any
 * type into which a calculation can be resolved.
 *
 * It represents the interpolation position as a simplified calculation.
 */
function postParseInterpolatePosition(position, node) {
    if (isCalculation(position)) {
        if (position.types.includes('<calc-function>')) {
            position = position.value
            position.types.push(node.definition.name)
            return position
        }
        if (matchNumericType(getCalculationType(position, '<number>'), numericTypes, '<number>')) {
            return simplifyCalculation(position, '<number>')
        }
        return error(node)
    }
    return position
...
module.exports = {
    ...
    '<input-position>': postParseInterpolatePosition,
    ...
    '<palette-interpolate()>': postParseInterpolate,
    '<path()>': postParsePath,
    '<percentage>': postParseNumeric,
    '<polygon()>': postParsePath,
    '<progress-source>': postParseInterpolatePosition,
