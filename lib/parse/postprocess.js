...
import { isHexadecimal, isIdentifierCharacter, isWhitespace as isWhitespaceCharacter } from '../utils/string.js'
...
function postParseSyntax(syntax, node) {
    if (
        ...
        && cssWideKeywords.includes(serializeComponentValue(syntax).toLowerCase())
    ) {
        return error(node)
    }
    return syntax
...
function postParseViewTransitionClass(names, node) {
    if (isList(names) && names.some(name => reserved.viewTransitionClass.includes(name.value.toLowerCase()))) {
        return error(node)
    }
    return names
}
