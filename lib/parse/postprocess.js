...
import namedFeatures from '../values/named-features.js'
...
const units = ['%', ...[...dimensions.definitions.values()].flatMap(dimension => dimension.units)]
...
function postParseCalcValue(value, node) {
    ...
    if (value.types[0] === '<dimension-token>') {
        const type = dimensions.getTypeFromUnit(value.unit)
        if (type) {
            return { ...dimensions.canonicalize(value), types: [...value.types, type] }
        }
        return error(node)
    }
    return value
...
function postParseMediaFeatureRange(range, node) {
    ...
    const { type, value: definition } = getFeatures(node)[name.value]
    if (type !== 'range') {
        return error(node)
    }
    for (let value of range.splice(0)) {
        if (value.types.includes('<mf-value>')) {
            ...
            value = parseGrammar(value, definition, node)
            if (isFailure(value)) {
                return error(node)
            }
