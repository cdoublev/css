...
function getInputComponentValuesFromMatch(match, input) {
    if (Array.isArray(match)) {
        ...
        return input.slice(
            input.findIndex(component => component.start === head.start),
            input.findIndex(component => component.start === tail.start) + 1)
    }
    return [input.find(component => component.start === match.start)]
}
 ...
 *
 * It rejects the rule when it is lacking a mandatory declaration.
 */
 ...
 *
 * It represents the keyword in lowercase, with an aliased name replaced with
 * its target name.
 */
 ...
 */
function postParseMediaFeaturePlain(declaration, { context, input }, parser) {
 ...
 *
 * It aborts parsing the range when the feature type is `discrete` or some value
 * does not match the feature value definition.
 *
 ...
 *
 * It represents round() with its default optional values, to simplify further
 * processing.
 */
 ...
 *
 * It aborts parsing the feature when it is a declaration whose value does not
 * match the feature value definition or is either `revert` or `revert-layer`.
 *
 * It represents the feature with the result from its parsing.
 */
 ...
 *
 * It represents the block declaration with the result from its parsing.
 */
