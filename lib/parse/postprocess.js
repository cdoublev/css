...
import { isCalculation, isComputationallyIndependent, isFailure, isList, isOmitted, isWhitespace } from '../utils/value.js'
...
function postParseFunctionParameter(parameter, node) {
    ...
    if (!isOmitted(type)) {
        if (!isOmitted(defaultValue)) {
            const definition = serializeComponentValue(type.types[0] === '<function>' ? type.value : type)
...
function postParseMediaFeatureName(name, node) {
    ...
    let contextName = getMediaFeatureContextName(node)
...
function postParseMediaFeaturePlain(declaration, node) {
    ...
    const definition = descriptors[getMediaFeatureContextName(node)][unprefixed].value
...
function postParseMediaFeatureRange(range, node) {
    ...
    const { type, value: definition } = descriptors[getMediaFeatureContextName(node)][name.value]
    if (type !== 'range') {
        return error(node)
    }
 ...
 *
 * It represents the lines with `skip-all` when none are omitted.
 */
function postParseTextDecorationSkipSelf(lines) {
    if (isList(lines) && lines.every(line => !isOmitted(line))) {
        return keyword('skip-all', ['text-decoration-skip-self'])
    }
    return lines
}
...
export default {
    ...
    'text-decoration-skip-self': postParseTextDecorationSkipSelf,
