...
const { comma, notAll } = require('../values/defaults.js')
...
const { property: { keywords: cssWideKeywords } } = require('../values/substitutions.js')
...
const reserved = {
    ...
    flowInto: ['auto', 'none'],
    fontFamilyName: [
        ...nonTerminal['<generic-family-name>'].split(' | '),
 ...
 * @param {object} node
 * @returns {SyntaxError|attribute}
 * @see {@link https://drafts.csswg.org/css-values-5/#funcdef-attr}
 *
 * It aborts parsing when the attribute is nested in <src()>.
 */
function postParseAttribute(attribute, node) {
    if (findContext('function', node, node => node.input.current?.name === 'src')) {
        return error(node)
    }
    return attribute
}
 ...
 * @see {@link https://github.com/w3c/csswg-drafts/pull/10077}
 *
 ...
 */
function postParseColorStop(stop, { input }) {
    const [color, positions] = stop
    if (positions.length === 2) {
        input.data.splice(input.index, 1, comma, color, positions.pop())
        input.reconsume()
    }
    return stop
}
