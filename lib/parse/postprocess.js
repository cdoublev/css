...
import * as counterStyles from '../values/counter-style.js'
...
import { canonicalize, definitions as dimensions, getTypeFromUnit } from '../values/dimension.js'
import {
...
import { definitions as features, named as namedFeatures } from '../values/feature.js'
...
import { getRandomBaseValue } from '../state.js'
...
const reserved = {
    ...
    flowInto: ['auto', 'none'],
    fontFamilyName: [
        ...types['<generic-font-complete>'].split(' | '),
        ...types['<generic-font-incomplete>'].split(' | '),
        ...types['<system-font-family-name>'].split(' | '),
    ],
    keyframeName: ['none'],
    lineName: ['auto', 'span'],
    ...
    viewTransitionType: ['none'],
    voiceFamilyName: [...types['<gender>'].split(' | '), 'preserve'],
}
const units = ['%', ...[...dimensions.values()].flatMap(dimension => dimension.units)]
 ...
 * @returns {object[]}
 *
 * It returns the matched value as a flat list of tokens, which should be used
 * to validate it against a refined grammar.
 */
function getInputTokensFromMatch(match, input) {
    if (isList(match)) {
        ...
        return input.slice(
            ...
            input.findIndex(value => value.end === tail.end) + 1)
    }
    const { end, start, types: [type] } = match
    if (type === '<function>' || type === '<block>') {
        return input.slice(
            input.findIndex(value => value.start === start),
            input.findIndex(value => value.end === end) + 1)
    }
    return [input.find(value => value.start === start)]
}
 ...
 *
 * It aborts parsing when the symbols are not in descending order of weights.
 */
...
function postParseAnimateableFeature(name, node) {
    return reserved.animateableFeature.includes(toLowerCase(name.value)) ? error(node) : name
}
...
function postParseAnimationTimeline(timeline, node) {
    if (!getRule(node)?.definition.elemental || (timeline.types.includes('<keyword>') && isProducedBy(node, '<progress-source>'))) {
        return error(node)
    }
    return timeline
}
 ...
 * @param {object} unit
 * @returns {object}
 * @see {@link https://drafts.csswg.org/css-values-5/#typedef-attr-unit}
 *
 * It represents the unit in lowercase when it is a supported unit.
 */
function postParseAttributeUnit(unit) {
    const lowercase = toLowerCase(unit.value)
    if (units.includes(unit.value.toLowerCase())) {
        return { ...unit, value: lowercase }
    }
    return unit
}
 ...
 *
 * It represents the radii with explicit default values to simplify later
 * processing.
 */
function postParseBorderRadiusProperty(radii) {
    const [[h1, h2 = h1, h3 = h1, h4 = h2], vertical] = radii
    const horizontal = list([h1, h2, h3, h4])
    if (isOmitted(vertical)) {
        return list([horizontal, horizontal], '/', radii.types)
    }
    const [, [v1 = h1, v2 = v1, v3 = v1, v4 = v2]] = vertical
    return list([horizontal, list([v1, v2, v3, v4])], '/', radii.types)
}

/**
 * @param {object[]} radii
 * @returns {object[]}
 * @see {@link https://drafts.csswg.org/css-borders-4/#typedef-border-radius-syntax}
 *
 * It represents the radii with a whitespace separated list to simplify later
 * processing.
 */
function postParseBorderRadiusType(radii) {
    if (radii.types.includes('<slash-separated-border-radius-syntax>')) {
        const [head, tail] = radii
        return list(isOmitted(tail) ? [head] : [head, tail[1]], ' ', ['<legacy-border-radius-syntax>', '<border-radius>'])
    }
    return radii
}
 ...
 * @returns {object}
 * @see {@link https://drafts.csswg.org/css-values-4/#typedef-calc-keyword}
 *
 * It represents the keyword representing a constant as a <number>.
 */
function postParseCalcKeyword(keyword) {
 ...
 *
 * It represents the product with a flat list of invert and leaf nodes (step 3)
 * or with an unwrapped standalone node.
 */
 ...
 * @param {object} node
 * @returns {SyntaxError|object}
 * @see {@link https://drafts.csswg.org/css-values-5/#funcdef-calc-size}
 *
 * It aborts parsing when a calculation does not match <length-percentage>.
 *
 * It represents the size with simplified calculations.
 */
function postParseCalcSize(size, node) {
    const values = []
    for (let value of size.value) {
        if (isCalculation(value)) {
            if (!matchNumericType(getCalculationType(value, '<length>'), '<length>', '<length>')) {
                return error(node)
            }
            value = simplifyCalculation(value, '<length>')
        }
        values.push(value)
    }
    return { ...size, value: list(values) }
}
 ...
 * @see {@link https://drafts.csswg.org/css-values-4/#parse-a-calculation}
 *
 * It represents the sum with a flat list of negate and product nodes (step 4)
 * or with an unwrapped standalone node.
 */
 ...
 * @see {@link https://drafts.csswg.org/css-values-4/#parse-a-calculation}
 *
 * It represents the value with the corresponding dimension type.
 *
 * It represents the value without a simple block wrapper (step 5.1).
 */
function postParseCalcValue(value, node) {
    if (isBlock(value)) {
        return value.value
    }
    if (value.types[0] === '<dimension-token>') {
        const type = getTypeFromUnit(value.unit)
        if (type) {
            return { ...canonicalize(value), types: [...value.types, type] }
        }
        return error(node)
    }
    return value
 ...
 *
 * It aborts parsing when the color is relative whereas the context requires an
 * absolute color.
 */
 ...
 *
 * It invalidates the schemes when they include a trailing `only`.
 *
 * It aborts parsing when some scheme is a reserved keyword in the context.
 */
function postParseColorScheme(schemes, node) {
    if (isList(schemes)) {
        for (const [index, scheme] of schemes[0].entries()) {
            const lowercase = toLowerCase(scheme.value)
            if (lowercase === 'only' && (index + 1) === schemes.length) {
                return
            }
            if (reserved.colorScheme.includes(lowercase)) {
                return error(node)
            }
        }
    }
...
function postParseComponents(components, node) {
    if (components.some(component => reserved.components.includes(toLowerCase(component.value)))) {
        return error(node)
    }
    return components
}
...
function postParseContainerNameProperty(names, node) {
    if (isList(names) && names.some(name => isError(postParseContainerNameType(name, node)))) {
        return error(node)
    }
    return names
}
...
function postParseContainerNameType(name, node) {
    return reserved.containerName.includes(toLowerCase(name.value)) ? error(node) : name
}
 ...
 *
 * It aborts parsing or invalidates the name when it is a reserved keyword in
 * the context.
 */
function postParseCounterName(name, node) {
    if (reserved.counterName.includes(toLowerCase(name.value))) {
        if (getFunctionName(node) === 'reversed') {
            return error(node)
        }
        return
    }
 ...
 *
 * It represents the name with a keyword when it matches a predefined name.
 */
function postParseCounterStyleName(name, node) {
    ...
    if (reserved.counterStyleName.includes(lowercase)) {
        if (getDeclarationName(node)?.startsWith('list-style')) {
            return
        }
        return error(node)
    }
    if (counterStyles.predefined.includes(lowercase)) {
        if (!getDeclaration(node) && counterStyles.nonOverridable.includes(lowercase)) {
            return error(node)
        }
 ...
 *
 * It represents the type without a type() wrapper when it stands alone.
 */
function postParseCSSType(type) {
    const { value } = type
    if (isFunction(type) && isList(value) && isOmitted(value[1])) {
        return value[0]
    }
    return type
}
 ...
 * @returns {SyntaxError|object}
 *
 * It aborts parsing when the custom function is defined with an invalid
 * <dashed-ident> for its name.
 */
function postParseCustomFunctionDefinition(definition, node) {
    if (isFailure(parseGrammar(definition[0].name, '<dashed-ident>', node))) {
        return error(node)
    }
    return definition
}
...
function postParseCustomIdentifier(identifier) {
    const lowercase = toLowerCase(identifier.value)
 ...
 * @param {object} node
 * @returns {SyntaxError|object}
 * @see {@link https://drafts.csswg.org/css-values-5/#funcdef-cycle}
 *
 * It aborts parsing when the cycle is nested in another cycle.
 */
function postParseCycle(cycle, node) {
    if (findFunction(node, node => node.definition.name === 'cycle')) {
        return error(node)
    }
    return cycle
}
 ...
 */
function postParseDashedIdentifier(identifier) {
 ...
 *
 * It aborts parsing the dimension when its unit is unknown or when its value is
 ...
 * @see {@link https://drafts.csswg.org/css-flexbox-2/#valdef-flex-flex-basis}
 *
 * It invalidates the basis when it is `0px` declared as `0`, which was first
 * claimed by <'flex-shrink'> before backtracking.
 */
function postParseFlexBasis(basis, { parent: { children } }) {
    if (basis.types.includes('<length>') && basis.start === undefined && isOmitted(children[0]?.value[1])) {
        return
    }
    return basis
}
 ...
 * @see {@link https://drafts.csswg.org/css-fonts-4/#font-family-name-value}
 *
 * It aborts parsing the name when it is a reserved keyword in the context.
 */
 ...
 *
 * It aborts parsing when all arguments are invalid in the context.
 *
 * It represents the substitution with the first valid value.
 */
function postParseFirstValid(substitution, node) {
    const { context } = node
    const definition = { ...getDeclaration(node).definition, type: 'non-terminal' }
    for (let value of substitution.value) {
        if (!isFailure(value = parseDeclarationValue(value, definition, context))) {
            return value
        }
    }
    return error(node)
...
function postParseFlowInto(name, node) {
    if (isList(name) && reserved.flowInto.includes(toLowerCase(name[0].value))) {
        return error(node)
    }
    return name
}
 ...
 *
 * It aborts parsing when the format is an unknown legacy format string.
 */
 ...
 * @param {object} node
 * @returns {SyntaxError|CSSFontPaletteValuesRuleImpl}
 * @see {@link https://drafts.csswg.org/css-fonts-4/#at-ruledef-font-palette-values}
 *
 * It aborts parsing the rule when it is missing a declaration for
 * `font-family`.
 */
function postParseFontPaletteValues(rule, node) {
    return rule.fontFamily ? rule : error(node)
}
 ...
 *
 * It aborts parsing when the list is empty (ignoring invalid font sources).
 *
 ...
 *
 * It aborts parsing when some selector is invalid and the context disallows
 * forgiving parsing (strict).
 */
 ...
 * @see {@link https://drafts.csswg.org/css-syntax-3/#typedef-function-token}
 *
 * It aborts parsing the token when it does not match the expected value.
 */
function postParseFunctionToken(token, node) {
    const { definition: { range } } = node
    if (range) {
        const name = parseGrammar(token.value, range, node)
        if (isFailure(name)) {
            return name
        }
        token = { ...token, value: name.value }
    }
    return token
}
 ...
 */
function postParseFunctionParameter(parameter, node) {
    const [name, type, defaultValue] = parameter
    if (node.parent?.children.some((parameter, index) => ++index % 2 && parameter.value[0].value === name.value)) {
        return error(node)
    }
    if (!isOmitted(type)) {
        if (!isOmitted(defaultValue)) {
            const definition = serializeComponentValue(isFunction(type) ? type.value : type)
            if (isFailure(parseGrammar(defaultValue[1], definition, node))) {
                return error(node)
            }
        }
        if (isFunction(type)) {
 ...
 *
 * It aborts parsing or invalidates the line when its name is is a reserved
...
function postParseGridLine(line, node) {
    if (isList(line)) {
        ...
        if (!isOmitted(identifier)) {
            const name = toLowerCase(identifier.value)
            if (name === 'auto') {
                return error(node)
            }
            if (name === 'span') {
    ...
    if (line.types.includes('<custom-ident>') && toLowerCase(line.value) === 'span') {
        return
    }
    return line
}
 ...
 *
 * It represents the areas with collapsed null cell tokens and with cell tokens
 ...
 *
 * It aborts parsing when the interpolation has inconsistent absolute positions
 * or when the progress position is absolute but no stop position is absolute.
 */
function postParseInterpolate(interpolation, node) {
    ...
    const positions = isOmitted(tail) ? head : [...head, ...tail.flatMap(item => item[2])]
 ...
 *
 * It aborts parsing when the name is a reserved keyword in the context or the
 * empty string.
 */
function postParseKeyframesName(name, node) {
    if (name.types.includes('<custom-ident>') && reserved.keyframeName.includes(toLowerCase(name.value))) {
        return error(node)
    }
    if (name.types.includes('<string>') && name.value === '') {
        return error(node)
    }
    return name
}
 ...
 * target name.
 */
function postParseKeyword(keyword, node) {
    const lowercase = toLowerCase(keyword.value)
    const { definition: { range } } = node
    if (lowercase === toLowerCase(range)) {
        return { ...keyword, value: lowercase }
    }
    if (getFunctionName(node) === range) {
    ...
    const { aliases, mappings } = compatibility.values.keywords[getDeclarationName(node)] ?? {}
    if (aliases?.get(lowercase) === range) {
        return { ...keyword, value: range }
    }
 ...
 *
 * It aborts parsing when the name includes a globally reserved keyword.
 */
...
function postParseLineNames(names, node) {
    if (names.value.some(name => reserved.lineName.includes(toLowerCase(name.value)))) {
        return error(node)
    }
    return names
}
 ...
 * @returns {object|object[]}
 * @see {@link https://drafts.csswg.org/css-anchor-position-1/#typedef-position-try-fallbacks-try-tactic}
 * @see {@link https://svgwg.org/svg2-draft/painting.html#PaintOrderProperty}
 *
 * It represents the list items in the same order as declared.
 */
function postParseListToDeclaredOrder(list) {
 ...
 * @param {object} log
 * @returns {object}
 * @see {@link https://drafts.csswg.org/css-values-4/#funcdef-log}
 *
 * It represents the log with its base and the preceding comma replaced with
 * omitted values when the base is equal to the base of natural logarithms, to
 * simplify later processing.
 */
function postParseLog(log) {
    const { value } = log
    const base = value.at(-1)
    if (base.value === Math.E) {
        value.splice(-2, 2, omitted, omitted)
    }
    return log
}
...
function getFeatures(node) {
    ...
    switch (rule) {
        case '@container':
            return features[rule][node.context.trees.at(-1).definition.name]
        case '@import':
        case '@media':
        case '@sheet':
            return features['@media']
        default:
            return descriptors[`@${getFunctionName(node).split('-progress')[0]}`] ?? {}
    }
}
...
function postParseMediaFeatureName(name, node) {
    ...
    const features = getFeatures(node)

    if (features[lowercase]?.type !== 'range' && getFunctionName(node)?.endsWith('-progress')) {
        return error(node)
    }

    const target = compatibility.descriptors[getRuleName(node)]?.aliases.get(unprefixed)
    ...
    if (lowercase === unprefixed) {
        ...
        if (features[unprefixed]?.type === 'range') {
            return { ...name, value: lowercase }
        }
    }
    return error(node)
 ...
 * definition.
 *
 * It represents the value with the result of parsing against its definition.
 */
function postParseMediaFeaturePlain(declaration, node) {
    const [name,, value] = declaration
    if (!value.types.includes('<mf-value>')) {
        return declaration
    }
    const unprefixed = name.value.replace(/(min|max)-/, '')
    const definition = getFeatures(node)[unprefixed].value
    const list = getInputTokensFromMatch(value, node.input.data)
    const match = parseGrammar(list, definition, node)
    if (isFailure(match)) {
        return error(node)
    }
 ...
 * be parsed against its definition.
 *
 * It represents the range with the values resulting from parsing against their
 * definitions.
 */
function postParseMediaFeatureRange(range, node) {
    const name = range.find(value => value.types.includes('<mf-name>'))
    const feature = getFeatures(node)[name.value]
    if (feature.type !== 'range') {
        return error(node)
    }
    for (let value of range.splice(0)) {
        if (value.types.includes('<mf-value>')) {
            value = getInputTokensFromMatch(value, node.input.data)
            value = parseGrammar(value, feature.value, node)
            if (isFailure(value)) {
                return error(node)
            }
 ...
 *
 * It represents the list with invalid media queries replaced with `not all`.
 */
...
function postParseMediaType(type, node) {
    const lowercase = toLowerCase(type.value)
    return reserved.mediaType.includes(lowercase) ? error(node) : { ...type, value: lowercase }
}
...
function postParseNSPrefix(prefix, node) {
    const name = prefix[0]
    if (isOmitted(name) || node.context.namespaces.has(name.value)) {
        return prefix
    }
 ...
 *
 * It aborts parsing the numeric when its value is outside a defined range.
 */
 ...
 * @see {@link https://svgwg.org/svg2-draft/painting.html#PaintOrderProperty}
 *
 * It represents the paint operations in the same order as declared.
 */
function postParsePaintOrder(paint) {
    return isList(paint) ? postParseListToDeclaredOrder(paint) : paint
}
...
function postParsePath(path, node) {
    if (isDeclaredBy(node, 'offset-path')) {
        return { ...path, value: list([omitted, omitted, ...path.value.slice(2)]) }
    }
    return path
}
 ...
 * @param {object} node
 * @returns {SyntaxError|CSSPropertyRuleImpl}
 * @see {@link https://drafts.css-houdini.org/css-properties-values-api-1/#at-ruledef-property}
 *
 * It aborts parsing the rule when its `initial-value` is invalid according to
 * its `syntax`.
 */
function postParseProperty(rule, node) {
    const { initialValue, syntax } = rule
    if (syntax === '"*"') {
        return cssWideKeywords.includes(initialValue) ? error(node) : rule
    }
    if (initialValue) {
        const context = createVirtualContext(node.context.globalObject, '@style')
        const match = parseGrammar(initialValue, syntax.slice(1, -1), context)
 ...
 * @returns {object}
 */
function getPseudoElementDefinition({ value: [, [, { name, value }]] }) {
    return pseudos.elements[name ? `${name}()` : value]
}
 ...
 * @returns {boolean}
 */
function isValidPseudoClass(type, node) {
    if (pseudos.userActions[type] || pseudos.logical[type]) {
        return true
    }
    // :has() nested in :has()
    if (type === 'has()' && findFunction(node, node => node.definition.name === 'has')) {
        return false
    }
    ...
    const pseudoElement = findSibling(
        ...
        true)
    if (pseudoElement) {
        return getPseudoElementDefinition(pseudoElement).classes?.includes(type)
    }
    return true
}
 ...
 * - is an unknown pseudo-class identifier
 * - is invalid in the context
 * - has an invalid argument
 */
function postParsePseudoClassSelector(selector, node) {
    // Pseudo-element
    if (isProducedBy(node, '<pseudo-element-selector>')) {
        return selector
    }
    ...
    if (name) {
        ...
        const type = `${name}()`
        const definition = pseudos.classes[type]
        if (definition && isValidPseudoClass(type, node)) {
            node = { ...node, definition: { name, type: 'function', value: definition.value } }
            value = parseGrammar(value, definition.value, node)
            if (!isFailure(value)) {
                pseudo.name = name
                pseudo.value = value
                return list([colon, pseudo], '', ['<pseudo-class-selector>'])
            }
        }
        return error(node)
    }
    ...
    if (pseudos.aliases.classes.has(value)) {
        value = pseudos.aliases.classes.get(value)
    }
    if (pseudos.classes[value] && isValidPseudoClass(value, node)) {
 ...
 */
function postParsePseudoElementSelector(selector, node) {
    ...
    if (name) {
        name = toLowerCase(name)
        const type = `${name}()`
        const definition = pseudos.elements[type]
        if (definition && isValidPseudoElement(type, node)) {
            node = { ...node, definition: { name, type: 'function', value: definition.value } }
            value = parseGrammar(value, definition.value, node)
            if (!isFailure(value)) {
                pseudo.name = name
    ...
    value = toLowerCase(value)
    const isDefined = pseudos.elements[value]
        ?? (value.startsWith('-webkit-') && !node.context.strict)
 ...
 *
 * It aborts parsing when the shape of the gradient is a circle declared with
 ...
 *
 * It aborts parsing when the size is declared with two <radial-radius> for
 * circle().
 */
function postParseRadialSize(size, node) {
    if (size.length === 2 && getFunctionName(node) === 'circle') {
        return error(node)
    }
    return size
}
 ...
 *
 * It aborts parsing when the key is element-dependent while the context is not
 * elemental.
 *
 * It represents random():
 * - with its value in a comma separated list, to simplify later processing
 * - with an explicit default <random-key> when omitted
 * - with `property-*-scoped` replaced with the appropriate <random-ua-ident>
 * - with a `base` value when it can be resolved
 */
function postParseRandom({ value: [key,, min,, max,, step], ...props }, node) {
    if (isOmitted(key)) {
        if (!getRule(node)?.definition.elemental) {
            return error(node)
        }
        key = list([omitted, keyword('element-scoped'), keyword('property-index-scoped')], ' ', ['<random-cache-key>', '<random-key>'])
    }
    let base
    // Assert: the key is a <random-cache-key> or a fixed <number>
    if (key.types.includes('<random-cache-key>')) {
        let [identifier, elementScoped, propertyScoped] = key
        // Replace `property-*-scoped` with the corresponding `<random-ua-ident>`
        if (propertyScoped.types.includes('<keyword>')) {
            const declaration = getDeclaration(node)
            const property = declaration.definition.name
            if (propertyScoped.value === 'property-index-scoped') {
                let index = declaration.state.extended.get('random') ?? 0
                declaration.state.extended.set('random', ++index)
                propertyScoped = customIdentifier(`ua-${property}-${index}`, ['<random-ua-ident>'])
            } else {
                propertyScoped = customIdentifier(`ua-${property}`, ['<random-ua-ident>'])
            }
            key = list([identifier, elementScoped, propertyScoped], ' ', ['<random-cache-key>', '<random-key>'])
        }
        // Resolve the random base value if it is not scoped to element
        if (isOmitted(elementScoped)) {
            base = getRandomBaseValue(
                node.context.globalObject,
                {
                    identifier: isOmitted(identifier) ? null : serializeComponentValue(identifier),
                    scope: isOmitted(propertyScoped) ? null : serializeComponentValue(propertyScoped),
                })
        }
    } else {
        base = key[1].value
    }
    return { ...props, base, value: list(isOmitted(step) ? [key, min, max] : [key, min, max, step], ',') }
}
 ...
 * @param {object} node
 * @returns {SyntaxError|object[]|undefined}
 * @see {@link https://drafts.csswg.org/css-values-5/#typedef-random-cache-key}
 *
 * It invalidates the key when it is the `fixed` keyword.
 *
 * It aborts parsing when the key is element-dependent while the context is not
 * elemental.
 */
function postParseRandomCacheKey(key, node) {
    const [, elementScoped, propertyScoped] = key
    if (propertyScoped.value === 'fixed') {
        return
    }
    if ((isOmitted(elementScoped) && isOmitted(propertyScoped)) || getRule(node)?.definition.elemental) {
        return key
    }
    return error(node)
}
 ...
 * @param {object} node
 * @returns {SyntaxError|object[]}
 * @see {@link https://drafts.csswg.org/css-values-5/#typedef-random-key}
 *
 * It aborts parsing when the key is element-dependent while the context is not
 * elemental.
 *
 * It represents the key with `element-scoped property-index` when it is `auto`.
 */
function postParseRandomKey(key, node) {
    if (key.value === 'auto') {
        if (getRule(node)?.definition.elemental) {
            return list([omitted, keyword('element-scoped'), keyword('property-index-scoped')], ' ', ['<random-cache-key>', '<random-key>'])
        }
        return error(node)
    }
    return key
 ...
 *
 * It aborts parsing when the lower bound is lower than the higher bound.
 */
 ...
 *
 * It represents round() with its arguments in a comma separated list and with
 * explicit default values, to simplify later processing.
 */
function postParseRound({ value: [strategy,, a,, b], ...props }) {
    if (isOmitted(strategy)) {
        strategy = keyword('nearest', ['<rounding-strategy>'])
    }
    if (isOmitted(b) && strategy.value !== 'line-width') {
 ...
 *
 * It represents the name in lowercase if it is a standard property.
 */
function postParseStyleFeatureName(name, node) {
    const definition = getDeclarationDefinition(getRule(node), name.value)
    if (definition) {
        if (definition.name === '--*') {
            return definition
        }
        return { ...name, value: definition.name }
    }
    return error(node)
}
 ...
 * @see {@link https://drafts.csswg.org/css-conditional-5/#typedef-style-feature-name}
 *
 * It aborts parsing when the declaration value is `revert`, `revert-layer`, or
 * `revert-rule`.
 */
 ...
 * @param {object} node
 * @returns {SyntaxError|object}
 * @see {@link https://drafts.csswg.org/css-conditional-5/#typedef-supports-at-rule-fn}
 *
 * It aborts parsing when the argument is not a defined at-rule.
 *
 * It represents the function with its argument in lowercase.
 */
function postParseSupportsAtRule(fn, node) {
    const { value } = fn
    const lowercase = toLowerCase(value.value)
    const definition = findRule(`@${lowercase}`)
    if (definition && !definition.qualified) {
        return { ...fn, value: { ...value, value: lowercase } }
    }
    return error(node)
}
 ...
 * @param {object} node
 * @returns {SyntaxError|object}
 * @see {@link https://drafts.csswg.org/css-conditional-5/#typedef-supports-env-fn}
 *
 * It aborts parsing when the argument is not a <dashed-ident> or a defined
 * standard environment variable.
 *
 * It represents the function with its argument in lowercase.
 */
function postParseSupportsEnvironmentVariable(fn, node) {
    const { value } = fn
    if (value.value.startsWith('--')) {
        return fn
    }
    const lowercase = toLowerCase(value.value)
    if (environment[lowercase]) {
        return { ...fn, value: { ...value, value: lowercase } }
    }
    return error(node)
}
 ...
 * @param {object} node
 * @returns {SyntaxError|object}
 * @see {@link https://drafts.csswg.org/css-conditional-5/#typedef-supports-font-format-fn}
 *
 * It aborts parsing when the argument is a <string>.
 */
function postParseSupportsFontFormat(fn, node) {
    if (fn.value.types.includes('<string>')) {
        return error(node)
    }
    return fn
}
 ...
 * @param {object} node
 * @returns {SyntaxError|object}
 * @see {@link https://drafts.csswg.org/css-conditional-5/#typedef-supports-named-feature-fn}
 *
 * It aborts parsing when the argument is not a defined named feature.
 *
 * It represents the function with its argument in lowercase.
 */
function postParseSupportsNamedFeature(fn, node) {
    const { value } = fn
    const lowercase = toLowerCase(value.value)
    if (namedFeatures.includes(lowercase)) {
        return { ...fn, value: { ...value, value: lowercase } }
    }
    return error(node)
}
 ...
 *
 * It aborts parsing when symbols are missing for the declared symbol type.
 */
...
function postParseSyntax(syntax, node) {
    if (
        getRuleName(node) === '@property'
        && cssWideKeywords.includes(toLowerCase(serializeComponentValue(syntax)))
    ) {
        return error(node)
    }
    return syntax
 ...
 * @returns {object|object[]}
 * @see {@link https://drafts.csswg.org/css-values-5/#typedef-syntax-single-component}
 *
 * It represents the component in lowercase if it is an identifier.
 */
function postParseSyntaxSingleComponent(component) {
    if (component.types.includes('<ident>')) {
        return { ...component, value: toLowerCase(component.value) }
    }
    return component
}
 ...
 */
function postParseSyntaxString(syntax, node) {
    return parseGrammar(syntax.value, '<syntax>', node) ?? error(node)
}
 ...
 * @param {object} node
 * @returns {SyntaxError|object}
 * @see {@link https://drafts.csswg.org/css-values-5/#typedef-syntax-type-name}
 *
 * It aborts parsing when the name `url` is used in <attr-type>.
 */
function postParseSyntaxTypeName(name, node) {
    if (name.value === 'url' && isProducedBy(getFunction(node), '<attr-type>')) {
        return error(node)
    }
    return name
}
 ...
 *
 * It represents the list with an <explicit-track-list> when all track sizes are
 * explicit, to simplify later processing.
 */
 ...
 *
 * It represents the range with a custom representation.
 */
function postParseUnicodeRange(range, node) {
    const text = []
    range.flat().forEach(({ end, start }) => {
        const representation = node.input.source.slice(start, end)
        if (toLowerCase(representation) !== 'u') {
 ...
 * @param {object} node
 * @returns {SyntaxError|object|object[]}
 * @see {@link https://drafts.csswg.org/css-view-transitions-2/#propdef-view-transition-class}
 *
 * It aborts parsing when some name is a reserved keyword in the context.
 */
function postParseViewTransitionClass(names, node) {
    if (isList(names) && names.some(name => reserved.viewTransitionClass.includes(toLowerCase(name.value)))) {
        return error(node)
    }
    return names
}
 ...
 * @param {object} node
 * @returns {SyntaxError|object}
 * @see {@link https://drafts.csswg.org/css-view-transitions-2/#propdef-view-transition-name}
 *
 * It aborts parsing when the name is a reserved keyword in the context.
 */
 ...
 * @param {object} type
 * @returns {SyntaxError|object}
 * @see {@link https://drafts.csswg.org/css-view-transitions-2/#typedef-view-transition-type}
 *
 * It aborts parsing when the type is a reserved keyword in the context.
 */
function postParseViewTransitionType(type, node) {
    const lowercase = toLowerCase(type.value)
    if (reserved.viewTransitionType.includes(lowercase) || lowercase.startsWith('-ua-')) {
        return error(node)
    }
    return type
}
 ...
 */
function postParseWholeValue(value, node) {
    const definition = { ...getDeclaration(node).definition, type: 'non-terminal' }
    if (isOmitted(value)) {
        return definition.name.startsWith('--') ? value : error(node)
    }
    return parseDeclarationValue(value, definition, node.context) ?? error(node)
}
 ...
 *
 * It represents zero with an angle when it is accepted as an alternative
 * (browser conformance).
 */
...
export default {
    ...
    "<'border-radius'>": postParseBorderRadiusProperty,
    "<'color-scheme'>": postParseColorScheme,
    "<'flex-basis'>": postParseFlexBasis,
    '<a-n-plus-b>': postParseAnB,
    '<angle>': postParseDimension,
    '<animateable-feature>': postParseAnimateableFeature,
    '<attr-unit>': postParseAttributeUnit,
    '<border-radius>': postParseBorderRadiusType,
    '<calc-interpolate()>': postParseInterpolate,
    '<calc-keyword>': postParseCalcKeyword,
    '<calc-product>': postParseCalcProduct,
    '<calc-size()>': postParseCalcSize,
    ...
    '<cycle()>': postParseCycle,
    '<dashed-ident>': postParseDashedIdentifier,
    ...
    '<font-family-name>': postParseFontFamilyName,
    ...
    '<function-token>': postParseFunctionToken,
    ...
    '<random-cache-key>': postParseRandomCacheKey,
    '<random-key>': postParseRandomKey,
    ...
    '<supports-at-rule-fn>': postParseSupportsAtRule,
    '<supports-env-fn>': postParseSupportsEnvironmentVariable,
    '<supports-font-format-fn>': postParseSupportsFontFormat,
    '<supports-named-feature-fn>': postParseSupportsNamedFeature,
    '<symbols()>': postParseSymbols,
    '<syntax-single-component>': postParseSyntaxSingleComponent,
    '<syntax-string>': postParseSyntaxString,
    '<syntax-type-name>': postParseSyntaxTypeName,
    '<syntax>': postParseSyntax,
    '<time>': postParseDimension,
    '<track-list>': postParseTrackList,
    '<transform-interpolate()>': postParseInterpolate,
    '<try-tactic>': postParseListToDeclaredOrder,
    '<urange>': postParseUnicodeRange,
    '<view-transition-type>': postParseViewTransitionType,
    '<voice-family-name>': postParseVoiceFamilyName,
    '<whole-value>': postParseWholeValue,
    '<zero>': postParseZero,
    '@font-palette-values': postParseFontPaletteValues,
    '@property': postParseProperty,
    ...
    'view-transition-class': postParseViewTransitionClass,
