 ...
 *
 * It aborts parsing when names are for <'container-name'> and includes `none`,
 * or when it is a single name for <container-name> that is `none`.
 */
...
function postParseFeatureTagValue(tag, parser, node) {
    return /^[\u0000-\u007F]{4}$/.test(tag[0].value) ? tag : error(node)
}
...
function postParseGridLine(line, parser, node) {
    ...
    return (type.has('custom-ident') && (value === 'auto' || value === 'span')) ? undefined : line
}
...
function postParseKeyframeName(name, parser, node) {
    ...
    if (value === 'none') {
        ...
        const ident = parser.parse(tokenize(new Stream(value)), '<custom-ident>')
        if (ident) {
            return ident
        }
    }
