...
function postParseCustomFunctionDefinition(definition, node, parser) {
    if (isFailure(parser.parseCSSGrammar(definition[0].name, '<dashed-ident>', node.context))) {
        return error(node)
    }
    return definition
}
...
function postParseFunctionParameter(parameter, node, parser) {
    ...
    if (!isOmitted(type)) {
        if (!isOmitted(defaultValue)) {
            ...
            if (isFailure(parser.parseCSSGrammar(defaultValue[1], definition, context))) {
                return error(node)
            }
        }
...
function postParseWholeValue(value, node, parser) {
    ...
    return parser.parseCSSDeclarationValue(value, definition, context) ?? error(node)
}
