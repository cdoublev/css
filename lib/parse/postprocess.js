...
const { isColon, isOmitted, isWhitespace } = require('../utils/value.js')
const { isComputationallyIndependent, parseSyntax } = require('./syntax.js')
 ...
 *
 * It aborts parsing when symbols are not ordered of strictly descending weight.
 */
...
function postParseAnB(notation, node) {
    ...
    for (const component of (Array.isArray(notation) ? notation : [notation])) {
        if (!isOmitted(component)) {
            const { end, unit = '', value } = component
            // Invalid whitespace between an optional `+` and `n`
            if (value === '+' && text === '' && isWhitespace(node.input.data.at(end))) {
                return error(node)
            }
            text += value + unit
        }
 ...
 * @param {object} node
 * @returns {SyntaxError|object}
 * @see {@link https://drafts.csswg.org/css-values-5/#valdef-progress-animation-timeline}
 *
 * It aborts parsing when the timeline is a keyword used as <progress> value.
 */
function postParseAnimationTimeline(timeline, node) {
    if (timeline.types.includes('<keyword>') && findParent(node, node => node.definition.name === '<progress>')) {
        return error(node)
    }
    return timeline
}
 ...
 *
 * It represents radii by replacing an omitted radius with a default value.
 */
 ...
 *
 * It aborts parsing when the scheme is a reserved keyword in the context.
 */
function postParseColorScheme(scheme, node) {
    if (Array.isArray(scheme) && scheme[0].some(identifier => reserved.colorScheme.includes(identifier.value.toLowerCase()))) {
        return error(node)
    }
    return scheme
}
 ...
 *
 * It aborts parsing when some name includes a reserved keyword in the context.
 */
function postParseContainerNameProperty(name, node) {
    return Array.isArray(name) && name.some(name => postParseContainerNameType(name, node))
        ? error(node)
        : name
}
 ...
 *
 * It aborts parsing when the name includes a reserved keyword in the context.
 */
function postParseContainerNameType(name, node) {
    return reserved.containerName.includes(name.value.toLowerCase())
        ? error(node)
        : name
}
 ...
 *
 * It aborts parsing the identifier when it is a CSS reserved identifier or a
 * CSS-wide keyword.
 */
...
function postParseFirstValid(substitution, node, { parseCSSGrammar }) {
    ...
    if (substitution.value.some(value => parseCSSGrammar(value, definition, context))) {
        return substitution
    }
    return error(node)
}
...
/**
 * @param {object|object[]} flow
 * @param {object} node
 * @returns {SyntaxError|object|object[]}
 * @see {@link https://drafts.csswg.org/css-regions-1/#propdef-flow-into}
 *
 * It aborts parsing when the flow name is a reserved keyword in the context.
 */
function postParseFlowInto(flow, node) {
    if (Array.isArray(flow) && reserved.flowInto.includes(flow[0].value.toLowerCase())) {
        return error(node)
    }
    return flow
}
 ...
 *
 * It aborts parsing when the format is a string representing an unrecognized
 * legacy format.
 */
function postParseFontFormat(format, node) {
    const { types, value } = format
    if (types.includes('<string>') && !legacyStringFontFormats.includes(value)) {
        return error(node)
    }
    return format
}
 ...
 *
 * It discards @font-palette-values when it is lacking mandatory descriptor
 * declarations.
 */
function postParseFontPaletteValues(palette, node) {
    if (palette.value.declarations.find(declaration => declaration.name === 'font-family')) {
        return palette
    }
    return error(node)
}
...
function postParseForgivingSelectorList(selectors, node) {
    const { context: { strict } } = node
    const valid = []
    for (const selector of selectors) {
        if (selector === null || selector instanceof SyntaxError) {
            if (strict) {
                return error(node)
            }
        } else {
 ...
 * @param {object} node
 * @returns {SyntaxError|object}
 * @see {@link https://drafts.csswg.org/css-mixins-1/#typedef-function-dependency-list}
 * @see {@link https://drafts.csswg.org/css-mixins-1/#typedef-function-parameter-list}
 *
...
function postParseFunctionParameter(parameter, node) {
    ...
    if (node.parent.children.some(parameter => parameter.value[0].value === value)) {
        return error(node)
    }
    return parameter
}
 ...
 *
 * It aborts parsing when the orientation is not either `auto`, `0`, `0deg`,
 * `90`, `90deg`.
 */
...
function postParseGridLine(line, node) {
    ...
    if (line.types.includes('<custom-ident>')) {
        const value = line.value.toLowerCase()
        if (value === 'auto') {
            return error(node)
        }
        if (value === 'span') {
 ...
 *
 * It aborts parsing the color when its value is not hexadecimal number.
 */
 ...
 *
 * It aborts parsing when the selector value is not an identifier.
 */
 ...
 *
 * It aborts parsing the integer when its value is not an integer or is out of
 * range.
 */
 ...
 *
 * It aborts parsing the keyword when its value does not match the specified
 * value, or legacy mapped value.
 *
 * It replaces a legacy alias value by the target value.
 */
function postParseKeyword(keyword, { context, definition: { range } }) {
    ...
    const substitutions = compatibility.values[context.declaration?.definition.name]?.get(range)
    if (substitutions) {
        const { aliases = [], mappings = [] } = substitutions
        if (aliases.includes(lowercase)) {
            return { ...keyword, value: range }
        }
        if (mappings.includes(lowercase)) {
            return { ...keyword, value: lowercase }
        }
    }
    return null
...
function postParseLineNames(names, node) {
    return (names.value.some(name => reserved.lineName.includes(name.value.toLowerCase())))
        ? error(node)
        : names
}
 ...
 *
 * It aborts parsing the name when it is prefixed with `min` or `max` but is not
 * used in a plain (declaration) context.
 *
 * It represents the aliased name with its target name.
 */
 ...
 *
 * It aborts parsing the declaration when its value does not match the feature
 * value definition.
 */
function postParseMediaFeaturePlain(declaration, { context }, parser) {
    const [name,, input] = declaration
    const unprefixed = name.value.replace(/(min|max)-/, '')
    const definition = descriptors[context.rule.definition.name][unprefixed].value
 ...
 *
 * It aborts parsing the range when the matched feature type is discrete or when
 * some value does not match the feature value definition.
 */
function postParseMediaFeatureRange(range, { context }, parser) {
    let [left,, name,, right] = range
    if (name.types.includes('<mf-value>')) {
        const value = name
        name = left
        left = value
    }
    const { type, value } = descriptors[context.rule.definition.name][name.value]
    if (
        type === 'range'
        && parser.parseCSSValue([left], value, context)
        && (!right || parser.parseCSSValue([right], value, context))
    ) {
        return range
    }
    return null
...
function postParseNDashDigitDimension(dimension) {
    ...
    if (/^n-\d+$/.test(unit) && Number.isInteger(value)) {
        return dimension
    }
    return null
}
...
function postParseNDashDimension(dimension) {
    const { unit, value } = dimension
    if (unit === 'n-' && Number.isInteger(value)) {
        return dimension
    }
    return null
}
 ...
 *
 * It aborts parsing the numeric when its value is out of range.
 */
...
function postParsePath(path, node) {
    const { value: [fill,, ...tail] } = path
    if (!isOmitted(fill) && node.context.declaration?.definition.name === 'offset-path') {
 ...
 * @param {object} node
 * @returns {SyntaxError|object[]}
 * @see {@link https://drafts.csswg.org/css-values-5/#typedef-progress}
 *
 * It aborts parsing when the progress calculation type does not match <number>
 * or <percentage>.
 */
function postParseProgress(progress, node) {
    const [value] = progress
    if (isCalculation(value)) {
        const type = getCalculationType(value, '<number>')
        if (!matchNumericType(type, ['<number>', '<percentage>'], '<number>')) {
            return error(node)
        }
        progress[0] = simplifyCalculationTree(value, '<number>')
    }
    return progress
}
 ...
 *
 * It discards @property when it is lacking mandatory descriptor declarations.
 */
function postParseProperty(property, node, parser) {
    const { value: { declarations } } = property
    if (!declarations.some(declaration => declaration.name === 'inherits')) {
        return error(node)
    }
    const syntax = declarations.findLast(declaration => declaration.name === 'syntax')
    if (!syntax) {
        return error(node)
    }
    const { value: { definition } } = syntax
    const initial = declarations.find(declaration => declaration.name === 'initial-value')
    if (definition === '*') {
        if (initial && initial.value.length === 1 && cssWideKeywords.includes(initial.value[0].value)) {
            return error(node)
        }
        return property
    }
    if (initial && !isOmitted(initial.value)) {
        const match = parser.parseCSSGrammar(initial.value, definition)
        if (match && isComputationallyIndependent(match)) {
            return property
        }
    }
    return error(node)
...
function postParsePseudoClassSelector(selector, node, parser) {
    ...
    if (name) {
        ...
        const { classes: { functions: { [lowercase]: definition } } } = pseudos
...
function postParsePseudoElementSelector(selector, node, parser) {
    ...
    if (name) {
        const lowercase =  name.toLowerCase()
        const { elements: { functions: { [lowercase]: definition } } } = pseudos
...
function postParseRadialGradient(gradient, node) {
    const [configuration] = gradient
    if (!isOmitted(configuration) && !isOmitted(configuration[0])) {
        const [[aspect]] = configuration
        if (!isOmitted(aspect) && aspect.every(component => !isOmitted(component))) {
 ...
 *
 * It aborts parsing when circle() is specified with two <radial-radius>.
 */
 ...
 *
 * It aborts parsing when a range has a lower bound lower than the higher bound.
 */
 ...
 *
 * It represents the round function with default optional values.
 */
function postParseRound({ value: [strategy,, a,, b], ...props }) {
    ...
    return { ...props, value: [strategy, a, b] }
}
 ...
 *
 * It aborts parsing the integer when its value is not an integer or when it was
 * specified without a leading `+` or `-`.
 */
function postParseSignedInteger(integer) {
    const { sign, value } = integer
    if (sign && Number.isInteger(value)) {
        return integer
    }
    return null
}
 ...
 * @returns {SyntaxError|object}
 *
 * It aborts parsing the feature when it is a declaration whose value is either
 * `revert` or `revert-layer`.
 */
function postParseStyleFeature(feature, node, parser) {
    if (feature.types.includes('<declaration>')) {
        feature = parser.parseCSSDeclaration(feature)
        if (feature?.value?.value?.startsWith?.('revert')) {
            return null
        }
    }
    return feature
 ...
 * @param {object} parser
 * @returns {object|null}
 * @see {@link https://drafts.csswg.org/css-conditional-3/#typedef-supports-decl}
 * @see {@link https://github.com/w3c/csswg-drafts/issues/8795}
 *
 * It aborts parsing the block when its declaration is invalid.
 */
function postParseSupportsDeclaration(block, node, parser) {
    const declaration = parser.parseCSSDeclaration(block.value)
    return declaration ? map(block, () => declaration) : null
}
 ...
 *
 * It aborts parsing when the edge is `auto` for the `text-box` shorthand.
 */
function postParseTextBoxEdge(edge, node) {
    if (node.context.declaration?.definition.name === 'text-box' && edge.value === 'auto') {
        return error(node)
    }
    return edge
}
 ...
 * @see {@link https://drafts.csswg.org/css-inline-3/#propdef-text-box}
 *
 * It aborts parsing when the trim is `none` for the `text-box` shorthand.
 */
function postParseTextBoxTrim(trim, node) {
    if (node.context.declaration?.definition.name === 'text-box' && trim.value === 'none') {
        return error(node)
    }
    return trim
}
...
function postParseViewTransitionName(name, node) {
    if (reserved.viewTransitionName.includes(name.value.toLowerCase())) {
        return error(node)
    }
    return name
}
...
module.exports = {
    ...
    "<'text-box-edge'>": postParseTextBoxEdge,
    "<'text-box-trim'>": postParseTextBoxTrim,
    '<an+b>': postParseAnB,
    ...
    '<supports-decl>': postParseSupportsDeclaration,
