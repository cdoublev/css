 ...
 * @returns {object|null}
 */
function getMediaFeatureDefinition(name, node) {
    switch (node.context.rule?.definition.name) {
        case '@container':
            return descriptors['@container'][name]
        default:
            return descriptors['@media'][name]
    }
}
...
function postParseMediaFeatureBoolean(name, node) {
    const { type, value } = getMediaFeatureDefinition(name.value, node)
...
function postParseMediaFeatureName(name, node) {
    ...
    const context = node.context.rule?.definition.name ?? '@media'
    const target = compatibility.descriptors[context]?.aliases.get(unprefixed)
    if (lowercase === unprefixed) {
        if (target) {
            return { ...name, value: target }
        }
        if (getMediaFeatureDefinition(lowercase, node)) {
            return { ...name, value: lowercase }
        }
    } else if (node.parent?.parent?.definition.name === '<mf-plain>'/* only for <mf-name> tests: */ || !node.parent) {
        if (target) {
            return { ...name, value: `${lowercase.includes('min-') ? 'min' : 'max'}-${target}` }
        }
        if (getMediaFeatureDefinition(unprefixed, node)) {
            return { ...name, value: lowercase }
        }
    }
    return null
...
function postParseMediaFeaturePlain(declaration, node, parser) {
    ...
    const definition = getMediaFeatureDefinition(unprefixed, node).value
    return parser.parseCSSValue([input], definition, node.context) ? declaration : null
}
 ...
 */
function postParseMediaFeatureRange(range, node, parser) {
    ...
    if (
        type === 'range'
        && parser.parseCSSValue([left], value, node.context)
        && (!right || parser.parseCSSValue([right], value, node.context))
    ) {
        return range
    }
    return null
