...
const { isCalculation, isColon, isComputationallyIndependent, isOmitted, isWhitespace } = require('../utils/value.js')
...
function postParseProperty(property, node, parser) {
    ...
    let syntax = declarations.findLast(declaration => declaration.name === 'syntax')
    if (!syntax) {
        return error(node)
    }
    let initial = declarations.find(declaration => declaration.name === 'initial-value')
    syntax = serializeCSSComponentValue(syntax.value)
    if (syntax === '*') {
    ...
    if (initial && !isOmitted(initial.value)) {
        initial = parser.parseCSSGrammar(initial.value, syntax)
        if (initial && isComputationallyIndependent(initial)) {
            return property
        }
    }
    return error(node)
