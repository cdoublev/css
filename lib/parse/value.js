 ...
 */
function setOmittedCombinationValues(values, types, combination, location) {
    types.forEach(type => {
        if (!combination.includes(type)) {
            const { position } = type
...
function backtrack(list, node) {
    ...
    const { combinations, nodes, types, startIndex, values } = state
    if (repeat) {
        if (isLastNodeWithState(node)) {
            ...
            nodes.pop()
            // Resume parsing against the next node type
            state.skip = true
        }
    ...
    if (type === ' ') {
        ...
        // Discard the last match value
        const { location } = values.pop()
        // Get the node type of the discarded match value
        const nodeType = value[values.length]
        // Restore it in the remaining node types
        types.unshift(nodeType)
        // Resume parsing the discarded match value
    ...
    if (type === '&&') {
        if (isLastNodeWithState(node)) {
            list.moveTo(startIndex)
            const { value: nextCombination } = combinations.next()
            // Start over parsing against another combination
            if (nextCombination) {
                types.splice(0, Infinity, ...nextCombination)
                values.splice(0)
                return state
            }
            // Match failure
    ...
    if (type === '||') {
        if (isLastNodeWithState(node)) {
            ...
            if (nextCombination) {
                ...
                types.splice(0, Infinity, ...nextCombination)
                values.splice(0)
                setOmittedCombinationValues(values, value, nextCombination, startIndex)
                return state
            }
            // Match failure
            states.delete(node)
        ...
        // Resume parsing against another combination
        const { value: nextCombination } = combinations.next()
        const nextTypes = nextCombination.slice(nextCombination.length - types.length - 1)
 ...
 * @returns {object[]}
 */
function* permute(nodes, length, size, p, permutation = [], used = []) {
    if (p === size) {
    ...
    for (let i = 0; i < length; ++i) {
        if (!used[i]) {
            used[i] = true
            permutation[p] = nodes[i]
            yield* permute(nodes, length, size, p + 1, permutation, used)
            used[i] = false
        }
    }
}
 ...
 * @returns {Iterator}
 *
 * It generates combinations in lexicographic order.
 */
function* createCombinationIterator(nodes, every = true) {
    const { length } = nodes
    let size = length
    while (true) {
        yield* permute(nodes, length, size, 0)
        if (every || --size === 0) {
            return
        }
    }
...
function createState(node, list) {
    ...
    if (repeat) {
        ...
        const combinations = createCombinationIterator(setNodePositions(value), type === '&&')
        const { value: types } = combinations.next()
        state = { combinations, startIndex, types, values: [] }
    } else if (type === '|') {
 ...
 * @param {boolean} round
 * @returns {object}
 * @see https://drafts.csswg.org/css-values-4/#calc-internal
 */
 ...
 * @returns {string|null}
 * @see https://drafts.csswg.org/css-values-4/#calc-syntax
 */
 ...
 * @returns {object|null}
 *
 * This is a workaround to an exceeded maximum call stack size occurring when
 * backtracking using recursion, because most user agents do not support tail
 * call optimization.
 */
function parseCombinationTrempoline(parse, list, node) {
    let state = useState(node, list)
    let match = parse(list, state)
    while (!state.abort && !match) {
        state = backtrack(list, node)
        if (!state.abort) {
            match = parse(list, state)
        }
    }
    return match
}
 ...
 * @returns {List}
 */
function parseCombination(list, { abort, types, values }) {
    if (abort) {
        return null
    }
...
function parseNode(list, node, isRoot) {
    ...
    switch (type) {
        ...
        case '&&':
        case '||':
            return parseCombinationTrempoline(parseCombination, list, node)
        case '|':
            return parseSingle(list, node)
        // Single type
        case 'basic':
 ...
 * @returns {object}
 * @see https://drafts.csswg.org/css-syntax-3/#consume-simple-block
 */
 ...
 */
function consumeDeclaration(components) {
    const { value: name } = components.consume()
    const declaration = { name, value: [] }
    components.consumeRunOf(' ')
    components.consume(':', false)
    components.consumeRunOf(' ')
    while (!components.atEnd()) {
        declaration.value.push(consumeComponentValue(components))
    }
    if (declaration.value.at(-1) === ' ') {
        declaration.value.pop()
    }
    const [penultimate, last] = declaration.value.slice(-2)
    if (penultimate === '!' && last.type?.has('ident') && last.value === 'important') {
        declaration.value.splice(-2, 2)
 ...
 * @param {object} tokens
 * @returns {object}
 * @see https://drafts.csswg.org/css-syntax-3/#consume-an-at-rule
 */
function consumeAtRule(tokens) {
    const prelude = []
    const atRule = { name: tokens.consume().value, prelude }
    for (const token of tokens) {
        if (token === ';') {
            return atRule
        }
 ...
 * @returns {object}
 * @see https://drafts.csswg.org/css-syntax-3/#normalize-into-a-token-stream
 */
 ...
 * @param {string} string
 * @returns {object[]}
 * @see https://drafts.csswg.org/cssom/#parse-a-css-declaration-block
 */
