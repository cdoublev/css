...
function consumeIdentifierLike(chars) {
    ...
    if (chars.consume('(')) {
        ...
        return { end: chars.index + 1, start, types: ['<function-token>'], value: serializeIdentifier({ value }) }
    }
    return identifier
}
...
function consumeNumeric(chars) {
    ...
    if (startsWithIdentifier(...chars.next(3))) {
        ...
        return { end: chars.index + 1, start, types: ['<dimension-token>'], unit: toLowerCase(unit.value), value }
    }
    if (chars.consume('%')) {
