...
function consumeNumber(chars) {
    ...
    return { signed, type: new Set([type]), value: convertStringToNumber(representation) }
}
...
function consumeNumeric(chars) {
    ...
    if (startsWithIdentifier(...chars.next(3))) {
        return { signed, type: new Set(['dimension']), unit: consumeIdentifier(chars).toLowerCase(), value }
    }
    if (chars.consume('%')) {
        // TODO: cleanup validation functions for numerics
        return { signed, type: new Set(['percentage']), unit: '%', value }
    }
    return number
}
