...
function consumeEscaped(chars) {
    ...
    let representation = ''
    if (char === null) {
        createError({ message: 'invalid escape sequence (missing code point)', type: SyntaxError })
...
function consumeNumber(chars, representation) {
    ...
    return {
        ...
        type: new Set(['number']),
        value: convertStringToNumber(number),
    }
}
...
function consumeString(chars, representation) {
    ...
    createError({ context: representation, message: 'unclosed string', type: SyntaxError })
    return { representation, type: new Set(['string']), value }
}
...
function consumeUrl(chars, representation) {
    ...
    createError({ context: representation, message: 'unclosed url', type: SyntaxError })
    return {
        representation,
        type: new Set(['url']),
        value,
...
function consumeToken(chars) {
    ...
    if (char === '#') {
        ...
        createError({ message: 'invalid escaped newline', type: SyntaxError })
