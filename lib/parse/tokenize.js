...
function consumeUrl(chars, { start }) {
    ...
    for (const char of chars) {
        ...
        if (isQuote(char) || char === '(' || isNonPrintableCharacter(char)) {
            ...
            value += `${char}${consumeBadUrl(chars)}`
            return { end: chars.index + 1, start, types: ['<bad-url-token>'], value }
        }
        if (char === '\\') {
 ...
 *
 * It deviates from the specification by tokenizing consecutive whitespaces or
 * comments into a single <whitespace-token>.
 *
 * It deviates from the specification by tokenizing a whitespace, `:`, `;`, `,`,
 * `(`, `)`, `[`, `]`, `{`, `}`, into a <delimiter-token>.
 */
