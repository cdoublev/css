...
const { hasSubstitution } = require('../values/validation.js')
 ...
 * @param {object[]} list
 * @returns {object}
 * @see {@link https://drafts.csswg.org/css-backgrounds-3/#background}
 *
 * It allocates each background layer value to the comma separated list of the
 * corresponding longhand values, replacing an omitted `background-box` value by
 * the `background-clip` value, if any or vice-versa, or any other omitted
 * longhand value by its corresponding initial value.
 *
...
function parseBorderRadius(value) {
    ...
    return horizontal.map((radius, index) => createList([radius, vertical[index]]))
}

/**
 * @param {object[]} list
 * @returns {object}
 * @see {@link https://drafts.csswg.org/css-fonts-4/#font-prop}
 */
