...
import { isColon, isComma, isDelimiter, isOmitted, isWhitespace, length, omitted } from '../values/value.js'
...
function preParseCombinator(node) {
    ...
    const pseudoElement = findSibling(
        ...
        true)
    if (pseudoElement) {
        const { value: [, [, { name, value }]] } = pseudoElement
        const definition = pseudos.elements[name ?? value]
