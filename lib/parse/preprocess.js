...
import { isColon, isComma, isDelimiter, isFunction, isOmitted, isWhitespace } from '../utils/value.js'
 ...
 * @see {@link https://drafts.csswg.org/css-values-4/#calc-syntax}
 *
 * It aborts parsing when the next token is a sum operator that is not preceded
 * by a whitespace.
 */
...
function preParseCalcValue(node) {
    // Assert: <calc-value> is always used in a function
    const context = findFunction(node, node => !isProducedBy(node.context, '<calc-value>'))
    let count = context.state.extended.get('calc-terms') ?? 0
    if (count++ === MAX_CALCULATION_TERMS) {
        return error(node)
    }
    context.state.extended.set('calc-terms', count)
}
 ...
 * @see {@link https://drafts.csswg.org/selectors-5/#typedef-combinator}
 *
 * It aborts parsing a combinator when complex selectors are invalid in the
 * context.
 *
 * It aborts parsing when the input is at the back of a pseudo-element with no
 * internal structure, or in a functional pseudo compounded to it, but is not at
 * its end. It simply aborts parsing the combinator if the input is at its end.
 *
...
function preParseCombinator(node) {
    ...
    const pseudoElement = findSibling(
        ...
        true)
    if (pseudoElement) {
        const { value: [, [, pseudo]] } = pseudoElement
        const definition = isFunction(pseudo)
            ? pseudos.elements.functions[pseudo.name]
            : pseudos.elements.identifiers[pseudo.value]
 ...
 * comma (with some exceptions).
 *
 * It accepts an omitted value as replacing a comma except between a repetition
 * of values.
 */
...
function hasInvalidSiblingCombinator({ input, parent }) {
    ...
    return -1 < input.index && isOmitted(combinator) && !isWhitespace(input.current)
}
 ...
 *
 * It aborts parsing a complex selector unit when the preceding combinator is
 * omitted with no interleaving whitespace.
 */
 ...
 * @see {@link https://drafts.csswg.org/selectors-4/#white-space}
 *
 * It aborts parsing a compound selector when the preceding combinator is
 * omitted and there is no interleaving whitespace.
 *
...
function preParseImageSet(node) {
    ...
    if (
        token?.types[0] === '<function-token>' && toLowerCase(token.value) === 'image-set'
...
function preParseImageSetOption(node) {
    const token = node.input.next()
    if (token) {
        const name = token.value ? toLowerCase(token.value) : null
        if (
...
function preParseInterpolate(node) {
    if (properties[getDeclarationName(node)]?.animate === false) {
        const name = node.input.next()?.name
        return (name && toLowerCase(name) === 'interpolate') ? error(node) : null
    }
}
 ...
 * @returns {SyntaxError|undefined}
 *
 * It aborts parsing when the context is element-dependent.
 */
