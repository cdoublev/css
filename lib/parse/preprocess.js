 ...
 * @returns {boolean}
 */
function hasArbitrarySibling({ definition, parent: { definition: { type, value } } }) {
    if (type === 'repetition') {
        return false
    }
    const next = value[value.indexOf(definition) + 1]
...
function preParseComma(node) {
    ...
    if (current === undefined || isComma(current)) {
        ...
        if (parent.definition.type === 'repetition') {
            return null
        }
        return omitted
    }
    if (comma) {
        if (input.atEnd() && !hasArbitrarySibling(node)) {
            return error(node)
        }
        return comma
    }
