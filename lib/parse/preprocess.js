...
const { findFunction, findParent, findSibling, getDeclarationName, getFunction, getRule, isProducedBy } = require('../utils/context.js')
...
function preParseTypeInterpolate(node) {
    ...
    const next = input.next()
    if (
        next?.types[0] === '<function>'
        && next.name === definition.name.slice(1, -3)
