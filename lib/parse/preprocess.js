...
function preParseCombinator({ tree }, node) {
    ...
    if (findParentTree(tree, tree => tree.root.definition.name === 'compound-selector')) {
...
function preParseCompoundSelector(parser, node) {
    ...
    if (combinator?.value.omitted && !isWhitespace(list.current)) {
        return null
    }
    const topLevel = getTopLevelTree(tree)
    // Logical combination pseudo-classing pseudo-element
    if (tree !== topLevel && pseudos.logical[`${topLevel.list.current.name}()`]) {
        const pseudoElement = findLastNode(
            topLevel.nodes,
            node => node.definition.name === 'pseudo-element-selector',
