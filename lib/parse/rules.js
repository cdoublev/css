 ...
 *
 * It invalidations the an+b notation when there is no whitespace between `+`
 * and `n`.
 *
 * It transforms the matched component values into a simplified representation
 * of the `an+b` expression as a plain object with `a` and `b` as properties.
 */
 ...
 * @param {Parser} parser
 * @returns {object[]|null}
 * @see {@link https://drafts.csswg.org/selectors/#typedef-attr-matcher}
 *
 * It invalidates an attribute matcher when there is a whitespace between its
 * specific symbol and the equal sign.
 */
 ...
 * @param {Parser} parser
 * @returns {object[]|null}
 * @see {@link https://drafts.csswg.org/selectors/#typedef-class-selector}
 *
 * It invalidates a class name selector when there is a whitespace between the
 * full stop and the class name.
 */
 ...
 * @param {Parser} parser
 * @returns {object|null}
 * @see {@link https://drafts.csswg.org/selectors/#typedef-combinator}
 *
 * It invalidates a double pipe combinator with a whitespace separating the two
 * pipes.
 */
 ...
 * @param {object[]} selector
 * @returns {object[]|null}
 * @see {@link https://drafts.csswg.org/selectors/#typedef-complex-selector}
 * @see {@link https://drafts.csswg.org/selectors/#pseudo-element-structure}
 *
 * It invalidates a complex selector that includes a compound selector ending
 * with a pseudo-element without a defined internal structure, combined with
 ...
 * @param {object[]} selector
 * @returns {object[]|null}
 * @see {@link https://drafts.csswg.org/selectors/#typedef-compound-selector}
 * @see {@link https://drafts.csswg.org/selectors/#sub-pseudo-elements}
 *
 * It invalidates a compound selector that includes a pseudo-element that:
 * - precedes a pseudo-class that can not apply to this pseudo-element
 * - is not an originating element but precedes another another pseudo-element
 */
 ...
 * @param {object} id
 * @returns {object|null}
 * @see {@link https://drafts.csswg.org/selectors/#typedef-id-selector}
 *
 * It invalidates an identifier selector that is an hexadecimal number.
 *
 * Ideally, it would be parsed as a terminal but `<id-selector>` is expanded to
 * `<hash>` (token) in Selectors.
 */
function postParseIDSelector(id) {
    return id.type.has('id') ? id : null
}
 ...
 * @param {Parser} parser
 * @returns {object[]}
 * @see {@link https://drafts.csswg.org/selectors/#typedef-ns-prefix}
 *
 * It invalidates a namespace prefix that has not been declared or when there is
 * a whitespace between the prefix and the pipe.
 */
 ...
 * @returns {object|null}
 * @see {@link https://drafts.csswg.org/css-nesting/#nesting-selector}
 *
 * It invalidates a nesting selector that is not used in a nested style rule.
 */
 ...
 *
 * It removes the optional fill-rule when it is `nonzero` (default).
 */
 ...
 * @param {Parser} parser
 * @returns {object|null}
 * @see {@link https://drafts.csswg.org/selectors/#typedef-pseudo-class-selector}
 *
 * It invalidates:
 ...
 *
 * It normalizes the representation by removing the `:` colon(s).
 */
 ...
 * @param {Parser} parser
 * @returns {object|null}
 * @see {@link https://drafts.csswg.org/selectors/#typedef-pseudo-element-selector}
 *
 * It invalidates a pseudo-element when there is a whitespace between the two
 * preceding colons.
 */
 ...
 *
 * It invalidates a gradient when:
 * - the gradient shape is a circle whose size is authored with two values, a
 * percentage, or a negative value
 * - the gradient shape is an ellipse whose size is authored with a single value
 * or negative value(s)
 *
 * It replaces an omitted gradient position value by `center` (default).
 *
 ...
 * @param {Parser} parser
 * @returns {object[]|null}
 * @see {@link https://drafts.csswg.org/selectors/#typedef-type-selector}
 *
 * It invalidates a universal selector qualified with a namespace prefix when
 * there is a whitespace between the pipe and the selector.
 */
function postParseTypeSelector(selector, { list }) {
    const [namespace, { value }] = selector
    if (value === '*' && !namespace.omitted && list.prev() === ' ') {
        return null
    }
    return selector
}
 ...
 *
 * It invalidates a unicode range when:
 * - it does not start with `U+` or `u+`
 * - it start or end has 0 or more than 6 hexadecimal digits
 * - it start and end values are not separated with `-`
 * - it start has a valeur greater than the end value
 * - it end has a value greather than `10FFFF` (max. allowed code point)
 * - it includes an invalid wildcard `?`
 *
 * It normalizes the unicode range into an object representing the `start` and
 * `end` code points of the range as decimal values.
 */
 ...
 * @param {Parser} parser
 * @returns {object[]|null}
 * @see {@link https://drafts.csswg.org/selectors/#typedef-wq-name}
 *
 * It invalidates a type or attribute qualified by a namespace prefix when there
 * is a whitespace between the pipe and the type or attribute.
 */
function postParseWQName(name, { list }) {
    const [namespace] = name
    if (!namespace.omitted && list.prev() === ' ') {
        return null
    }
    return name
}
