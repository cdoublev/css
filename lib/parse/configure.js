...
import { enterVirtualContext } from '../utils/context.js'
...
function enterStyleRule(node) {
    ...
    if (node.definition.name === '<declaration>' && context.definition.name !== 'supports') {
        return node
    }
    return { context: enterVirtualContext(node, '@style'), ...props }
}
