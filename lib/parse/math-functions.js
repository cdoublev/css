 ...
 * @returns {object|null}
 */
function getCalculationNode({ parent }) {
    if (parent) {
        if (parent.definition.name === 'calc-sum') {
            return parent
        }
        return getCalculationNode(parent)
    }
    return null
}
 ...
 * @returns {object}
 *
 * If `type` is included in a <type-percentage>/etc mixed type `production`, it
 * returns the production name as the context type and `type` as the resolution
 * type, otherwise it only returns `type` as the context type.
 */
function getNumericContextTypes(type, production) {
    if (type === 'integer') {
        type = 'number'
    }
    if (type !== 'percentage' && production?.type === '|' && production.value.some(({ name }) => name === 'percentage')) {
 ...
 * @returns {object|null}
 */
function getParentCalculationNode({ parent }, index = -1) {
    if (parent) {
        const { tail } = parent
        if (tail.definition.type === 'function') {
            return getParentCalculationNode(parent, index)
        }
        return getCalculationNode(tail)
    }
    return null
}
