...
import {
    ...
    MEDIA_NETWORK_STATE_LOADING,
    MEDIA_READY_STATE_HAVE_CURRENT_DATA,
    PRECEDING_NODE_POSITION,
    TEXT_NODE_TYPE,
} from '../utils/dom/constants.js'
import { every, filter, iterate, some } from '../utils/dom/collection.js'
import {
    findAncestorElement,
    findFirstSiblingElementIndex,
    findLastSiblingElementIndex,
    ...
    isSVGElement,
    isShadowRoot,
    leftCombinedElements,
    rightCombinedElements,
    traverse,
} from '../utils/dom/node.js'
import { getElementLanguageTag, matchesLanguageRange, parseLanguageTag } from '../utils/dom/language.js'
import { isDelimiter, isOmitted, list } from '../values/value.js'
import { serializeComponentValue, serializeIdentifier } from '../serialize.js'
import CSSPseudoElement from '../cssom/CSSPseudoElement.js'
 ...
 * @returns {boolean}
 */
function canUseElementCache(context, options) {
    return !context.anchors && !context.scopes && !options.includeSubtrees
}
 ...
 * @param {object} options
 */
function addSelectorToElementCache(selector, element, context, options) {
    if (canUseElementCache(context, options)) {
        selector = serializeComponentValue(selector)
        const { elementCache, tree } = context
        if (elementCache.has(selector)) {
            const cache = elementCache.get(selector)
            if (cache.has(tree)) {
                cache.get(tree).push(element)
            } else {
                cache.set(tree, [element])
            }
        } else {
            elementCache.set(selector, new Map([[tree, [element]]]))
        }
    }
}

/**
 * @param {ElementImpl|ParentNodeImpl} tree
 * @param {object} [context]
 * @returns {object}
 */
function createContext(tree, { elementCache = new Map, namespaces = new Map, root = tree.getRootNode({}), ...context } = {}) {
    return { elementCache, namespaces, root, ...context }
}
 ...
 * @param {ElementImpl} element
 * @returns {boolean}
 * @see {@link https://drafts.csswg.org/selectors-4/#focus-pseudo}
 * @see {@link https://html.spec.whatwg.org/multipage/semantics-other.html#selector-focus}
 */
function hasFocus(element) {
    if (isNavigableContainer(element)) {
        return false
    }
    const { document: { focus } } = states.get(element.ownerDocument.defaultView)
    if (!focus) {
        return false
    }
    if (focus.node === element) {
        return true
    }
    const { shadowRoot } = element
    if (shadowRoot) {
        return hasFocusWithin(shadowRoot)
    }
    return false
}
 ...
 * @param {ElementImpl} element
 * @returns {boolean}
 * @see {@link https://drafts.csswg.org/selectors-4/#focus-visible-pseudo}
 */
function hasFocusVisible(element) {
    const { document: { focus }, user } = states.get(element.ownerDocument.defaultView)
    return focus?.node === element && (focus.visible || user.visibleFocus)
}

/**
 * @param {ElementImpl} element
 * @returns {boolean}
 * @see {@link https://drafts.csswg.org/selectors-4/#focus-within-pseudo}
 */
function hasFocusWithin(element) {
    ...
    for (const child of traverse(element, { inclusive: true, includeSubtrees: true })) {
        if (child === focus.node) {
            return true
        }
    }
...
function hasMeterValue(element, value) {
    return isHTMLElement(element, 'meter') && getMeterValueRegion(element) === value
}
...
function hasRelativeMatching(element, selectors, context, options) {
    ...
    return selectors.some(([combinator, selector]) => {
        ...
        function matchRight(left, combinator, compound, index = 0) {
            for (const right of rightCombinedElements(left, combinator)) {
                if (
                    matchCompoundSelector(right, compound, context, options)
...
function hasSlotted(element) {
    return isHTMLElement(element, 'slot') && 0 < element.assignedNodes({ flatten: true }).length
}
 ...
 * @returns {boolean}
 */
function isAnBOfSelector(element, { value: { a, b } }, selector, context, options, reverse) {
    const predicate = isOmitted(selector) ? sibling => sibling : sibling => matchElementAgainstSelectors(sibling, selector[1], context, options)
    const n = reverse ? findLastSiblingElementIndex(element, predicate) : findFirstSiblingElementIndex(element, predicate)
 ...
 * @see {@link https://drafts.csswg.org/selectors-4/#link-pseudo}
 * @see {@link https://html.spec.whatwg.org/multipage/semantics-other.html#selector-link}
 */
function isAnyLink(element) {
    return element.getAttributeNode('href') && (isHTMLElement(element, 'a', 'area') || isSVGElement(element, 'a'))
}
...
function isBuffering(element) {
    return isHTMLElement(element, ...medias)
        && !element.paused
...
function isCandidateForConstraintValidation(element) {
    return isHTMLElement(element, ...submitables)
        // && is not barred from constraint validation
        && element.type !== 'button'
        && element.type !== 'hidden'
        && element.type !== 'reset'
        && !(isHTMLElement(element, 'button') && !isSubmitButton(element))
        && !(isHTMLElement(element, 'input', 'textarea') && element.hasAttribute('readonly'))
        && isEnabled(element)
        && !findAncestorElement(element, element => isHTMLElement(element, 'datalist'))
}
...
function isDefault(element) {
    if (isHTMLElement(element, 'input') && inputTypeAttributes[element.type].includes('checked')) {
...
function isDisabled(element) {
    if (isHTMLElement(element, 'optgroup')) {
    ...
    if (isHTMLElement(element, 'option')) {
        ...
        const group = findAncestorElement(
            element,
            element => isHTMLElement(element, 'optgroup'),
            element => isHTMLElement(element, 'datalist', 'hr', 'option', 'select'))
        return group?.hasAttribute('disabled')
    }
    if (isHTMLElement(element, 'button', 'fieldset', 'input', 'select', 'textarea')) {
        if (element.hasAttribute('disabled')) {
            return true
        }
        const fieldSet = findAncestorElement(
            element,
            element => isHTMLElement(element, 'fieldset') && element.hasAttribute('disabled'))
        if (fieldSet) {
            const { firstElementChild } = fieldSet
            return !isHTMLElement(firstElementChild, 'legend')
                || !findAncestorElement(element, parent => parent === firstElementChild, parent => parent === fieldSet)
        }
    }
    return false
}
...
function isEnabled(element) {
    return isHTMLElement(element, 'button', 'fieldset', 'input', 'optgroup', 'option', 'select', 'textarea')
        && !isDisabled(element)
}
...
function isFirstOfType(element) {
    return !findPreviousSiblingElement(element, sibling => haveSameType(element, sibling))
}
...
function isHeading(element) {
    return isHTMLElement(element, 'h1', 'h2', 'h3', 'h4', 'h5', 'h6')
}
...
function isHostContextMatching(element, selector, context) {
    if (isHost(element, context)) {
        ...
        return matchCompoundSelector(element, selector, context)
            || findAncestorElement(element, element => matchCompoundSelector(element, selector, context))
    }
    return false
}
...
function isIndeterminate(element) {
    if (isHTMLElement(element, 'progress')) {
    ...
    if (isHTMLElement(element, 'input')) {
        ...
        if (type === 'radio') {
            ...
            const selectors = parseGrammar(`input[type=radio][name="${name}"]:checked`, '<selector-list>', root.defaultView)
...
function isInvalid(element) {
    if (isHTMLElement(element, 'form', 'fieldset')) {
...
function isLastOfType(element) {
    return !findNextSiblingElement(element, sibling => haveSameType(element, sibling))
}
...
function isMuted(element) {
    return isHTMLElement(element, ...medias) && element.muted
}

/**
 * @param {ElementImpl} element
 * @returns {boolean}
 * @see {@link https://html.spec.whatwg.org/multipage/document-sequences.html#navigable-container}
 */
function isNavigableContainer(element) {
    return isHTMLElement(element, 'frame', 'iframe', 'embed', 'object')
}
...
function isOpen(element) {
    return element.hasAttribute('open') && isHTMLElement(element, 'details', 'dialog')
}
...
function isOptional(element) {
    ...
    if (isHTMLElement(element, 'select', 'textarea')) {
...
function isPaused(element) {
    return isHTMLElement(element, ...medias) && element.paused
}
...
function isReadOnly(element) {
    return !isReadWrite(element) && isHTMLElement(element)
}
...
function isSeeking(element) {
    return isHTMLElement(element, ...medias) && element.seeking
}
...
function isSubmitButton(element) {
    return (isHTMLElement(element, 'button') && element.type === 'submit')
        || (isHTMLElement(element, 'input') && (element.type === 'image' || element.type === 'submit'))
}
 ...
 * @returns {boolean}
 */
function matchPseudoClassSelector(element, { name, value }, context, options) {
    switch (name) {
        case undefined:
            break
        ...
        case 'has':
            return hasRelativeMatching(element, value, context, options)
        case 'host':
        ...
        case 'not':
            return !isMatching(element, value, context)
        case 'nth-child':
            return isAnBOfSelector(element, ...value, context, options)
        case 'nth-last-child':
            return isAnBOfSelector(element, ...value, context, options, true)
    ...
    switch (value) {
        ...
        case 'focus-visible':
            return hasFocusVisible(element)
        case 'focus-within':
...
/**
 * @param {CSSPseudoElementImpl} element
 * @param {*[]} selector
 * @param {object} context
 * @param {object} options
 * @returns {boolean}
 */
function matchPseudoCompoundSelector(element, [pseudoElement, pseudoClasses], context, options) {
    return element.selectorText === serializeComponentValue(pseudoElement)
        && pseudoClasses.every(selector => matchPseudoClassSelector(element, selector, context, options))
}

/**
 * @param {CSSPseudoElementImpl|ElementImpl} element
 * @param {*[]} selector
 * @param {object} context
 * @param {object} options
 * @returns {boolean}
 */
function matchPseudoCompoundSelectors(element, selector, context, options) {
    for (let index = selector.length - 1; -1 < index; index--) {
        if (!CSSPseudoElement.isImpl(element)) {
            return false
        }
        if (!matchPseudoCompoundSelector(element, selector[index], context, options)) {
            return false
        }
 ...
 * @returns {boolean}
 */
function matchPseudoElementAgainstComplexSelector(element, selector, context, options) {

    let [head, tail] = selector
    let compound
    let pseudoCompounds

    // Resolve and replace <pseudo-compound-selector>* in the subject with empty list
    if (tail.length === 0) {
        [compound, pseudoCompounds] = head
        const unit = list([compound, list([], '')], '', ['<complex-selector-unit>'])
        selector = [unit, tail]
    } else {
        let combinator
        tail = [...tail];
        [combinator, [compound, pseudoCompounds]] = tail.pop()
        const unit = list([compound, list([], '')], '', ['<complex-selector-unit>'])
        tail.push(list([combinator, unit], ''))
        selector = [head, list(tail, '')]
    }
    selector = list(selector, '', ['<complex-selector>'])

    return matchPseudoCompoundSelectors(element, pseudoCompounds, context, options)
        && matchComplexSelector(element.element, selector, context, options)
}
 ...
 */
function matchPseudoElementAgainstSelectors(element, selectors, context, options) {

    if (selectors.types.includes('<complex-real-selector-list>')) {
        return false
    }

    context = createContext(element.element, context)
    options = createOptions(context, options)

    if (options.all) {
        return selectors.every(selector => matchPseudoElementAgainstComplexSelector(element, selector, context, options))
    }
    return selectors.find(selector => matchPseudoElementAgainstComplexSelector(element, selector, context, options)) ?? null
}
 ...
 * @returns {boolean}
 */
function matchAttributeValueSelector(actual, matcher, expected) {
...
function* matchAttributeNameSelector(element, [prefix, name], { namespaces = new Map }) {
    ...
    name = name.value
    if (isHTMLElement(element) && isHTMLDocument(element.ownerDocument)) {
    ...
    if (element.hasAttributeNS(namespace, name)) {
        yield element.getAttributeNodeNS(namespace, name)
    }
}
...
function matchAttributeSelector(element, { value: selector }, context) {
    if (selector.types.at(-1) === '<wq-name>') {
        return !matchAttributeNameSelector(element, selector, context).next().done
    }
    const [name, matcher, { value: specified }, { value: modifier }] = selector
    const lowercase = toLowerCase(specified)
    for (const attribute of matchAttributeNameSelector(element, name, context)) {
        let expected
        let actual
        if (isHTMLElement(element) && isCaseInsensitiveAttribute(attribute, modifier)) {
            expected = lowercase
            actual = toLowerCase(attribute.value)
        } else {
            expected = specified
            actual = attribute.value
        }
        if (matchAttributeValueSelector(actual, matcher, expected)) {
            return true
        }
    }
    return false
 ...
 * @returns {boolean}
 */
function matchSubclassSelector(element, selector, context, options) {
    return selector.every(subclass => {
        ...
        return matchPseudoClassSelector(element, subclass[1], context, options)
    })
}
 ...
 * @see {@link https://html.spec.whatwg.org/multipage/semantics-other.html#case-sensitivity-of-selectors}
 */
function matchTypeNameSelector(element, { value: selector }) {
    if (isHTMLElement(element) && isHTMLDocument(element.ownerDocument)) {
 ...
 * @returns {boolean}
 */
function matchCompoundSelector(element, selector, context, options) {
    if (selector.types.includes('<complex-selector-unit>')) {
        const [compound, pseudos] = selector
        const origin = CSSPseudoElement.is(element) ? element.element : element
        return matchPseudoCompoundSelectors(element, pseudos, context, options)
            && (isOmitted(compound) || matchCompoundSelector(origin, compound, context, options))
    }
    // It can be omitted only when matching a pseudo-element
    if (isOmitted(selector)) {
        return true
    }
    const [type, subclasses] = selector
    return matchTypeSelector(element, type, context)
        && matchSubclassSelector(element, subclasses, context, options)
}
 ...
 */
function matchComplexSelector(element, [head, tail], context, options) {
    let { skipSubject } = options
    function matchLeft(right, index) {
        if (index === -1) {
            return skipSubject || matchCompoundSelector(right, head, context, options)
        }
        const { [index]: [combinator, compound] } = tail
        if (skipSubject || matchCompoundSelector(right, compound, context, options)) {
            if (right === element) {
                addSelectorToElementCache(compound, element, context, options)
                skipSubject = false
            }
            for (const left of leftCombinedElements(right, combinator)) {
 ...
 */
function matchElementAgainstSelectors(element, selectors, context, options) {
    context = createContext(element, context)
    options = createOptions(context, options)
    if (options.all) {
        const results = context.selectorCache?.get(element)
        return selectors.filter(selector => results?.get(selector) ?? matchComplexSelector(element, selector, context, options))
    }
    const results = new Map
    context.selectorCache?.set(element, results)
    const match = selectors.find(selector => {
        const result = matchComplexSelector(element, selector, context, options)
 ...
 * @yields {ElementImpl}
 */
function* matchTreeAgainstSelectorsUsingDOMGetters(tree, selectors, context, options) {

    if (options.includeSubtrees) {
        return
    }

    const { anchors, namespaces, root, scopes } = context

    for (const selector of selectors) {

        // Resolve <compound-selector> and <pseudo-compound-selector>*
        const [head, tail] = selector
        let compound = tail.at(-1)?.[1] ?? head
        let pseudoCompounds = []
        if (compound.types.includes('<complex-selector-unit>')) {
            [compound, pseudoCompounds] = compound
        }

        let candidates
        if (isOmitted(compound)) {

            if (tree.nodeType === DOCUMENT_FRAGMENT_NODE_TYPE) {
                continue
            }
            candidates = tree.getElementsByTagNameNS('*', '*')

        } else {

            let [type, subclasses] = compound

            // <type-selector>
            if (isDelimiter('&', type)) {
                candidates = anchors ?? scopes?.roots ?? [root.documentElement ?? root.host]
            } else if (tree.nodeType !== DOCUMENT_FRAGMENT_NODE_TYPE) {
                if (isOmitted(type)) {
                    candidates = tree.getElementsByTagNameNS('*', '*')
                } else {
                    let [prefix, name] = type
                    prefix = serializeComponentValue(prefix)
                    if (prefix === '') {
                        ...
                        prefix = namespaces.get(prefix.slice(0, -1))
                    }
                    if (isHTMLDocument(tree.ownerDocument ?? tree)) {
                        name = toLowerCase(name.value)
                    } else {
                        name = name.value
                    }
                    candidates = tree.getElementsByTagNameNS(prefix, name)
                }
            }

            // <subclass-selector>*
            if (candidates) {
                candidates = filter(candidates, element => matchSubclassSelector(element, subclasses, context, options))
            } else {
                const unmatched = new Set(candidates)

                // Find an initial set of candidates
                for (const subclass of unmatched) {
                    if (isDelimiter('&', subclass)) {
                        candidates = anchors ?? scopes?.roots ?? [root.documentElement ?? root.host]
                        unmatched.delete(subclass)
                        break
                    }
                    if (subclass.types.includes('<id-selector>') && options.first) {
                        const candidate = tree.getElementById(subclass.value)
                        if (!candidate) {
                            return
                        }
                ...
                // Filter candidates with the remaining unmatched type and subclass selectors
                candidates = filter(candidates, element => matchSubclassSelector(element, unmatched, context, options))
                if (tree.nodeType === DOCUMENT_FRAGMENT_NODE_TYPE) {
                    candidates = filter(candidates, element => matchTypeSelector(element, type, context))
                }
            }
        }

        for (const element of iterate(candidates)) {
            if (scopes?.limits?.some(limit => limit.contains(element))) {
                continue
            }
            if (!matchPseudoCompoundSelectors(element, pseudoCompounds, context, options)) {
                continue
            }
            if (matchElementAgainstSelectors(element, [selector], context, { ...options, skipSubject: true })) {
                yield [element, selector]
            }
        }
...
/**
 * @param {NodeImpl[]} trees
 * @param {*[]} selectors
 * @param {object} context
 * @param {object} options
 * @yields {object}
 */
function* matchTreesAgainstSelectorsUsingElementCache(trees, selectors, context, options) {

    const { elementCache } = context

    if (elementCache.size === 0 || !canUseElementCache(context, options)) {
        return
    }

    selectorLoop: for (const selector of selectors) {

        const [head, tail] = selector
        const subject = serializeComponentValue(tail.at(-1)?.[1] ?? head)

        if (!elementCache.has(subject)) {
            continue
        }

        for (const [tree, elements] of elementCache.get(subject).entries()) {
        ...
        // There is a match against the selector in all trees
        selectors.delete(selector)
    }
}
 ...
 */
function matchTreesAgainstSelectors(trees, selectors, context, options) {
    ...
    // Optimization: get matches using the element cache
    for (const [element, tree] of matchTreesAgainstSelectorsUsingElementCache(trees, unmatched, context, options)) {
        if (!matches.has(element)) {
            matches.set(element, tree)
        }
        if (first) {
            break
        }
    ...
    for (const tree of trees) {
        ...
        // Optimization: get matches using DOM element getters
        for (const [element, selector] of matchTreeAgainstSelectorsUsingDOMGetters(tree, unmatched, context, options)) {
            if (!matches.has(element)) {
                matches.set(element, tree)
            }
            if (first) {
        ...
        // Normal: get matches by traversing the tree
        for (const element of traverse(tree, options)) {
            ...
            if (matchElementAgainstSelectors(element, [...unmatched.values()], context, options)) {
                if (first) {
                    return element
                }
                matches.set(element, tree)
    ...
    if (selectors.length !== unmatched.size) {
        sorted.sort((a, b) => {
            if (trees.indexOf(a[1]) < trees.indexOf(b[1])) {
                return -1
            }
            if (trees.indexOf(b[1]) < trees.indexOf(a[1])) {
                return 1
            }
            const position = a[0].compareDocumentPosition(b[0])
            if (position & FOLLOWING_NODE_POSITION) {
                return -1
            }
            if (position & PRECEDING_NODE_POSITION) {
                return 1
            }
            return 0
        })
...
export {
    ...
    matchPseudoElementAgainstSelectors,
