 ...
 * @returns {boolean}
 */
function matchPseudoCompoundSelectors(element, selector, context, { pseudos }) {
    return selector.every(selector => {
        const [pseudoElement, pseudoClasses] = selector
        const type = `::${pseudoElement.types[0] === '<function>' ?  `${pseudoElement.name}()` : pseudoElement.value}`
        const pseudo = pseudos.includes(type) && element.pseudo(type)
        return pseudo && pseudoClasses.length === 0
    })
}
