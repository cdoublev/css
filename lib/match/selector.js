...
function isHost(element, context) {
    return element === context.tree.getRootNode({}).host
}
 ...
 */
function isScopingRoot(element, context) {
    const { scope, tree } = context
    switch (scope) {
        case undefined: {
...
function* traverse(tree, context = {}, inclusive = context.scopes?.inclusive) {
    const { scopes, excludeSubtrees } = context
 ...
 */
function matchComplexSelector(element, [head, tail], context) {
    function matchLeft(right, index) {
        if (index === -1) {
            return matchCompoundSelector(right, head, context)
        }
        const { [index]: [combinator, compound] } = tail
        if (matchCompoundSelector(right, compound, context)) {
 ...
 */
function matchElementAgainstSelectors(element, selectors, context, all) {
    if (all) {
        return selectors.filter(selector => matchComplexSelector(element, selector, context))
    }
    return selectors.some(selector => matchComplexSelector(element, selector, context))
}
...
function matchTreesAgainstSelectors(trees, selectors, context = {}, first) {
    ...
    const { scopes: { roots } = {}, pseudos } = context
    ...
    trees.forEach(tree => {
        context = roots ? { ...context, scope: tree, tree } : { ...context, tree }
