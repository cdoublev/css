 ...
 */
function hasHighValue(element) {
    return isOfType(element, 'meter') && getMeterValueRegion(element) === 'high'
}
 ...
 */
function hasLowValue(element) {
    return isOfType(element, 'meter') && getMeterValueRegion(element) === 'low'
}
 ...
 * @returns {boolean}
 */
function hasNextSibling({ nextElementSibling }, predicate) {
 ...
 */
function hasOptimalValue(element) {
    return isOfType(element, 'meter') && getMeterValueRegion(element) === 'optimal'
}
 ...
 * @returns {boolean}
 */
function hasPreviousSibling({ previousElementSibling }, predicate) {
...
function hasRelativeMatching(element, selectors, options) {
    return selectors.some(([combinator, [head, tail]]) => {
        let anchored = getRightCombinedElements(element, combinator)
            .filter(element => matchCompoundSelector(element, head, options))
        if (anchored.length === 0) {
            return false
        }
        for (const [combinator, compound] of tail) {
            anchored = anchored
                .flatMap(element => getRightCombinedElements(element, combinator))
                .filter(element => matchCompoundSelector(element, compound, options))
            if (anchored.length === 0) {
                return false
            }
        }
...
function isFirstOfType(element) {
    return !hasPreviousSibling(element, sibling => haveSameType(element, sibling))
}
...
function isLastOfType(element) {
    return !hasNextSibling(element, sibling => haveSameType(element, sibling))
}
 ...
 * @returns {Element[]}
 */
function getAncestors({ parentElement }) {
    const elements = []
    while (parentElement) {
 ...
 * @returns {Element[]}
 */
function getLeftCombinedElements(element, combinator) {
    switch (serializeComponentValue(combinator)) {
        ...
        case '+':
            return element.previousElementSibling ? [element.previousElementSibling] : []
        case '~':
 ...
 * @returns {Element[]}
 */
function getNextSiblings({ nextElementSibling }) {
    const elements = []
    while (nextElementSibling) {
 ...
 * @returns {Element[]}
 */
function getPreviousSiblings({ previousElementSibling }) {
    const elements = []
    while (previousElementSibling) {
        elements.push(previousElementSibling)
 ...
 * @returns {Element[]}
 */
function getRightCombinedElements(element, combinator) {
    switch (serializeComponentValue(combinator)) {
        case '>':
...
function* traverse(tree, options = {}, inclusive = options.scopes?.inclusive) {
    const { limits, excludeShadowTrees } = options
...
function matchPseudoClassSelector(element, { name, value }, options) {
    ...
    switch (value) {
        ...
        case 'optimal-value':
            return hasOptimalValue(element)
        case 'optional':
 ...
 */
function matchAttributeNameSelector({ attributes, namespaceURI, ownerDocument }, selector, { namespaces = new Map }) {
    ...
    if (prefix === '*') {
        return attributes.find(attribute => attribute.localName === name)
    }
    const namespace = prefix ? namespaces.get(prefix) : null
    return attributes.find(({ localName, namespaceURI }) => localName === name && namespaceURI === namespace)
}
 ...
 */
function matchComplexSelector(element, selector, options) {
    const [head, tail] = selector
    let considered = [element]
    for (const [combinator, unit] of tail.reverse()) {
        considered = considered
            .filter(element => matchCompoundSelector(element, unit, options))
            .flatMap(element => getLeftCombinedElements(element, combinator))
        if (considered.length === 0) {
            return false
        }
    }
    return considered.some(element => matchCompoundSelector(element, head, options))
}
