...
import { every, find, iterate, some } from '../utils/dom/collection.js'
import {
    findAncestor,
...
import { isHTMLDocument, traverse } from '../utils/dom/tree.js'
 ...
 * @returns {object}
 */
function createContext(element, { elementCache = new Map, tree = element.getRootNode({}), ...context } = {}) {
    return { elementCache, tree, ...context }
}
...
function isScopingRoot(element, { scopes, tree }) {
    ...
    return element === (tree.host ?? tree.documentElement)
}
...
function matchAttributeSelector(element, { value: selector }, context) {
    ...
    switch (serializeComponentValue(matcher)) {
        ...
        case '|=':
            return expected === actual || actual.startsWith(`${expected.split('-')[0]}-`)
        case '~=':
 ...
 */
function matchTreesAgainstSelectors(trees, selectors, context, { first, includeSubtrees } = {}) {
    ...
    const { anchors, elementCache, scopes, pseudos } = context
    ...
    const options = {
        includeSubtrees,
        inclusive: context.scopes?.inclusive,
        limits: context.scopes?.limits,
    }
