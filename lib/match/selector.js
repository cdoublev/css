...
function matchComplexSelector(element, [head, tail], context, cached) {
    function matchLeft(right, index) {
        if (index === -1) {
            if (matchCompoundSelector(right, head, context)) {
    ...
    return matchLeft(element, tail.length - (cached ? 2 : 1))
}
...
function matchElementAgainstSelectors(element, selectors, context, all) {
    if (all) {
        ...
        return selectors.filter(selector => results?.get(selector) ?? matchComplexSelector(element, selector, context))
    }
    const results = new Map
    context.selectorCache?.set(element, results)
    return selectors.some(selector => {
        const result = matchComplexSelector(element, selector, context)
...
function matchTreesAgainstSelectors(trees, selectors, context = {}, first) {
    ...
    if (!anchors && !scopes && 0 < elementCache.size) {
        ...
        for (const selector of selectors) {
            ...
            const subject = tail.at(-1)?.[1] ?? head
            const key = serializeComponentValue(subject)

            if (elementCache.has(key)) {
                const elements = elementCache.get(key)
                // The subject selector is not combined to other selectors
                if (subject === head) {
                    elements.forEach(element => selected.add(element))
                    continue
                }
                elements.forEach(element => {
                    if (matchElementAgainstSelectors(element, [selector], context)) {
