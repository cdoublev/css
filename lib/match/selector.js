 ...
 * @returns {boolean}
 */
function canUseElementCache({ anchors, scopes }, options) {
    return !anchors && !scopes && !options.includeSubtrees
}
 ...
 * @param {object} [options]
 * @returns {*[]|null}
 * @see {@link https://drafts.csswg.org/selectors-4/#match-against-pseudo-element}
 */
...
function matchCompoundSelector(element, selector, context, options) {
    if (selector.types.includes('<complex-selector-unit>')) {
        ...
        return (isOmitted(compound) || matchCompoundSelector(element, compound, context, options))
            && matchPseudoCompoundSelectors(element, pseudos, context, options)
    }
...
function* matchTreeAgainstSelectorsUsingDOMGetters(tree, selectors, context, options) {
    ...
    const { includeSubtrees, first } = options
        ...
        if (isOmitted(compound)) {
            ...
            if (candidates) {
                ...
                for (const subclass of unmatched) {
                    ...
                    if (subclass.types.includes('<id-selector>') && first) {
