...
function* traverse(tree, options = {}, inclusive = options.scopes?.inclusive) {
    ...
    for (let index = 0; index < children.length; index++) {
        yield* traverse(children.item(index), options, true)
    }
 ...
 */
function matchElementAgainstSelectors(element, selectors, options, all) {
    if (all) {
        return selectors.filter(selector => matchComplexSelector(element, selector, options))
    }
    return selectors.some(selector => matchComplexSelector(element, selector, options))
}
 ...
 */
function matchPseudoElementAgainstSelectors(pseudo, selectors, options) { }
 ...
 */
function matchTreesAgainstSelectors(trees, selectors, options = {}, first) {
    ...
    trees.forEach(tree => {
        ...
        for (const element of traverse(tree, options)) {
            if (matchElementAgainstSelectors(element, selectors, options)) {
                if (first) {
                    return selected
                }
            ...
            pseudos?.forEach(type => {
                ...
                if (pseudo && matchPseudoElementAgainstSelectors(pseudo, selectors, options)) {
