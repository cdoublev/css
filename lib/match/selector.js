 ...
 * @returns {boolean}
 */
function canUseElementCache({ anchors, scopes }) {
 ...
 */
function addSelectorToElementCache(selector, element, context, { elementCache }) {
    if (canUseElementCache(context)) {
...
function matchTreesAgainstSelectors(trees, selectors, context, options) {
    ...
    if (0 < elementCache.size && canUseElementCache(context)) {
    ...
    for (const tree of trees) {
        for (const element of traverse(tree, options)) {
            if (matchElementAgainstSelectors(element, selectors, context, options)) {
                if (first) {
                    return element
                }
                selected.add(element)
