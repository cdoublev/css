...
function matchFeature(feature, operator, value, globalObject) {
    switch (feature) {
        ...
        case 'display-mode': {
            const { document: { manifest } } = states.get(globalObject.top)
            if (manifest) {
        ...
        case 'display-state':
            return value === states.get(globalObject.top).agent.viewport.state
        case 'dynamic-range':
 ...
 * @returns {boolean}
 */
function matchType(type, globalObject) {
    switch (type) {
        case 'all':
            return true
        case 'print':
...
export default function match(query, globalObject) {
    ...
    if (matchType(body.value, globalObject)) {
