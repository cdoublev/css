...
import { getContrast, getLabLuminance } from '../values/color.js'
 ...
 * @returns {object}
 */
function getResolvedCalculationFunction({ range, round, value }, globalObject) {
...
function getResolvedLength(value, globalObject) {
    switch (value.unit) {
        ...
        case 'dvb':
        case 'dvh': {
            const { agent: { viewport: { interfaces: { bottom, top } } } } = states.get(globalObject)
        ...
        case 'dvi':
        case 'dvw': {
            const { agent: { viewport: { interfaces: { left, right } } } } = states.get(globalObject)
...
function getResolvedValue(value, globalObject, nested) {
    for (let index = value.types.length - 1; 0 <= index; index--) {
        switch (value.types[index]) {
            case '<angle>':
            case '<frequency>':
            case '<resolution>':
            case '<time>':
            ...
            case '<calc-function>': {
                value = getResolvedCalculationFunction(value, globalObject)
...
function matchFeature(feature, operator, value, globalObject) {
    switch (feature) {
        ...
        case 'any-hover': {
            const { system: { pointers } } = states.get(globalObject)
            if (value === 'none') {
        ...
        case 'color-index':
            return compareNumerics(value, operator, states.get(globalObject).system.display.colorIndex)
        ...
        case 'display-mode': {
            if (globalObject.frameElement) {
                return matchFeature(feature, value, globalObject.frameElement)
            }
            const { document: { manifest } } = states.get(globalObject)
            if (manifest) {
        ...
        case 'display-state':
            return value === states.get(globalObject.frameElement ?? globalObject).agent.viewport.state
        case 'dynamic-range':
        case 'video-dynamic-range':
            return value === (states.get(globalObject).system.display.hdr ? 'high' : 'standard')
        case 'environment-blending':
            return value === states.get(globalObject).system.display.blending
        case 'forced-colors':
            return value === (states.get(globalObject).user.forcedColors ? 'active' : 'none')
        case 'grid':
            return value === (states.get(globalObject).system.display.graphicMode ? 0 : 1)
        case 'height':
            return compareNumerics(value, operator, globalObject.innerHeight)
        case 'horizontal-viewport-segments':
            return compareNumerics(value, operator, states.get(globalObject).system.display.segments[0])
        case 'vertical-viewport-segments':
            return compareNumerics(value, operator, states.get(globalObject).system.display.segments[1])
        case 'hover':
            return value === (states.get(globalObject).system.pointers[0]?.motionable ? 'hover' : 'none')
        case 'inverted-colors':
            return value === (states.get(globalObject).user.invertedColors ? 'inverted' : 'none')
        case 'monochrome':
            return compareNumerics(value, operator, states.get(globalObject).system.display.monochrome)
        case 'nav-controls': {
            const { agent: { navigation } } = states.get(globalObject)
            if (value === 'none') {
        ...
        case 'overflow-block':
            return value === states.get(globalObject).agent.viewport.overflow[1]
        case 'overflow-inline':
            return value === states.get(globalObject).agent.viewport.overflow[0]
        case 'pointer':
            return value === (states.get(globalObject).system.pointers[0]?.precision ?? 'none')
        case 'prefers-color-scheme': {
            const { user: { forcedColors } } = states.get(globalObject)
            if (forcedColors) {
        ...
        case 'prefers-contrast': {
            const { user: { forcedColors, highContrast } } = states.get(globalObject)
            if (forcedColors) {
        ...
        case 'prefers-reduced-data':
            return value === (states.get(globalObject).user.reducedData ? 'reduce' : 'no-preference')
        case 'prefers-reduced-motion':
            return value === (states.get(globalObject).user.reducedMotion ? 'reduce' : 'no-preference')
        case 'prefers-reduced-transparency':
            return value === (states.get(globalObject).user.reducedTransparency ? 'reduce' : 'no-preference')
        case 'resizable':
            return value === (states.get(globalObject).agent.viewport.resizable ? 'true' : 'false')
        case 'resolution': {
            return compareNumerics(value, operator, globalObject.devicePixelRatio)
        }
        case 'scan':
            return value === (states.get(globalObject).system.display.interlaced ? 'interlace' : 'progressive')
        case 'scripting':
            return value === states.get(globalObject).agent.scripting
        case 'shape':
            return value === states.get(globalObject).system.display.shape
        case 'ua-color-scheme':
            return value === getUserPreferredColorScheme(globalObject)
        case 'update':
            return value === states.get(globalObject).system.display.update
        case 'width':
...
function matchMediaType(type, globalObject) {
    switch (type) {
        ...
        case 'print':
        case 'screen':
            return type === states.get(globalObject).agent.type
        default:
            return false
    }
}
