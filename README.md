...
```js
const { cssom, install, parser: { parseGrammar, parseStyleSheet } } = require('@cdoublev/css')
...
const stylesheet = parseStyleSheet('style {}')
...
const value = parseGrammar('(width < 30rem), (orientation: portrait)', '<media-query-list>')
```
